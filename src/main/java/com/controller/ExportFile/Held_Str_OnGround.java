...
public class Held_Str_OnGround extends AbstractPdfView {
	...
	public Held_Str_OnGround(String Type,List<String> TH,String Heading,String username,String foot){
		this.Type = Type;
		this.TH = TH;
		...
		for(int i=0;i< aList.size();i++) {
			...
			for(int j = 0;j<l.size();j++) {
				if(TH.get(0).equals("SR NO")) {
					relativeWidths[0] = (float) 0.20;
				}if(TH.get(1).equals("ARM CODE")) {
					relativeWidths[1] = (float) 0.30;
				}if(TH.get(2).equals("ARM DESK")) {
					relativeWidths[2] = (float) 3.00;
				}if(TH.get(3).equals("TOTAL")) {
					relativeWidths[3] = (float) 0.30;
				}
