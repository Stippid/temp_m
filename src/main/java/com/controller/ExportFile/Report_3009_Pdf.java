...
public class Report_3009_Pdf extends AbstractPdfView {
	...
	ArrayList<ArrayList<String>> list_authoffrs;
	ArrayList<ArrayList<String>> list_authciv;
	ArrayList<ArrayList<String>> list_postedoffrs;
	ArrayList<ArrayList<String>> list_postedciv;
	ArrayList<ArrayList<String>> list_trade;	
	ArrayList<ArrayList<String>> list_rd;
	...
	public Report_3009_Pdf(List<String> TH, List<String> TH1, List<String> TH2, List<String> TH3, List<String> TH4,
			List<String> TH5,List<String> TH6, String foot, String username, 
			ArrayList<ArrayList<String>> list_authoffrs,
			ArrayList<ArrayList<String>> list_authciv,
			ArrayList<ArrayList<String>> list_postedoffrs,
			ArrayList<ArrayList<String>> list_postedciv, 
			...
			String cont_div, String cont_bde, String hd_remarks1, String hd_remarks2) {
		this.TH = TH;this.TH2 = TH2;this.TH3 = TH3;this.TH4 = TH4;this.TH5 = TH5;this.TH6 = TH6;this.foot=foot;this.username = username;
		this.list_authoffrs = list_authoffrs;this.list_authciv = list_authciv;this.list_postedoffrs = list_postedoffrs;
		this.list_postedciv = list_postedciv;this.list_trade = list_trade;this.list_rd = list_rd;this.list_summary = list_summary;this.list_Main = list_Main;
	...
	protected void buildPdfMetadata(Map<String, Object> model, Document document, HttpServletRequest request) {
		...
		Chunk underline2 = new Chunk("IAFF - 3009: UNIT STR RETURN ", fontTableHeadingMainHead);
...
//		tableright.addCell(pi6);
		...
		*/
		...
		Chunk pSERVING = new Chunk("AUTH STR", fontTableHeading);
...
//		p1.setUnderline(0.1f, -2f);
		...
		table1.setWidths(new int[]{3,2,2,2,2});
		Paragraph p;
		...
		table2.setWidths(new int[]{2,2,2,2,2});
		...
		for(int t=0;t<TH3.size();t++) {
			p = new Paragraph(TH3.get(t),fontTableHeading);
			p.setAlignment(Element.ALIGN_CENTER);
			table2.addCell(p);
		}
...
//		Phrase p3 = new Phrase("RE-EMPLOYED", fontTableHeading);
		...
		Chunk pRE = new Chunk("POSTED STR", fontTableHeading);
		pRE.setUnderline(0.1f, -2f);
		p3.add(pRE);
		
		PdfPTable table3 = new PdfPTable(21);
		table3.setWidths(new int[]{4,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2});
		...
		if(list_postedoffrs.size() > 0) {
			for(int i=0;i< list_postedoffrs.size();i++) {
				List<String> l = list_postedoffrs.get(i);
				for(int j = 0;j<l.size();j++) {
					p = new Paragraph(l.get(j),fontTableValue2);
					table3.addCell(p);
				}
...
//		Phrase p4 = new Phrase("DESERTER", fontTableHeading);
		...
		Chunk pDESERTER = new Chunk("CIVILIAN EMPLOYEE", fontTableHeading);
		pDESERTER.setUnderline(0.1f, -2f);
		p4.add(pDESERTER);
		
		PdfPTable table4 = new PdfPTable(5);
		table4.setWidths(new int[]{2,2,2,2,2});
     ...
     Phrase p5 = new Phrase();
		...
		PdfPTable table5 = new PdfPTable(30);
		table5.setWidths(new int[]{2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2});
...
//		Chunk pauth = new Chunk(" AUTH AS PER WE & HELD STR AS PER IAFF-3008 ", fontTableHeading);
		Chunk pheld = new Chunk(" RANK AND TRADE WISE (JCOS/OR) ", fontTableHeading);
		pheld.setUnderline(0.1f, -2f);
		p7.add(pheld);
		
		PdfPTable table8 = new PdfPTable(11);
		table8.setWidths(new int[]{3,3,2,2,2,2,2,2,2,2,2});
		...
		if(list_trade.size() > 0) {
			for(int i=0;i< list_trade.size();i++) {
				List<String> l = list_trade.get(i);
				for(int j = 0;j<l.size();j++) {
					p = new Paragraph(l.get(j),fontTableValue2);
					table8.addCell(p);
				}
...
//			Chunk pauth = new Chunk(" AUTH AS PER WE & HELD STR AS PER IAFF-3008 ", fontTableHeading);
			Chunk psum = new Chunk(" RELIGIOUS DENOMINATION ", fontTableHeading);
			psum.setUnderline(0.1f, -2f);
			p6.add(psum);
			
			PdfPTable table7 = new PdfPTable(6);
			table7.setWidths(new int[]{3,3,2,2,2,2,});
