...
import com.dao.mnh.Daily_UnusuaLOccurrenceReportDAO;

@Controller
@RequestMapping(value = {"admin","/" ,"user"}) 
public class Daily_UnusuaLOccurrenceReportController {
	...
	@Autowired
	private Daily_UnusuaLOccurrenceReportDAO oc;
	...
	@RequestMapping(value = "/mnh_DR_unusual", method = RequestMethod.GET)
	public ModelAndView mnh_DR_unusual(ModelMap Mmap,HttpSession session,@RequestParam(value = "msg", required = false) String msg,HttpServletRequest request) {
	      Boolean val = roledao.ScreenRedirect("mnh_DR_unusual", session.getAttribute("roleid").toString());
          if(val == false) {
		...
		List<Map<String, Object>> list = oc.getsearch_Daily_unusual_Report("","", "ALL",to_date, date1, "ALL", "ALL");
		Mmap.put("list",list);
		
		return new ModelAndView("Report_Daily_unusual_occurrenceTiles");
	}
	
	
	@RequestMapping(value = "/getsearch_Daily_unusual_Report",method=RequestMethod.POST)
	public ModelAndView getsearch_Daily_unusual_Report(ModelMap Mmap,HttpSession session,@RequestParam(value = "msg", required = false) String msg,
		...
		List<Map<String, Object>> list = oc.getsearch_Daily_unusual_Report(sus1,unit1, cmd1,frm_dt1, to_dt1,serv1,cat1);
		Mmap.put("list", list);
