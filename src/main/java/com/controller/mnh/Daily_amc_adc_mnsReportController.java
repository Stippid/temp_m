...
import com.dao.mnh.Daily_amc_adc_mns_ReportDAO;


@Controller
@RequestMapping(value = {"admin","/" ,"user"})

public class Daily_amc_adc_mnsReportController {
	...
	@Autowired
	private Daily_amc_adc_mns_ReportDAO ad;
	
	MNH_CommonController mcommon = new MNH_CommonController();
	AllMethodsControllerOrbat all =new AllMethodsControllerOrbat();
	@RequestMapping(value = "/mnh_DR_adm2report", method = RequestMethod.GET)
	public ModelAndView mnh_DR_adm2report(ModelMap Mmap,HttpSession session,@RequestParam(value = "msg", required = false) String msg,HttpServletRequest request) {
	      Boolean val = roledao.ScreenRedirect("mnh_DR_adm2report", session.getAttribute("roleid").toString());
          if(val == false) {
		...
		List<Map<String, Object>> list = ad.getsearch_Daily_amc_adc_mns_Report("","","ALL",to_date, date1, "ALL");
		Mmap.put("list",list);
		
		return new ModelAndView("Report_Daily_amc_adc_mnsTiles");
	}
	@RequestMapping(value = "/getsearch_Daily_amc_adc_mns_Report",method=RequestMethod.POST)
	public ModelAndView getsearch_Daily_amc_adc_mns_Report(ModelMap Mmap,HttpSession session,@RequestParam(value = "msg", required = false) String msg,
		...
		List<Map<String, Object>> list = ad.getsearch_Daily_amc_adc_mns_Report(sus1,unit1, cmd1,frm_dt1, to_dt1,serv1);
		Mmap.put("list", list);
