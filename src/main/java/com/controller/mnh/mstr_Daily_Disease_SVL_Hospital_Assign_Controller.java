...
import com.models.mnh.TB_MED_DAILY_DISEASE_HOSP_ASSIGN;
...
public class mstr_Daily_Disease_SVL_Hospital_Assign_Controller {
	@Autowired
	...
	MNH_CommonController mcommon = new MNH_CommonController();
		...
		@RequestMapping(value = "/admin/mnh_daily_disease_svl_hospital_assign", method = RequestMethod.GET)
		public ModelAndView mnh_hospital_assign(ModelMap Mmap,HttpSession session,@RequestParam(value = "msg", required = false) String msg,HttpServletRequest request) {
			...
			Boolean val = roledao.ScreenRedirect("mnh_daily_disease_svl_hospital_assign", roleid);
			/*if(val == false) {
			...
			Mmap.put("msg", msg);
			return new ModelAndView("mnh_daily_dsurve_hospital_assignTiles");
		}
	
		
		@RequestMapping(value = "/daily_disease_svl_hospital_assignAction", method = RequestMethod.POST)
		public ModelAndView ms_hos_assignAction(@ModelAttribute("daily_disease_svl_hospital_assignCMD") TB_MED_DAILY_DISEASE_HOSP_ASSIGN rs,
				HttpServletRequest request,ModelMap model,HttpSession session,@RequestParam(value = "msg", required = false) String msg1) {
			String  roleid = session.getAttribute("roleid").toString();
		
		
			 Boolean val = roledao.ScreenRedirect("mnh_daily_disease_svl_hospital_assign", session.getAttribute("roleid").toString());
	         if(val == false) {
	           return new ModelAndView("AccessTiles");
	          }
	         if(request.getHeader("Referer") == null ) {
			...
			for(int i=0; i<values.length;i++)
			{
				...
				qce = sessionHQL.createQuery("select id  from TB_MED_DAILY_DISEASE_HOSP_ASSIGN b where b.sus_no =:sus_no and user_id =:deo_user_id" );
			...
			qce = sessionHQL.createQuery("select sus_no  from TB_MED_DAILY_DISEASE_HOSP_ASSIGN b where b.user_id =:deo_user_id" );
			qce.setParameter("deo_user_id", deo_user_id);
			@SuppressWarnings("unchecked")
			...
			result.removeAll(sus_no_list_ar);		
			 ...
			 for(int i =0; i<result.size();i++) {
				 ...
				 qce = sessionHQL.createQuery("delete from TB_MED_DAILY_DISEASE_HOSP_ASSIGN b where b.sus_no =:sus_no_list and user_id =:deo_user_id" );
				 qce.setParameter("deo_user_id", deo_user_id);
			 ...
			 model.put("msg", msg);			 
			 return new ModelAndView("redirect:mnh_daily_disease_svl_hospital_assign");		
		}	
}
