...
public class A_VehTypeOfStockController {
	
	@Autowired
	private Type_Of_Stock_ADAO tosA;
	...
	@RequestMapping(value = "/type_of_stock_a", method = RequestMethod.GET)
	public ModelAndView type_of_stock_a(ModelMap Mmap,HttpSession session,@RequestParam(value = "msg", required = false) String msg,HttpServletRequest request) {
		String  roleid = session.getAttribute("roleid").toString();
		Boolean val = roledao.ScreenRedirect("type_of_stock_a", roleid);	
		if(val == false) {
		...
		Mmap.put("msg", msg);
		return new ModelAndView("type_of_stock_a_Tiles","type_of_stock_a_CMD", new TB_TMS_MVCR_PARTA_DTL());
	}
	
	
	@RequestMapping(value = "/type_of_stock_a_Action", method = RequestMethod.POST)
	public ModelAndView addItemEntryForm(@ModelAttribute("type_of_stock_a_CMD") TB_TMS_MVCR_PARTA_DTL rs,
			HttpServletRequest request,ModelMap model,HttpSession session) {
		model.put("msg", "Your Data Was Saved Successfully");
		return new ModelAndView("redirect:type_of_stock_a");	
	}
	
	
	@RequestMapping(value = "/admin/getTypeofstockA", method = RequestMethod.POST)
	public ModelAndView getTypeofstockA(ModelMap Mmap, HttpSession sessionA,
			@RequestParam(value = "msg", required = false) String msg,
			@RequestParam(value = "sus_no1", required = false) String sus_no,
			@RequestParam(value = "mct1", required = false) String mct,
			@RequestParam(value = "type_of_stock_a1", required = false) String type_of_stock,
		...
		if (!sus_no.equals("") & sus_no.length() == 8 & !depot_name.equals("") & !type_of_stock.equals("")
			...
			ArrayList<List<String>> list = tosA.getSearchStockA(sus_no, mct, type_of_stock, roleType);
			...
			if (list.size() == 0) {
				...
				for (int i = 0; i < list.size(); i++) {
					String type_of_stock_a1 = list.get(i).get(3);
					if (type_of_stock_a1 == null || type_of_stock_a1.equals("") || type_of_stock_a1.equals(null)) {
				...
				Mmap.put("type_of_stock_a1", type_of_stock12);
			}
		} else {
		...
		Mmap.put("type_of_stock", type_of_stock);
		Mmap.put("std_nomclature", std_nomclature);
		return new ModelAndView("type_of_stock_a_Tiles");
	}
	
	
	@RequestMapping(value = "/UpdatestockA", method = RequestMethod.POST)
	public @ResponseBody String UpdatestockA(String type_of_stock,int id,HttpSession session1 ) {
		...
		String hql = "UPDATE TB_TMS_CENSUS_DRR_DIR_DTL set type_of_stock=:type_of_stock where id=:id";
	...
	@RequestMapping(value = "/getDepotSUSNoList_A", method = RequestMethod.POST)
	public @ResponseBody List<String> getDepotSUSNoList_A(String sus_no,HttpSession sessionA) {
		String roleAccess = sessionA.getAttribute("roleAccess").toString();
		String roleSusNo = sessionA.getAttribute("roleSusNo").toString();
		if(roleAccess.equals("Unit")  || roleAccess.equals("Depot")){
			sus_no = roleSusNo;
		...
		Session session = HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("select distinct sus_no from TB_TMS_CENSUS_DRR_DIR_DTL where issue_condition = '1' and upper(sus_no) like :sus_no ").setMaxResults(10);
	...
	@RequestMapping(value = "/getDepotUnitNameList_A", method = RequestMethod.POST)
	public @ResponseBody List<String> getDepotUnitNameList_A(String unit_name,HttpSession sessionA) {
		String roleAccess = sessionA.getAttribute("roleAccess").toString();
		String roleSusNo = sessionA.getAttribute("roleSusNo").toString();
	...
	@RequestMapping(value = "/getMCT_NomenList_A", method = RequestMethod.POST)
	public @ResponseBody List<String> getMCT_NomenList_A(String sus_no,HttpSession sessionA,String stock) {
		String roleAccess = sessionA.getAttribute("roleAccess").toString();
		String roleSusNo = sessionA.getAttribute("roleSusNo").toString();
		...
		Session session = HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();		
		Query q = session.createQuery("select distinct m.std_nomclature from TB_TMS_BANUM_DIRCTRY b,TB_TMS_CENSUS_DRR_DIR_DTL d,TB_TMS_MCT_MASTER m "
				+ "where m.status='ACTIVE' and b.ba_no=d.ba_no and d.sus_no=:sus_no and "
				+ "m.mct=b.mct and d.type_of_stock=:stock "
				+ " and d.ba_no not in (select ba_no from TB_TMS_CENSUS_DRR_DIR_DTL where sus_no=:d_sus_no and  type_of_stock in ('3','4','5')) "
	...
	@RequestMapping(value = "/getNomclature_A", method = RequestMethod.POST)
	public @ResponseBody List<String> getNomclature_A(String nomen,HttpSession sessionUserId) {
		nomen = nomen.replace("&#40;","(");
		nomen = nomen.replace("&#41;",")");
