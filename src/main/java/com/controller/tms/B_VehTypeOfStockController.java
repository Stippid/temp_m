...
import com.dao.tms.Type_of_stockDAO;
import com.models.TB_TMS_BANUM_DIRCTRY;
import com.models.TB_TMS_MVCR_PARTA_DTL;
import com.persistance.util.HibernateUtil;
@Controller
@RequestMapping(value = {"admin","/" ,"user"}) 
public class B_VehTypeOfStockController {
	
	@Autowired
	private Type_of_stockDAO sba1;
	@Autowired
	...
	@RequestMapping(value = "/type_of_stock", method = RequestMethod.GET)
	public ModelAndView type_of_stock(ModelMap Mmap,HttpSession session,@RequestParam(value = "msg", required = false) String msg,HttpServletRequest request) {
		...
		Boolean val = roledao.ScreenRedirect("type_of_stock", roleid);	
		if(val == false) {
		...
		Mmap.put("msg", msg);
		return new ModelAndView("type_of_stockTiles","type_of_stockCMD", new TB_TMS_MVCR_PARTA_DTL());
	}
	
	@RequestMapping(value = "/type_of_stockAction", method = RequestMethod.POST)
	public ModelAndView addItemEntryForm(@ModelAttribute("type_of_stockCMD") TB_TMS_MVCR_PARTA_DTL rs,
			HttpServletRequest request,ModelMap model,HttpSession session) {
		model.put("msg", "Your Data Saved Successfully");
		return new ModelAndView("redirect:type_of_stock");	
	}
	@Autowired
	Type_of_stockDAO sba;
	
	@RequestMapping(value = "/admin/getTypeofstock", method = RequestMethod.POST)
	public ModelAndView getTypeofstock(ModelMap Mmap, HttpSession sessionA,
			@RequestParam(value = "msg", required = false) String msg,
			@RequestParam(value = "sus_no1", required = false) String sus_no,
			@RequestParam(value = "mct1", required = false) String mct,
			@RequestParam(value = "type_of_stock1", required = false) String type_of_stock,
		...
		if (!sus_no.equals("") & sus_no.length() == 8 & !depot_name.equals("") & !type_of_stock.equals("")
			...
			ArrayList<List<String>> list = sba.getSearchStock(sus_no, mct, type_of_stock, roleType);
			...
			if (list.size() == 0) {
				...
				for (int i = 0; i < list.size(); i++) {
					String type_of_stock1 = list.get(i).get(3);
					if (type_of_stock1 == null || type_of_stock1.equals("") || type_of_stock1.equals(null)) {
				...
				Mmap.put("type_of_stock1", type_of_stock12);
			}
		} else {
		...
		Mmap.put("type_of_stock", type_of_stock);
		Mmap.put("std_nomclature", std_nomclature);
		return new ModelAndView("type_of_stockTiles");
	}
	
	@RequestMapping(value = "/Updatestock", method = RequestMethod.POST)
	public @ResponseBody String Updatestock(String type_of_stock,int id,HttpSession session1 ) {
		...
		String hql = "UPDATE TB_TMS_DRR_DIR_DTL set type_of_stock=:type_of_stock where id=:id";
	...
	@RequestMapping(value = "/getDepotSUSNoList", method = RequestMethod.POST)
	public @ResponseBody List<String> getDepotSUSNoList(String sus_no,HttpSession sessionA) {
		String roleAccess = sessionA.getAttribute("roleAccess").toString();
		String roleSusNo = sessionA.getAttribute("roleSusNo").toString();
		if(roleAccess.equals("Unit")  || roleAccess.equals("Depot")){
			sus_no = roleSusNo;
		...
		Session session = HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("select distinct sus_no from TB_TMS_DRR_DIR_DTL where typ_of_retrn='0' and type_of_issue='1' and upper(sus_no) like :sus_no ").setMaxResults(10);
	...
	@RequestMapping(value = "/getDepotUnitNameList", method = RequestMethod.POST)
	public @ResponseBody List<String> getDepotUnitNameList(String unit_name,HttpSession sessionA) {
		String roleAccess = sessionA.getAttribute("roleAccess").toString();
		String roleSusNo = sessionA.getAttribute("roleSusNo").toString();
		...
		@RequestMapping(value = "/getMCT_NomenList", method = RequestMethod.POST)
		public @ResponseBody List<String> getMCT_NomenList(String sus_no,HttpSession sessionA,String stock) {
			String roleAccess = sessionA.getAttribute("roleAccess").toString();
			String roleSusNo = sessionA.getAttribute("roleSusNo").toString();
			...
			Session session = HibernateUtil.getSessionFactory().openSession();
			Transaction tx = session.beginTransaction();		
			Query q = session.createQuery("select distinct m.std_nomclature from TB_TMS_BANUM_DIRCTRY b,TB_TMS_DRR_DIR_DTL d,TB_TMS_MCT_MASTER m "
					+ "where m.status='ACTIVE' and b.ba_no=d.ba_no and d.sus_no=:sus_no and "
					+ "m.mct=b.mct and d.type_of_stock=:stock "
					+ " and d.ba_no not in (select ba_no from TB_TMS_DRR_DIR_DTL where sus_no=:d_sus_no and  type_of_stock in ('3','4','5')) "
		...
		@RequestMapping(value = "/getNomclature", method = RequestMethod.POST)
		public @ResponseBody List<String> getNomclature(String nomen,HttpSession sessionUserId) {
			nomen = nomen.replace("&#40;","(");
			nomen = nomen.replace("&#41;",")");
