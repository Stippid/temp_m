...
public class Emoluments_DAOimpl implements Emoluments_DAO {
	...
	public ArrayList<ArrayList<String>> getsearch_Emolument(String personnel_no1,String cname1,String rank1, String from_date1,
			String ini_status1,String on_status1,String scase1,String cancel_status1 , String cr_by,String cr_date,String roleType){
...
//		 }
	...
	if(!cr_date.equals("")) {
				qry +=" and cast(e.created_dt as date) = cast(? as date)";
			}
...
//		
//			
			q= " select distinct  p.id, p.personnel_no,p.name,r.description as rank,orb.unit_name as unit_name,e.status,p.rank,e.update_status " + 
				" from tb_psg_trans_proposed_comm_letter p\r\n" + 
				" inner join tb_psg_emolument_parent_offcr e on p.id= e.comm_id\r\n" + 
				...
				"left join logininformation l1 on l1.username =e.created_by\r\n" + 
				" inner join tb_psg_census_battle_physical_casuality bc   on bc.comm_id = e.comm_id " +qry +  "   order by p.id desc"; 
	...
	public TB_PSG_EMOLUMENT_PARENT_OFFCR getEmolumentsByid(int id) {
		...
		TB_PSG_EMOLUMENT_PARENT_OFFCR updateid = (TB_PSG_EMOLUMENT_PARENT_OFFCR) sessionHQL.get(TB_PSG_EMOLUMENT_PARENT_OFFCR.class, id);
	...
	public ArrayList<ArrayList<String>> Get_EmoulData(BigInteger comm_id) {
		ArrayList<ArrayList<String>> alist = new ArrayList<ArrayList<String>>();
		Connection conn = null;
		String q="";
		...
		try{	  
			...
			q="select distinct p.id,p.personnel_no,p.name,r.description as rank ,orb.unit_name as unit_name ,TO_CHAR(bc.date_of_casuality,'dd-Mon-yyyy') as dt_of_casulity,e.id as eid" + 
					"	from tb_psg_trans_proposed_comm_letter p \r\n" + 
					"	inner join tb_psg_emolument_parent_offcr e on p.id::text= e.comm_id::text \r\n" + 
					"	inner join cue_tb_psg_rank_app_master r on r.id = p.rank  and r.status_active='Active'  \r\n" + 
					"	inner join tb_miso_orbat_unt_dtl orb on orb.sus_no = p.unit_sus_no and status_sus_no='Active'"
					+ "inner join tb_psg_census_battle_physical_casuality bc   on bc.comm_id::text = p.id::text\r\n" + 
					"inner join  (select max(id) as id ,comm_id::text from tb_psg_census_battle_physical_casuality \r\n" + 
	...
	public List<Map<String, Object>>   getPersonnelNoDataEmoulCont(String personnel_no) {
		...
		try {
			...
			q = "select c.id,c.cadet_no,c.batch_no,g.gender_name,\r\n" + 
					"c.date_of_birth ,c.name,cu.description,ct.course_name,\r\n" + 
					"c.parent_arm,c.regiment, cu.code as rankcode,c.date_of_commission ,u.unit_name,bc.date_of_casuality \r\n" + 
					"from tb_psg_trans_proposed_comm_letter c inner join tb_psg_mstr_gender g on c.gender=g.id \r\n" + 
					"inner join tb_miso_orbat_unt_dtl u  on c.unit_sus_no=u.sus_no \r\n" + 
					"inner join tb_psg_mstr_course_comm ct on ct.id=c.course \r\n" + 
					"inner join cue_tb_psg_rank_app_master cu  on cu.id=c.rank \r\n" + 
					"inner join tb_psg_census_battle_physical_casuality bc   on bc.comm_id = c.id\r\n" + 
					"inner join  (select max(id) as id ,comm_id from tb_psg_census_battle_physical_casuality where status=1 group by comm_id) ng1 on bc.id=ng1.id\r\n" + 
					"and bc.status='1'  where  c.personnel_no = ? order by c.personnel_no  ";
			stmt = conn.prepareStatement(q);
	...
	public ArrayList<ArrayList<String>> approve_update_eno_update(BigInteger comm_id)
	{
		...
		try{	  
			...
			if(comm_id != BigInteger.ZERO) {
				qry += " and cast(upper(p.comm_id) as character varying) = ? ";
			}
			
			
			
			q="update TB_PSG_EMOLUMENT_PARENT_OFFCR OFR set amount_rel =t1.amt, amount_rel_v = to_char(t1.amt, 'FM9,999,999,999,999,999') FROM\r\n" + 
									"(select sum(amount_release::float) as amt,p_id \r\n" + 
	                		" from TB_PSG_EMOLUMENT_CHILD_OFFCR where   cast(comm_id as character varying)=?  group by p_id) t1  where OFR.id=t1.p_id";
...
//	 	        }
	...
	public ArrayList<String> approve_update_eno_iteration(BigInteger comm_id)
	{
		...
		try{	  
			...
			q="select itteration_status from TB_PSG_EMOLUMENT_PARENT_OFFCR where  cast(upper(comm_id::text) as character varying) = ?  ";
	...
	public ArrayList<ArrayList<String>> App_getm_emu_detailsData_list(BigInteger comm_id,String c_status,String i_status) {
		...
		try {
			...
			sql = "select distinct bc.comm_id,bc.id,oem.agency_id,oem.type_of_benefits_id,oem.amount_due_v,oem.amount_rel_v,bc.p_id,"+
					...
					"from tb_psg_emolument_child_offcr bc\r\n" + 
					"			inner join tb_psg_emolument_parent_offcr oem on oem.id = bc.p_id  \r\n" + 
					...
					"			where cast(bc.comm_id as character varying)=?   "+qry;
...
//				list.add(add_more); // 4
	...
	public ArrayList<ArrayList<String>> App_getm_emu_detailsData_app_list(BigInteger comm_id) {
		...
		try {
			...
			sql = "select distinct bc.comm_id,bc.id,oem.agency_id,oem.type_of_benefits_id,oem.amount_due_v,oem.amount_rel_v,"+
					...
					"			inner join tb_psg_agency_mstr ae on ae.id = oem.agency_id\r\n" + 
					"			inner join tb_psg_benefits_mstr be on be.id = oem.type_of_benefits_id \r\n" + 
					"			where cast(bc.comm_id as character varying)=?  and bc.status='1' ";
...
//				list.add(add_more); // 4
	...
	public ArrayList<ArrayList<String>> View_Emoluments_History(BigInteger comm_id) {
		...
		try{	  
			...
			q="select pc.personnel_no,ag.agency_name,bn.benefits_name,e_p.amount_due, e_c.amount_release,\r\n" + 
					"e_c.reason      ,ltrim(TO_CHAR( e_c.updated_as_on ,'DD/MM/YYYY'),'0') as  updated_as_on ,pc.name,r.description as rank,e_c.amount_rem\r\n" + 
					...
					+ " where cast(e_p.comm_id as character varying) = ?   and e_c.status in (1,2) ";
				stmt=conn.prepareStatement(q);
				stmt.setString(1,String.valueOf(comm_id));
	...
	public ArrayList<ArrayList<String>> getm_emu_detailsDataListEdit(BigInteger comm_id) {
		...
		try {
			...
			sql = "select e.id as e_id ,c.id as c_id ,e.comm_id,e.amount_rel,e.amount_rel_v,e.amount_due,e.amount_due_v,e.reason ,"
					...
					" inner join tb_psg_emolument_child_offcr c on e.id= c.p_id\r\n" + 
 					" where cast(e.comm_id as character varying) =?   ";
...
//				list.add(add_more); // 4
				...
				list.add(rs.getString("comm_id"));// 2
