...
public class emolumentJcoDAOImpl implements emolumentJcoDAO{
...
private DataSource dataSource;
	...
	public Boolean getPersonnelNoAlreadyExistemJCOemolu(String personnel_no) {
		...
		try {
			...
			sql = "select case when count(cv.*) > 0 then  false else true  end as result from \r\n" + 
					"\r\n" + 
					"tb_psg_emolument_parent_offcr_jco cv  inner join \r\n" + 
					"tb_psg_census_jco_or_p pc on cv.jco_id=pc.id\r\n" + 
					"where pc.army_no = ? ";
