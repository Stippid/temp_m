...
public class WeaponAuthUnderModiDAOImpl implements WeaponAuthUnderModiDAO {
    ...
    ///////////////////////////////////////////////////////REOPRT//////////////////////////////////////////	
	 public String setApprovedWeaponModi(int appid, String username){
			...
			String hqlUpdate = "update CUE_TB_MISO_WEPE_WEAPONS_MDFS c set c.status = :status, aprv_rejc_by=:aprv_rejc_by, date_of_apprv_rejc=:date where c.id = :id";
	 ...
	 public String setDeleteWeaponModi(int appid){
			Session session = HibernateUtilNA.getSessionFactory().openSession();
			Transaction tx = session.beginTransaction();
			String hql = "delete from CUE_TB_MISO_WEPE_WEAPONS_MDFS where id = :id";
		...
		public CUE_TB_MISO_WEPE_WEAPONS_MDFS getCUE_TB_MISO_WEPE_WEAPONS_MDFSByid(int id) {
			...
			Query q = session.createQuery("from CUE_TB_MISO_WEPE_WEAPONS_MDFS where id=:id");
			q.setParameter("id", id);
			CUE_TB_MISO_WEPE_WEAPONS_MDFS list = (CUE_TB_MISO_WEPE_WEAPONS_MDFS) q.list().get(0);
		...
		public String UpdateWepeMasterValue(CUE_TB_MISO_WEPE_WEAPONS_MDFS WepeMasterValue){
		 	Session session = HibernateUtilNA.getSessionFactory().openSession();
			Transaction tx = session.beginTransaction();
			String hql = "update CUE_TB_MISO_WEPE_WEAPONS_MDFS set scenario=:scenario,modification=:modification,location=:location,formation=:formation,amt_inc_dec=:amt_inc_dec,remarks=:remarks,item_seq_no=:item_seq_no, status ='0' where id=:id";
			Query query = session.createQuery(hql).setString("location", WepeMasterValue.getLocation()).setString("formation", WepeMasterValue.getFormation()).setString("scenario", WepeMasterValue.getScenario()).setString("modification", WepeMasterValue.getModification()).setDouble("amt_inc_dec", WepeMasterValue.getAmt_inc_dec()).setString("remarks", WepeMasterValue.getRemarks()).setString("item_seq_no", WepeMasterValue.getItem_seq_no()).setInteger("id",WepeMasterValue.getId());
			...
			{				
				  ...
				  Connection conn = null;
				   ...
				   try{	  
					...
					q="select  a.we_pe_no,a.item_seq_no,m.item_type,n.auth_weapon as baseauth,a.amt_inc_dec,a.modification,a.scenario,concat(b.unit_name,c.unit_name,l.code_value)  as loc_for_unit ,a.reject_letter_no,a.reject_remarks,a.id,a.status,a.remarks, "
							...
							+ "from cue_tb_miso_wepe_weapons_mdfs a " + 
			...
			{				
				...
				String qry="";
				   try{	  
					...
					q="select  a.we_pe_no,a.item_seq_no,m.item_type,n.auth_weapon as baseauth,a.amt_inc_dec,a.modification,a.scenario,concat(b.unit_name,c.unit_name,l.code_value)  as loc_for_unit ,a.reject_letter_no,a.reject_remarks,a.id,a.status,a.remarks from cue_tb_miso_wepe_weapons_mdfs a " + 
							" left outer join tb_miso_orbat_code l on a.location=l.code and l.status_record='1' left outer join tb_miso_orbat_unt_dtl b on a.formation=b.sus_no and b.status_sus_no='Active' left outer join tb_miso_orbat_unt_dtl c on a.scenario_unit=c.sus_no and c.status_sus_no='Active'" + 
							" inner join cue_tb_miso_mms_item_mstr m on a.item_seq_no=m.item_code left outer join cue_tb_miso_wepe_weapon_det n on a.we_pe_no=n.we_pe_no and a.item_seq_no=n.item_seq_no" + 
							"  "+qry+" order by a.status,a.item_seq_no ";					
