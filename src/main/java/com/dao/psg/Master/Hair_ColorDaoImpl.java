...
public class Hair_ColorDaoImpl implements Hair_ColorDao {
	...
	public ArrayList<ArrayList<String>> search_hair_colour(String hair_cl_name, String status) {
		...
		try {
			...
			if (!hair_cl_name.equals("")) {
				qry += " and upper(hair_cl_name) like ?";
			}
			if (!status.equals("0")) {
			...
			q = "select id,upper(hair_cl_name) as hair_cl_name,status from tb_psg_mstr_hair_colour where id !=0 " + qry + " order by id desc";

			stmt = conn.prepareStatement(q);
			if (!qry.equals("")) {
				int j = 1;
				if (!hair_cl_name.equals("")) {
					stmt.setString(j, hair_cl_name.toUpperCase() + "%");
					j += 1;
				}
				if (!status.equals("0")) {
			...
			while (rs.next()) {
				...
				list.add(rs.getString("hair_cl_name"));

				String f = "";
				String f1 = "";
				String Update = "onclick=\"  if (confirm('Are You Sure You Want to Update This Colour Name ?') ){editData("
						+ rs.getInt("id") + ",'" + rs.getString("hair_cl_name") + "')}else{ return false;}\"";
	...
	public TB_HAIR_COLOUR getHairColourByid(int id) {
		...
		try {
			TB_HAIR_COLOUR updateid = (TB_HAIR_COLOUR) sessionHQL.get(TB_HAIR_COLOUR.class, id);
			tx.commit();
			return updateid;
	...
	public ArrayList<ArrayList<String>> search_hair_colourReport() {
		...
		try {
			...
			q = "select id,hair_cl_name from tb_psg_mstr_hair_colour where id !=0  order by id desc";
