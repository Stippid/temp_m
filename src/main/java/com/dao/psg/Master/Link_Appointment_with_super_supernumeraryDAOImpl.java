...
public class Link_Appointment_with_super_supernumeraryDAOImpl implements Link_Appointment_with_super_supernumeraryDAO{
	...
	try{	  
		...
		q="select id, appointment,status from tb_psg_link_appoinment_with_super_supernumerary where id!=0 " + qry;
			...
			while (rs.next()) {
				...
				String f = "";
				String f1 = "";
				String Update = "onclick=\"  if (confirm('Are You Sure You Want to Update This Appointment?') ){editData("+ rs.getInt("id") + ",'" + rs.getString("appointment")  + "','" + rs.getString("status")  + "')}else{ return false;}\"";
