	...
	public List<Map<String, Object>> search_Operation(String operation_name,String status) {
		...
		try {
			...
			if (!operation_name.equals("")) {
				qry += " and upper(operation_name) like ?";
			}
			if (!status.equals("0") ) {
				qry += " and upper(status) like ?";
			}
			 
		q = "SELECT id,operation_name,status FROM  tb_psg_mstr_operation where id!=0"+ qry;
			stmt = conn.prepareStatement(q);
			int j = 1;
			if (!operation_name.equals("")) {
				stmt.setString(j, operation_name.toUpperCase()+"%");
				j++;
			}
			if (!status.equals("0") ) {
			...
			while (rs.next()) {
				...
				if(status.equals("inactive"))
				{
					...
					String editData = "onclick=\"  if (confirm('Are you sure you want to update This Opretion Details?') ){editData("
							+ rs.getObject(1) + ")}else{ return false;}\"";
					String updateButton = "<i class='action_icons action_update' " + editData + " title='Edit Data'></i>";
					String Delete1 = "onclick=\"  if (confirm('Are you sure you want to delete Opretion Details?') ){deleteData("
							+ rs.getObject(1) + ")}else{ return false;}\"";
...
public TB_OPERATION getOperationByid(int id) {
		...
		Transaction tx = sessionHQL.beginTransaction();
	 	TB_OPERATION updateid = (TB_OPERATION) sessionHQL.get(TB_OPERATION.class, id);
...
public Long checkExitstingOperation(String operation_name1,int id1,String status1) {
	String hql="select count(id) from TB_OPERATION where operation_name1=:op1,status1=:s1 and  id!=:id and status='active'";
	Session session = HibernateUtil.getSessionFactory().openSession();
	Transaction tx = session.beginTransaction();
	Query q2= session.createQuery(hql);
	q2.setParameter("op1", operation_name1);
...
public String Update_Operation(TB_OPERATION obj,String username){
  ...
  String msg = "";
  try{
  	String hql = "update TB_OPERATION set operation_name=:operation_name,modify_by=:m1,modify_date=:m2 where id=:id and status='active'";

		Query query = sessionHQL.createQuery(hql).setString("operation_name", obj.getOperation_name())
...
public ArrayList<ArrayList<String>> search_OperaionReport()
{
	...
	try{	  
		...
		q="SELECT id,operation_name FROM  tb_psg_mstr_operation where id!=0 " ;
			stmt=conn.prepareStatement(q);
			int i = 1;
			ResultSet rs = stmt.executeQuery();      
			while (rs.next()) {
				...
				list.add(id);
				list.add(rs.getString("operation_name"));
