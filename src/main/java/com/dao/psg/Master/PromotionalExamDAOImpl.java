...
public class PromotionalExamDAOImpl implements PromotionalExamDAO{
	...
	public ArrayList<ArrayList<String>> search_PromoExam(String promoExam,String status)
	{
		...
		try{	  
			...
			q="select distinct id,promo_exam,status from tb_psg_mstr_promotional_exam c \r\n" + 
	...
	public TB_MSTR_PROMOTIONAL_EXAM getmtByid(int id) { Session sessionHQL =
			  HibernateUtil.getSessionFactory().openSession(); Transaction tx =
			  sessionHQL.beginTransaction(); TB_MSTR_PROMOTIONAL_EXAM updateid = (TB_MSTR_PROMOTIONAL_EXAM)
			  sessionHQL.get(TB_MSTR_PROMOTIONAL_EXAM.class, id); sessionHQL.getTransaction().commit();
