...
public class Change_TA_Status_History_DAOImpl implements Change_TA_Status_History_DAO{
	...
	public ArrayList<ArrayList<String>> change_ta(BigInteger comm_id, int census_id) {
		...
		try{	  
			...
			q="SELECT distinct id, census_id, comm_id, ta_status_authority, ltrim(TO_CHAR(ta_authority_date ,'DD-MON-YYYY'),'0') as date_of_authority ,"
					+ " ta_status, ltrim(TO_CHAR(date_of_ta_status ,'DD-MON-YYYY'),'0') as change_in_ta_date,status, created_by, "
					...
					"FROM tb_psg_change_ta_status_new WHERE status in (1,2) AND cast(comm_id as character varying)=?  order by id ";
				...
				while (rs.next()) {
					...
					list.add(rs.getString("change_in_ta_date"));
