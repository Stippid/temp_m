...
public class Update_Battle_And_Physical_Casuality_History_DAOImpl
		implements Update_Battle_And_Physical_Casuality_History_DAO {
	...
	public ArrayList<ArrayList<String>> update_battle_and_physical_casuality(BigInteger comm_id, int census_id) {
		...
		try{	  
			...
			q="select  pc.id,\r\n" + 
					...
					"where  pc.status in (1,2)  AND  cast(pc.comm_id as character varying) = ?  AND pc.census_id  = ? ORDER BY pc.id  \r\n" + 
					"\r\n" + 
					"" ;
