...
public class AVehDailyReceiptIssuereportDAOImpl implements AVehDailyReceiptIssuereportDAO{
	...
	public Boolean ifExistSusAndSer(String sus_no, String drr_dir_ser_no) {
		String q = "from TB_TMS_CENSUS_DRR_DIR_MAIN where sus_no =:sus_no and drr_dir_ser_no=:drr_dir_ser_no";
   	    Session session = HibernateUtil.getSessionFactory().openSession();
   	    Transaction tx = session.beginTransaction();
    	Query query = session.createQuery(q);
   		query.setParameter("sus_no", sus_no).setParameter("drr_dir_ser_no", drr_dir_ser_no);
   	    List<TB_TMS_CENSUS_DRR_DIR_MAIN> list = (List<TB_TMS_CENSUS_DRR_DIR_MAIN>) query.list();
   		tx.commit();
   		session.close();
   	    if(list.size() > 0) {
   	    	return true;
	...
	public Boolean ifExistSusAndSerDtNIssueType(String a, String ba_no,String issue_type) {
		String q = "from TB_TMS_CENSUS_DRR_DIR_DTL where issue_condition =:a and ba_no=:ba_no and issue_type = :issue_type ";
   	    Session session = HibernateUtil.getSessionFactory().openSession();
   	    Transaction tx = session.beginTransaction();
   	    Query query = session.createQuery(q);
	   	query.setParameter("a", a).setParameter("ba_no", ba_no);
	   	query.setParameter("issue_type", issue_type);
	   	List<TB_TMS_CENSUS_DRR_DIR_DTL> list = (List<TB_TMS_CENSUS_DRR_DIR_DTL>) query.list();
	   	tx.commit();
	   	session.close();
   	    if(list.size() > 0) {
   	    	return true;
	...
	public Boolean ifExistSusAndSerDtl(String a, String ba_no) {
		String q = "from TB_TMS_CENSUS_DRR_DIR_DTL where issue_condition =:a and ba_no=:ba_no ";
   	    Session session = HibernateUtil.getSessionFactory().openSession();
   	    Transaction tx = session.beginTransaction();
   	    Query query = session.createQuery(q);
	   	query.setParameter("a", a).setParameter("ba_no", ba_no);
	...
	public Boolean ifExistSusAndSerAndBANo(String sus_no, String drr_dir_ser_no, String ba_no ,String issue_condition) {
		String q = "from TB_TMS_CENSUS_DRR_DIR_DTL where sus_no =:sus_no and drr_dir_ser_no=:drr_dir_ser_no and ba_no=:ba_no and issue_condition = :issue_condition";
   	    Session session = HibernateUtil.getSessionFactory().openSession();
   	    Transaction tx = session.beginTransaction();
    	Query query = session.createQuery(q);
   		query.setParameter("sus_no", sus_no).setParameter("drr_dir_ser_no", drr_dir_ser_no).setParameter("ba_no", ba_no).setParameter("issue_condition", issue_condition);
   	    List<TB_TMS_CENSUS_DRR_DIR_DTL> list = (List<TB_TMS_CENSUS_DRR_DIR_DTL>) query.list();
   		tx.commit();
   		session.close();
   	    if(list.size() > 0) {
	...
	public Boolean ifExistSerAndSusDrr(String sus_no, String drr_dir_ser_no, Date creation_date) {
		String q = "from TB_TMS_CENSUS_DRR_DIR_DTL where sus_no =:sus_no and drr_dir_ser_no=:drr_dir_ser_no";
		Session session = HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
	...
	public List<String> getACensusVehSusBaNoDtlListCase2AgaiIssue(String sus_no,String ba_no)
  	{
		...
		try{	 
			...
			sql = " select distinct vba  from ( select 	d.id m,d.ba_no vba, 	c.ba_no nvba, 	d.issue_condition, 	d.sus_no n from 	tb_tms_census_drr_dir_dtl d 	\r\n" + 
					"left join tb_tms_census_retrn c on d.ba_no = c.ba_no  	\r\n" + 
					"where  upper(d.ba_no) like ? and d.status='1' and d.issue_condition ='3'  \r\n" + 
					"and d.ba_no not in(select ba_no from tb_tms_census_drr_dir_dtl where  issue_condition='4' \r\n" + 
					"and (status='0' or status = '1'))    and d.ba_no not in(select ba_no from tb_tms_census_drr_dir_dtl where   issue_condition='2' and status='0' ) \r\n" + 
					"\r\n" + 
					"and d.ba_no in (select ba_no from tb_tms_census_drr_dir_dtl where issue_condition ='3' and status = '1'  )\r\n" + 
					")a  where m in ( select max(m) from ( select 	d.id m,d.ba_no vba, 	c.ba_no nvba, 	d.issue_condition, 	d.sus_no n from 	tb_tms_census_drr_dir_dtl d 	\r\n" + 
					"left join tb_tms_census_retrn c on d.ba_no = c.ba_no  	\r\n" + 
					"where  upper(d.ba_no) like ? and d.status='1' and d.issue_condition ='3'  \r\n" + 
					"and d.ba_no not in(select ba_no from tb_tms_census_drr_dir_dtl where  issue_condition='4' \r\n" + 
					"and (status='0' or status = '1'))    and d.ba_no not in(select ba_no from tb_tms_census_drr_dir_dtl where   issue_condition='2' and status='0' ) \r\n" + 
					"\r\n" + 
					"and d.ba_no in (select ba_no from tb_tms_census_drr_dir_dtl where issue_condition ='3' and status = '1'  )\r\n" + 
					")j group by vba) and n =?  and a.nvba is null";
  	  	 	
			  stmt=conn.prepareStatement(sql);
			  stmt.setString(1,ba_no.toUpperCase() + "%");
			  stmt.setString(2,ba_no.toUpperCase() + "%");
		      stmt.setString(3,sus_no);
			ResultSet asm = stmt.executeQuery();
			
  	  	 	while(asm.next()){
				String ba_no1 = asm.getString("vba");
	    	    list.add(ba_no1);  	              
			}	   
			asm.close();
			stmt.close();
    	...
    	Connection conn = null;
			try{	  
				 conn = dataSource.getConnection();
				    ...
				    if(status.equals("3")) {
					   sql = "select distinct d.sus_no, d.drr_dir_ser_no, d.issue_condition,ltrim(TO_CHAR(d.creation_date ,'dd-mm-yyyy'),'0') as creation_date,d.status,(select unit_name from tb_miso_orbat_unt_dtl where sus_no = d.sus_no and status_sus_no = 'Active') from TB_TMS_CENSUS_DRR_DIR_DTL d";
				    }else {
						sql = "select distinct d.sus_no, d.drr_dir_ser_no,d.issue_condition,ltrim(TO_CHAR(d.creation_date ,'dd-mm-yyyy'),'0') as creation_date,d.status,(select unit_name from tb_miso_orbat_unt_dtl where sus_no = d.sus_no and status_sus_no = 'Active') from TB_TMS_CENSUS_DRR_DIR_DTL d where "+ qry1 + " order by creation_date desc";
					}
			      ...
			      while(rs.next()){
					...
					String View = "onclick=\"  View('"+rs.getString("drr_dir_ser_no")+"','"+rs.getString("status")+"','"+from_date+"','"+status+"','"+rs.getString("sus_no")+"','"+from_date+"','"+curr_date+"','"+rs.getString("unit_name")+"')\"";
	...
	public  ArrayList<List<String>> getApprovedBA_NoFromDRRAVehicle(String viewSerNo,String viewStatus,String viewDate,String viewSus,String roleType)
  	{
    	...
    	Connection conn = null;
			try{	  
			   conn = dataSource.getConnection();
			   String sql= ""; 
			   sql="select distinct a.id,a.ba_no,a.sus_no,a.unit_sus_no,a.drr_dir_ser_no,a.issue_condition,a.status  ,b.unit_name\r\n" + 
			  		"	from TB_TMS_CENSUS_DRR_DIR_DTL a\r\n" + 
			   ...
			   while(rs.next()){
				 ...
				 String Delete = "onclick=\"  if (confirm('Are You Sure you want to Delete this Record?') ){Delete1('"+ rs.getString("id") +"','"+rs.getString("drr_dir_ser_no")+"','"+ rs.getString("issue_condition") +"','"+ rs.getString("ba_no") +"','" + rs.getString("sus_no") +"')}else{ return false;}\"";
	 ...
	 public List<Map<String, String>> get_A_vech_daily(String sus_no_aprove, String ser_no_approve) {
	        ...
	        try {
	            conn = dataSource.getConnection();
	            String sql = "select a.unit_sus_no,c.std_nomclature,CAST(count(*) AS INT) as counts from tb_tms_census_drr_dir_dtl a "
