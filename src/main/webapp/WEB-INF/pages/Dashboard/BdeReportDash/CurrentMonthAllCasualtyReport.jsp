		...
		<div class="col-md-12">
			<div class="card">
				<div class="card-header" align="center">
					<strong style="text-decoration: underline;">Current Month Report</strong>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="watermarked" data-watermark="" id="divwatermark"
		...
		<div class="datatablediv" id="reportDiv">
			<div class="">
				<div class="watermarked" data-watermark="" id="divwatermark">
					<span id="ip"></span>
					<table id="AllCasualtyCurrentMonthReport"
						class="table no-margin table-striped table-hover table-bordered text-center">
						<thead style="font-size: 15px;">
							<tr>
								<th></th>
								<th></th>
								<th colspan="2" style="text-align: center;">PERS</th>
								<th colspan="3" style="text-align: center;">TRANSPORT</th>
								<th style="text-align: center;">EQPTS</th>
								<th colspan="2" style="text-align: center;">IT ASSETS (holding as on date)</th>
							</tr>
							<tr>
								<th style="text-align: center;">Ser No</th>
								<th style="text-align: center;">Unit Name</th>
								<th style="text-align: center;">3008</th>
								<th style="text-align: center;">3009</th>
								<th style="text-align: center;">A veh</th>
								<th style="text-align: center;">B veh</th>
								<th style="text-align: center;">C veh</th>
								<th style="text-align: center;">MCR</th>
								<th style="text-align: center;">Computing</th>
						...
						<tbody>
							<c:if test="${list.size() == 0}">
								<tr>
									<td colspan="10" style="color: red;">Data Not Available</td>
								</tr>
							</c:if>
						</tbody>
					</table>
...
<div class="col-md-12">
	<div align="center" class="print_btn">
		...
		<input type="button" class="btn btn-purple"
			style="padding: 5px 10px; font-size: 14px; font-weight: bold; color: white; background-color: purple;"
			value="Export to Excel"
			onclick="exportToExcel('AllCasualtyCurrentMonthReport', 'AllCasualtyCurrentMonthReport')">
	</div>
</div>

<c:url value="Download_unit_dashboard_Report" var="dwonloadUrl" />
<form:form action="${dwonloadUrl}" method="post" id="downloadForm"
	name="downloadForm" modelAttribute="type_of_report">
...
<script>
	...
	$(document).ready(function() {
		...
		colAdj("AllCasualtyCurrentMonthReport");

	});

	mockjax1('AllCasualtyCurrentMonthReport');
	table = dataTable('AllCasualtyCurrentMonthReport');
	...
	function data(tableName) {
		...
		if (tableName == "AllCasualtyCurrentMonthReport") {

			$.post("getAllCasualtyListCount?" + key + "=" + value, {
				Search : Search
			}, function(j) {

				FilteredRecords = j;
			});
			$.post("getAllCasualtyList?" + key + "=" + value, {
				...
				orderColunm : orderColunm
					...
					function(j) {
						...
						for (var i = 0; i < j.length; i++) {
							var sr = i + 1;
							jsondata.push([ sr, j[i].unit_name,
									j[i].status_3008, j[i].status_3009,
									j[i].aveh_status, j[i].bveh_status,
									j[i].cveh_status, j[i].mcr_status,
									j[i].computing, j[i].peripheral ]);
	...
	function exportToExcel(tableId, fileName) {
		...
		XLSX.utils.book_append_sheet(wb, ws, "AllCasualtyCurrentMonthReport");
		XLSX.writeFile(wb, fileName + ".xlsx");
	}

	function downloaddata() {
		document.getElementById('type_of_report').value = "AllCasualtyCurrentMonthReport";
