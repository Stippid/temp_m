		...
		<div class="col-md-12">
			<div class="card">
				<div class="card-header" align="center">
					<strong style="text-decoration: underline;"> Pers(civs)</strong>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="col-md-12">
		...
		<input type="button" class="btn btn-purple" style="padding: 5px 10px; font-size: 14px; 
			font-weight: bold; color: white; background-color: purple;" value="Export to Excel"
				onclick="exportToExcel('persCivs', 'Pers Held(Civs)')">
 ...
 <input type="hidden" name="type_of_report" id="type_of_report"  value="0">
	...
	<script>
		...
		function data(tableName) {
			...
			if (tableName == "persCivs") {

				$.post("getUnitReportPersCivscount?" + key + "=" + value, {
					Search : Search,
					sus_no : sus_no
				...
				$.post("getUnitReportPersCivs?" + key + "=" + value, {
					...
					for (var i = 0; i < j.length; i++) {
						...
						jsondata.push([ sr,j[i].unit_name, j[i].employee_no,j[i].designation,
								j[i].emp_name, j[i].classification_services,
								j[i].civ_type, j[i].dob,j[i].designation_date,
		...
		function printDiv(divName) {
			...
			row += '<div class="col-6 col-sm-6 col-md-6"><h3>Pers Held(Civs)</h3> </div> ';
		...
		function exportToExcel(tableId, fileName) {
			...
			XLSX.utils.book_append_sheet(wb, ws, "Pers Held(Civs)");
			XLSX.writeFile(wb, fileName + ".xlsx");
		}
		
		function downloaddata(){
            document.getElementById('type_of_report').value="civ_held";
