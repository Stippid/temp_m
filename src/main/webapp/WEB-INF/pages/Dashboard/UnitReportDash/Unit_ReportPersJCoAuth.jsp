...
<input type="button" class="btn btn-purple" style="padding: 5px 10px; font-size: 14px; 
			font-weight: bold; color: white; background-color: purple;" value="Export to Excel"
				onclick="exportToExcel('persJCoAuth', 'Pers Auth(Jcos)')">
...
<form:form action="${dwonloadUrl}" method="post" id="downloadForm" name="downloadForm" modelAttribute="type_of_report">
 <input type="hidden" name="type_of_report" id="type_of_report"  value="0">
	...
	<script>
		...
		mockjax1('persJCoAuth');
		table = dataTable('persJCoAuth');
		...
		function data(tableName) {
			...
			if (tableName == "persJCoAuth") {

				$.post("getUnitReportPersJcoAuthcount?" + key + "=" + value, {
					Search : Search,
					sus_no : sus_no
				...
				$.post("getUnitReportPersJcoAuth?" + key + "=" + value, {
		...
		function exportToExcel(tableId, fileName) {
			...
			XLSX.utils.book_append_sheet(wb, ws, "Pers Auth(Jcos)");
			XLSX.writeFile(wb, fileName + ".xlsx");
		}
		
		function downloaddata(){
			document.getElementById('type_of_report').value="jco_auth";
