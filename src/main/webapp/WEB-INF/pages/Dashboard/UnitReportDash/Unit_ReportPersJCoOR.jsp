...
<div class="animated fadeIn">
		...
		<div class="col-md-12">
			<div class="card">
				<div class="card-header" align="center">
					<strong style="text-decoration: underline;">Pers(JCO/OR)</strong>
				</div>
			</div>
		</div>
	</div>
				...
				<div class="row form-group">
					...
					<div class="col-md-8">
						<input type="text" id="army_no" name="army_no"
							class="form-control autocomplete" maxlength="11"
							autocomplete="off" onkeyup="return specialcharecter(this)"
							onkeypress="return AvoidSpace(event)">
					</div>
				</div>
			</div>
			<input type="hidden" id="type1" name="type1" value="JCO"
	...
	<div class="card">
		<div class="card-header" align="center">
			<input type="button" class="btn btn-info btn-sm" onclick="Search1();"
				value="Search"> <input type="button"
				class=" btn btn-success btn-sm" onclick="Clear();" value="Clear">
		</div>
	</div>
</div>
...
<div class="col-md-12">
	<div align="center" class="print_btn">
		...
		<input type="button" class="btn btn-purple"
			...
			onclick="exportToExcel('persJCoOR', 'Pers(JCO)')">
	</div>
</div>
...
<script>
	...
	function data(tableName) {
		...
		if (tableName == "persJCoOR") {
			var rank = $("#rank").val();
			var name = $("#name").val();
			var trade = $("#trade").val();
			var armyNo = $("#army_no").val();

			$.post("getUnitReportPersJCocount?" + key + "=" + value, {
				Search : Search,
				rank : rank,
				name : name,
				trade : trade,
			...
			$.post("getUnitReportPersJCo?" + key + "=" + value, {
				...
				for (var i = 0; i < j.length; i++) {
					var sr = i + 1;
					jsondata.push([ sr, j[i].unit_name, j[i].army_no, j[i].rk,
							j[i].name, j[i].trade, j[i].dob, j[i].dos,
							j[i].doe, j[i].tos, j[i].med_cat,
							j[i].army_courses, j[i].spousename, j[i].dom ]);
	...
	function exportToExcel(tableId, fileName) {
		...
		XLSX.utils.book_append_sheet(wb, ws, "Pers(JCO)");
			...
			function() {
				...
				susNoAuto.autocomplete({
					source : function(request, response) {
						$.ajax({
							type : 'POST',
							url : "getArmy_noListApproved_forunit?" + key + "="
									+ value,
							data : {
								army_no : army_no,
								type1 : type1
	...
	function downloaddata() {
		document.getElementById('type_of_report').value = "jco_held";
