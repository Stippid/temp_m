		...
		<div class="col-md-12">
			<div class="card">
				<div class="card-header" align="center">
					<strong style="text-decoration: underline;"> Pers(Offrs)</strong>
				</div>
	...
	<div class="col-md-12">
		<div align="center" class="print_btn">
			...
			<input type="button" class="btn btn-purple"
				...
				onclick="exportToExcel('persOffrs', 'Pers(Offrs)')">
	...
	<script>
		...
		function data(tableName) {
			...
			if (tableName == "persOffrs") {
				...
				$.post("getUnitReportPersOffrscount?" + key + "=" + value, {
					Search : Search,
					sus_no : sus_no
				...
				$.post("getUnitReportPersOffrs?" + key + "=" + value, {
					startPage : startPage,
					...
					for (var i = 0; i < j.length; i++) {
						...
						jsondata.push([ sr, j[i].unit_name, j[i].ic_no,
								j[i].rank, j[i].name, j[i].description,
								j[i].dob, j[i].dos, j[i].doc, j[i].tos,
								j[i].med_cat, j[i].cope, j[i].army_courses,
								j[i].civil_qualification, j[i].spouse_name,
								j[i].dom ]);
		...
		function printDiv(divName) {
			...
			row += '<div class="col-6 col-sm-6 col-md-6"><h3>Pers Held(Offrs)</h3> </div> ';
		...
		function exportToExcel(tableId, fileName) {
			...
			XLSX.utils.book_append_sheet(wb, ws, "Pers(Offrs)");
			XLSX.writeFile(wb, fileName + ".xlsx");
		}

		function downloaddata() {
			document.getElementById('type_of_report').value = "offrs_held";
