		...
		<div class="col-md-12">
			<div class="card">
				<div class="card-header" align="center">
					<strong style="text-decoration: underline;">Pers Auth(ORs)
					</strong>
				</div>
			</div>
		</div>
	...
	<div class="col-md-12">
		...
		<input type="button" class="btn btn-purple" style="padding: 5px 10px; font-size: 14px; 
			font-weight: bold; color: white; background-color: purple;" value="Export to Excel"
				onclick="exportToExcel('persOrAuth', 'Pers Auth(ORs)')">
		</div>
	</div>
 ...
 <input type="hidden" name="type_of_report" id="type_of_report"  value="0">
	...
	<script>
		...
		function data(tableName) {
			...
			if (tableName == "persOrAuth") {

				$.post("getUnitReportPersOrAuthcount?" + key + "=" + value, {
					Search : Search,
					sus_no : sus_no
				...
				$.post("getUnitReportPersOrAuth?" + key + "=" + value, {
		...
		function printDiv(divName) {
			...
			row += '<div class="col-6 col-sm-6 col-md-6"><h3>Pers Auth(OR)</h3> </div> ';
		...
		function exportToExcel(tableId, fileName) {
			...
			XLSX.utils.book_append_sheet(wb, ws, "Pers Auth(ORs)");
			XLSX.writeFile(wb, fileName + ".xlsx");
		}
		function downloaddata(){
            document.getElementById('type_of_report').value="or_auth";
