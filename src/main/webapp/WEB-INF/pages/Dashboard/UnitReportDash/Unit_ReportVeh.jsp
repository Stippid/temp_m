		...
		<div class="col-md-12">
			<div class="card">
				<div class="card-header" align="center">
					<strong style="text-decoration: underline;"> Veh ${type_veh}</strong>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="card-body card-block">  
							 <div class="row form-group" >
								<div class="col-md-6">
	              					<div class="row form-group">
						               <div class="col-md-4">
									<label class=" form-control-label" title="BA No is Mandatory">BA No </label>
										</div>
						                <div class="col-md-8">
								...
								<div class="col-md-6">
	              					<div class="row form-group">
						               <div class="col-md-4">
						                    <label class=" form-control-label"> Veh Type</label>
						                </div>
						                <div class="col-md-8">
						                   <input type="text" id="broadcat" name="broadcat" class="form-control autocomplete"  autocomplete="off"  onkeyup="return specialcharecter(this)"> 
						                </div>
						            </div>
	              				</div>
							</div>
							 <div class="row form-group" >
								<div class="col-md-6">
	              					<div class="row form-group">
						               <div class="col-md-4">
						                    <label class=" form-control-label">Broad Cat</label>
						                </div>
						                <div class="col-md-8">
						                   <input type="text" id="veh_type_select" name="veh_type_select" class="form-control autocomplete" autocomplete="off"  onkeyup="return specialcharecter(this)"> 
...
<input type="button" value="Print" class="btn btn-primary"
		...
		<input type="button" class="btn btn-purple" style="padding: 5px 10px; font-size: 14px; 
			font-weight: bold; color: white; background-color: purple;" value="Export to Excel"
				value="Export to Excel" onclick="exportToExcel('Veh', 'Veh Held')">
		</div>
	</div>
...
function Clear()
{
	window.location.reload();
		...
		function data(tableName) {
			...
			var ba_no=$("#ba_no").val();
			var broad_cat =$("#broadcat").val();
			 var veh_cat=$("#veh_type_select").val();
			 var sus_no = $('#sus_no').val();
			if (tableName == "Veh") {

				$.post("getUnitReportVehcount?" + key + "=" + value, {
					Search : Search,
					type_veh : type_veh,
					ba_no:ba_no,
					broad_cat:broad_cat,
				...
				$.post("getUnitReportVeh?" + key + "=" + value, {
					...
					for (var i = 0; i < j.length; i++) {
						...
						jsondata.push([ sr, j[i].unit_name, j[i].ba_no, j[i].broad_cat,
								j[i].veh_type, j[i].kms_run,
								j[i].classification, j[i].vintage,
								j[i].serv_unsv ]);
		...
		function printDiv(divName) {
			...
			row += '<div class="col-6 col-sm-6 col-md-6"><h3>Veh Held</h3> </div> ';
		...
		function exportToExcel(tableId, fileName) {
			...
			XLSX.utils.book_append_sheet(wb, ws, "Veh Held");
			...
			var  type="broad_cat";
				  susNoAuto.autocomplete({
				      source: function( request, response ) {
				        $.ajax({
				        	type: 'POST',
					    	url: "tmsveh_type?"+key+"="+value,
				        data: {broadcat:broadcat,type_veh:type_veh,type:type},
...
$("input#veh_type_select").keypress(function(){
	
	var broadcat = this.value;
	var type_veh = $("#type_veh").val();
	 var susNoAuto=$("#veh_type_select");
	 var type="veh_type";
		  susNoAuto.autocomplete({
		      source: function( request, response ) {
		        $.ajax({
		        	type: 'POST',
			    	url: "tmsBrodcat?"+key+"="+value,
		        data: {broadcat:broadcat,type_veh:type_veh,type:type},
		          success: function( data ) {
		        	 var susval = [];
		        	  var length = data.length-1;
		      ...
		      change: function(event, ui) {
		    	 if (ui.item) {   	        	  
		        	  ...
		        	  alert("Please Enter Broad cat");
		        	  document.getElementById("veh_type_select").value="";
		        	  susNoAuto.val("");	        	  
		        	  susNoAuto.focus();
		        	  return false;	             
		          }   	         
		...
		function downloaddata(){
            document.getElementById('type_of_report').value="veh_held";
            document.getElementById('type').value=$("#type_veh").val();
