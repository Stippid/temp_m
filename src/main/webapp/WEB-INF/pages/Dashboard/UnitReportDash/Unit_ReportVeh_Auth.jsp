		...
		<div class="col-md-12">
			<div class="card">
				<div class="card-header" align="center">
					<strong style="text-decoration: underline;"> Veh Auth</strong>
				</div>
			</div>
		</div>
	</div>
...
<input type="button" value="Print" class="btn btn-primary"
			...
			font-weight: bold; color: white; background-color: purple;" value="Export to Excel"
				value="Export to Excel" onclick="exportToExcel('Veh', 'Veh Auth')">
	...
	<script>
		...
		function data(tableName) {
			...
			if (tableName == "Veh") {
				...
				$.post("getUnitReportVehAuthcount?" + key + "=" + value, {
					Search : Search,
					type_veh : type_veh,
					sus_no : sus_no
				}, function(j) {
				...
				$.post("getUnitReportVehAuth?" + key + "=" + value, {
					...
					for (var i = 0; i < j.length; i++) {
						...
						jsondata
								.push([ sr, j[i].mct_no, j[i].mct_nomen,
										j[i].base, j[i].modification,
										j[i].gennotes, j[i].inlieu,
										j[i].reducedueinlieu, j[i].total ]);
		...
		function printDiv(divName) {
			...
			row += '<div class="col-6 col-sm-6 col-md-6"><h3> Veh Auth</h3> </div> ';
		...
		function exportToExcel(tableId, fileName) {
			...
			XLSX.utils.book_append_sheet(wb, ws, "Veh Auth");
			XLSX.writeFile(wb, fileName + ".xlsx");
		}
	
		function downloaddata(){
            document.getElementById('type_of_report').value="veh_auth";
