...
<div class="animated fadeIn">
	...
	<input type="button" value="Print" class="btn btn-primary"
			...
			font-weight: bold; color: white; background-color: purple;" value="Export to Excel"
				onclick="exportToExcel('wpn_eqpt_auth', 'Wpn & Eqpts Auth')">
		</div>
	</div>
 ...
 <input type="hidden" name="type" id="type"  value="0">
	...
	<script>
		...
		$(document).ready(function() {
			...
			colAdj('wpn_eqpt_auth');
		});

		mockjax1('wpn_eqpt_auth');
		table = dataTable('wpn_eqpt_auth');
		...
		function data(tableName) {
			...
			if (tableName == "wpn_eqpt_auth") {

				$.post("getUnitReportWpnEqptAuthcount?" + key + "=" + value, {
					Search : Search,
				...
				$.post("getUnitReportWpnAuthEqpt?" + key + "=" + value, {
					...
					for (var i = 0; i < j.length; i++) {
						var sr = i + 1;
						jsondata.push([ sr, j[i].item_code, j[i].item_type,
							j[i].type, j[i].total_ue_qty ]);
		...
		function printDiv(divName) {
			...
			row += '<div class="col-6 col-sm-6 col-md-6"><h3>Wpn & Eqpts Auth</h3> </div> ';
		...
		function downloaddata(){
            document.getElementById('type_of_report').value="eqpts_auth";
