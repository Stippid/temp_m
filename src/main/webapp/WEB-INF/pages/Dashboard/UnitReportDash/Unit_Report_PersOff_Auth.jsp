		...
		<div class="col-md-12">
			<div class="card">
				<div class="card-header" align="center">
					<strong style="text-decoration: underline;"> Pers
						Auth(Offrs)</strong>
				</div>
	...
	<div class="col-md-12">
		<div align="center" class="print_btn">
			...
			font-weight: bold; color: white; background-color: purple;" value="Export to Excel"
				onclick="exportToExcel('persOffrsAuth', 'Pers Auth(Offrs)')">
 ...
 <input type="hidden" name="type_of_report" id="type_of_report"  value="0">
	...
	<script>
		...
		$(document).ready(function() {
			...
			// 	clearAll();
			colAdj("persOffrsAuth");

		});

		mockjax1('persOffrsAuth');
		table = dataTable('persOffrsAuth');
		...
		function data(tableName) {
			...
			if (tableName == "persOffrsAuth") {

				$.post("getUnitReportPersOffrsAuthcount?" + key + "=" + value,
						{
							Search : Search,
							sus_no : sus_no
				...
				$.post("getUnitReportPersOffrsAuth?" + key + "=" + value, {
					startPage : startPage,
					...
					for (var i = 0; i < j.length; i++) {
						...
						jsondata.push([ sr, j[i].arm_desc, j[i].rank,
								j[i].base_auth, j[i].mod_auth, j[i].foot_auth,
								j[i].total_auth ]);
		...
		function printDiv(divName) {
			...
			row += '<div class="col-6 col-sm-6 col-md-6"><h3>Pers Auth(Offr)</h3> </div> ';
		...
		function exportToExcel(tableId, fileName) {
			...
			XLSX.utils.book_append_sheet(wb, ws, "Pers Auth(Offrs)");
			XLSX.writeFile(wb, fileName + ".xlsx");
		}
		
		function downloaddata(){
			document.getElementById('type_of_report').value="offrs_auth";
