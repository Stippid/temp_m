...
<script src="js/miso/commonJS/commonmethod.js" type="text/javascript"></script>
	...
	<div class="mt-1">
		<div class="col-md-12" >
			<div class="card">	
			<div class="card-header" align="center"> <strong style="text-decoration: underline;"> Comd Wise Auth of Tpt</strong> 
	    			</div>	
					<div class="card-body">
				...
				<div class="col-md-6">
					...
					<div class="col-md-6">
						...
						<div class="col-md-6">
							<div class="row form-group">
								<div class="col col-md-4">
									<label class=" form-control-label">Nomenclature of Veh:</label>
								</div>
								<div class="col-12 col-md-6">
                				<div class="selectBox"  onclick="showcheckbox()">
			
						<select name="nomenClature_1" id="nomenClature_1"
							class="form-control">
							<option>--Select--</option>
						</select>
						<div class="overSelect"></div>
					</div>
							<div id="std_nomclature" class="checkboxes ${not empty selectedBdesList}"
				...
				<a href="DocTypeReportWetPetFet?type=tpt" class="btn btn-success btn-sm" >Clear</a>  
...
function data(tableName) {
	...
	findselecteNC();
	var std_nomclature = $("#CheckValNC").val();
	var arm = $("#arm").val();
	...
	if (tableName == "getSearch") {
	    ...
	    $.post("getDocTypeDetaisReportTptount?" + key + "=" + value,{
	    	Search:Search,
		...
		$.post("getDocTypeDetaisReportTpt?" + key + "=" + value, {
			...
			for (var i = 0; i < j.length; i++) {
				...
				jsondata.push([j[i].comd_name, j[i].ff, j[i].nff, j[i].totalue]);
...
function showcheckbox(){
	...
	var selected = $("#prf_group").val();	
	if (selected==''){
		alert("Please select PRF Group");
		return false;
	}
	if (count==1){
	$.post("getitemtypedashTpt?"+key+"="+value,{selected:selected}, function(j) {
		  if (j != "") {
	            var length = j.length;
	            var options = '';
	...
	var checkboxes = document.getElementById("std_nomclature");
	$("#std_nomclature").toggle();
	$('.nomenClature_1').each(function() {
		$(this).show()
	})
	}
function findselecteNC(){
var nrSel = $(".NCCheckBox:checkbox:checked").map(function() {
