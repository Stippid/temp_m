...
<c:if test="${nBase[0][4] !=null}">
	...
	<c:if test="${dataf[0] =='C'}">
				...
				<div id="nMsgBoard" style="width:100%;height:inherit">
					...
					<div class="col-md-9">
						...
						<c:if test="${fn:length(nBase[2][5])>0}">							
							<p class="DBHead DBHeadC2 blink1 nPopHindiBold" style="font-size:3em;color:${nBase[2][8]}">*${nBase[2][5]}*</p>
						</c:if>						
					</div>
					<div class="col-md-2">
		...
		<div class="row" style="align-items: flex-start !important;">
			<c:forEach var="item" items="${nBase}" varStatus="num">
				<c:if test="${item[2] =='4'}">
				
			         <div class="col-md-4 nChipBlock" ondblclick="callPop('${item[6]}','${item[11]}');">							         			         	
			         	<div class="nChip nChipW" id="chartgauge_1" >
			         		<c:if test="${fn:length(item[4])>0}">							         		
			         			<img src="js/common/img/${item[4]}">
			         		</c:if>
			         		<span class="form-control Box Box1" style="color:${nBase[2][8]}!important;">${item[5]}</span>
			         	</div>
					 </div>
