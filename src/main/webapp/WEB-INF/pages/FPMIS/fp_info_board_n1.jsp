...
<script type="text/javascript">
	...
	function chgeffect() {
		$("#nrTableShowDtl").html("<tr><td colspan=' 8' class='blink' style='font-size:2.0em;'><center>Press 'Get Data' Button ...");
		...
		<div class="" align="center" style="width:99vw; margin:0px;padding:0px;border:0px;overflow: auto;">
			<div class="card">
				<div class="card-header mms-card-header ncard-body-bg"
					style="min-height: 50px; padding-top: 10px;"><h5><span>FUND PERFORMANCE INDICATOR</span></h5>
					
				</div>
				<div class="card-body card-block" style="height:60vh;background-color: azure;margin:0px;padding:0px;border:5px;width:100%;overflow: auto;">
	            	<div class="row form-group" id="nBase_data"  style="display:none;margin-top: -20;background-color: azure;">	
						<div id="divShowDtl" class="" style="width:100%;display1:none;z-index:3;background-color: azure;">
							<div style="width:100%">
								<div style="font-size:1.1vw;float:right;display:none;" id="nrTableDataDivDtlTitleee">&nbsp;&nbsp;Show&nbsp;&nbsp;								
							...
							<div class="" style="width:100%">
								...
								<div id="nMsgBoard" style="width:100%;">									
									<div class="row" style="background-color: azure;width: calc(79vw - 8px);display:inherit;margin-left: 5px;">
										<div class="" style="align-self:center;padding:3px;width: calc(98vw - 8px)!important;display:inline-flex;">
			    	                       ...
			    	                       <%-- <div class="col-md-3" style="text-align: left;float: left;">	    	                       
											...
											<div class="col-md-5">												
													...
													<select id="DtlBH" name="DtlBH" style="width:60%;" title="Select Budget Holder to Get Data" onchange="chgeffect();">
														...
														<c:forEach var="item" items="${n_hq}" varStatus="num">
																<c:if test="${item[1]==nSelp[0]}">
							   									...
							   									<c:if test="${item[3] =='-1'}">
								   										<option value="${item[3]}_${item[0]}_${item[1]}" ${issel}>${item[2]}</option>
								   								</c:if>
							   									<c:if test="${item[3] =='0'}">
								   										<option value="${item[3]}_${item[0]}_${item[1]}" ${issel}>${item[2]}</option>
								   								</c:if>																
										 	...
										 	<div class="col-md-3">
										 		<label class=" form-control-label">Amt in </label>&nbsp;&nbsp;
												<select id="amt_rs" name="amt_rs" class="form-control-sm" title="Select Amount Conversion" onchange="chgeffect();">
													<c:set var="amt_rss" value="${fn:split(n_sel,':')}"/>																							
														<option value="CR" 
														<c:if test="${nSelp[1]=='CR'}">
															selected														
														</c:if>													
														 >Crores</option>												
														<option value="RS"
														<c:if test="${nSelp[1]=='RS'}">
															selected														
														</c:if>
														>Rupees</option>
												</select>
												&nbsp;
												<input id="refdata" type="submit" class="btn btn-success btn-sm nGlow" value="Get Data" style="bbackground-color: purple;" data-toggle="tooltip" title="Get Data" onclick="return nGetData();">
										 	</div>
										 	
										 	
										 	
											<div class="col-md-4">
											<span><b>Filter Allottee&nbsp;:&nbsp;</b></span>
	 								...
	 								<div class="nPopTable" id="nrTableDataDivDtl" style="height:49vh;width:100%!important;margin-top:10px;overflow: auto;">
										...
										<table style="width:100%;border-collapse: separate;table-layout: fixed;">
							               	<thead style="text-align: center;line-height:25px;font-size: 1vw;border-spacing: 0 0PX!important;" id="th_nrTableShowDtl">  
							    				<tr>							    					
								   					<th width="130px" title="Fmn HQ of Budget Holder as per Orbat">Allottee Fmn HQ</th>
								      				<th width="130px" title="Who has Allotted Fund (High Level Budget Holder[HLBH])">Allotter (HLBH)</th>
								      				<th width="130px" title="To Whom Fund Allotted (Budget Holder)">Allottee (BH)</th>
								      				<th width="100px" class="EBBK">CBM Gp</th>
								      				<th width="100px" class="EBBK">Minor Head</th>
								      				<th width="150px" class="EBBK">Code Head</th>
<!-- 								      				<th width="110px" title="Fund Received">Fund Received</th>
								      				<th width="110px" title="Fund Allocated">Allocation</th>
 -->							      				<th width="110px" title="Fund Received / Fund Allocated">Allocation</th>
													...
													<th width="110px" class="">Flow Level - 1</th>
													<th width="110px" class="">Flow Level - 2</th>
													<th width="110px" class="">Flow Level - 3</th>
													<th width="110px" class="">Flow Level - 4</th>
													<th width="110px" class="">Flow Level - 5</th>
													<th width="110px" class="">Flow Level - 6</th>
													<th width="110px" class="">Flow Level - 7</th>
													<th width="110px" class="">Flow Level - 8</th>
													<th width="110px" class="">Flow Level - 9</th>
													<th width="150px" class="EBBK">CBM Detl</th>
									  			</tr>
							    		  	</thead>							    		  
										    <tbody id="nrTableShowDtl" style="font-size: 1vw;text-decoration: none;color:blue;">
											   <c:if test="${empty list[0][2]}">
													<tr>
														<td style="font-size: 15px; text-align: center; color: red;"
															colspan="8">No Transaction Available</td>
													</tr>
											   </c:if>
											   <c:if test="${not empty list[0][2]}">
											   		<c:if test="${nSelp[1] !='null'}">
											   	   		<c:set var="amtrs" value="${nSelp[1]}"></c:set>
											   	   	</c:if>
						                           <c:forEach var="item" items="${list}" varStatus="num" >
						                           	...
						                           	<c:if test="${item[0]=='T2'}">
						                           		...
						                           		<c:set var="aper" value=":1AP${item[26]}"></c:set>					                           		
						                           	</c:if>
						                           	<c:if test="${item[0]=='T3'}">
						                           		<c:set var="aaab" value="RLVL_T3"></c:set>
						                           		<c:set var="aaac" value="SUMT3"></c:set>
						                           		<c:set var="aper" value=":2AP${item[26]}"></c:set>		
						                           		<script></script>			                           		
						                           	</c:if>
													...
													<c:set var="rowid" value="${aaab}:SUS_${item[5]}:HQ_${item[3]}"></c:set>					
													<c:if test="${rowid!='RLVL_T1:SUS_:HQ_'}">
													<%-- data-rowidx="{'index':'${item}'}" --%>								                         	
											          <tr class="${aaac}${aper}" style="font-size:1.1vw;background-color:transparent;" id="${rowid}">
														 ...
														 <%-- <c:if test="${item[0]=='T3'}"> --%>
														 	<td class="EBBK"><span style="float:right">${item[27]}</span><br>${item[29]}</td>
														 	<td class="EBBK">${item[8]}</td>
														 	<td class="EBBK">${item[13]}<br>${item[9]}</td>
														 <%-- </c:if>
														 <c:if test="${item[0]!='T3'}">
														 	<td colspan='2' class="EBBK nTextRight ${totrow}" name="${aaac}${aper}">Sub Total</td>
														 </c:if> --%>			 													 
														 <!-- <td class='nTextRight' style="font-size: 1.2vw;"><script>document.write(Number('${item[10]}').toINR('','RS','INR','RS'))</script></td> -->
														 
														 <td class='nTextRight' style="font-size: 1.2vw;"><script>document.write(Number('${item[11]}').toINR('','RS','INR','${amtrs}'))</script></td>
														 <td class='nTextRight' style="font-size: 1.2vw;"><script>document.write(Number('${item[12]}').toINR('','RS','INR','${amtrs}'))</script></td>
														 <td class='nTextRight' style="font-size: 1.2vw;"><script>document.write(Number('${item[14]}').toINR('','RS','INR','${amtrs}'))</script></td>
														 <td class='nTextRight' style="font-size: 1.2vw;"><script>document.write(Number('${item[15]}').toINR('','RS','INR','${amtrs}'))</script></td>
														 <td class='nTextRight' style="font-size: 1.2vw;"><script>document.write(Number('${item[26]}').toINR('','RS','INR','RS'))</script></td>
														 <td class=''>${item[17]}</td>
														 <td class=''>${item[18]}</td>
														 <td class=''>${item[19]}</td>
														 <td class=''>${item[20]}</td>
			...
			<div class="card-footer" align="left" style="padding:0px!important;">
				<div class="row" style="font-size:0.8em">
					<div class="col-md-4">
						...
						<input type="text" id="bhld_slvl" name="nBase_slvl_src" placeholder="Search..." size="10" maxlength="10" class="form-control-sm"  style="text-transform: uppercase;"
					...
					<%-- <div class="col-md-4">
				 	...
				 	<div class="col-md-8">
						...
						<div class="col-md-8" style="displaya:none;text-align:right;">						
...
<script type="text/javascript">
		function nGetData(){
			var a=confirm("Please Note:\n\nData Calculation may take more time.\nThe Page may be inresponsive.\n\nDo you still want to Continue ...");
			if (a==true) {
				$("#nrTableShowDtl").html("<tr><td colspan=' 8' class='blink' style='font-size:2.0em;'><center>Data Calculation is under Progress. Please Wait ...");
		...
		$(document).ready(function() {
		    ...
		    $("#nrTableShowDtl tr").on("click", function() {
		    	//var value="0";
		    	var value = $(this).data("rowidx");
		    	var hlst="list["+value+"][19]";
		    	var hlst1="${"+hlst+"}";
		    	console.log(value);
		    	//console.log(a);
		    	/* if ($(this).data("rowidx").length>0) {
		    		var value = $(this).data("rowidx");
		    	}
