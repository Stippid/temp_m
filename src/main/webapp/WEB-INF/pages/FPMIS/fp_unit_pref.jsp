...
<body onload="setMenu();">
<form:form action="fp_unit_assignAction?${_csrf.parameterName}=${_csrf.token}" id="ms_hos_assign"  method="post" class="form-horizontal fp-form" commandName="fp_unit_assignCMD">
      <div class="containerr" align="center">
          <div class="card">
              <div class="card-header mms-card-header" style="padding:20px;">
		             <h5>CREATE BUDGET HOLDERS ( ALLOTTEE )</h5>
		      </div> 		      
		      <div class="card-body card-block ncard-body-bg">
		      		<div class="row">   
		      				 <div class="col-md-1" >
		      				 	<label for="text-input" class=" form-control-label">Est</label>&nbsp;&nbsp;
		      				 </div>
		               		 <div class="col-md-9" >
		               		 		<c:set var="cursus" value="${roleSusNo}"/>	               		 
										...
										<c:forEach var="item" items="${n_unt}" varStatus="num">
											<c:if test="${item[1]==cursus}">
												<option value="${item[1]}">${item[2]}</option>
											</c:if>
								        </c:forEach>
						            </select>
		               			 
		               			  <input type="hidden" id="username" name="username" class="form-control-sm form-control" value="${username}" readonly>
              ...
              <div class="card-body card-block" id="d_reg" style="display:">
				    <div class="col-md-12 row form-group">
            	        <div class="col-md-5">
            	             <b>List of Budget Holders</b>
            	             <input id="nrInputa" type="text" placeholder="Search Data..." size="20" style="float:right;" title="Search Budget Holder.">
					    </div>   
					    <div class="col-md-5 " align="right">
					         <b>Selected Budget Holders - <span id="tregn" style='font-size:14px;'>0</span></b>
					    </div> 
				    </div>
				    
				    
					<div class="col-md-12 row form-group" style="margin-left: 1px;">					
					     <div class="col-md-5 nPopTable" style="height: 45vh;overflow:auto;width:99%;border:1px solid #000;text-align: left;" id="srctable">
					     </div>
					     ...
					     <div class="col-md-2" style="padding-left: 45px;">
					      ...
					      <div class="col-md-5 nPopTable" style="height: 45vh;overflow:auto;width:99%;border:1px solid #000;text-align: left;" id="tartable">
     	 ...
     	 <c:if test="${n_noed=='Y'}">
  			  <div class="form-control card-footer" align="center" >
            	    <input type="submit" class="btn btn-success btn-sm nGlow" id="btn_save" value="Save"  onclick="" title="Click to Save Data.">
              </div>
         </c:if>
         <c:if test="${n_noed=='N'}">
...
function drawregn(j) {
	...
	for(var i = 0;i < a.length;i++){		
		...
		if(datap[1] != null){   
		 	...
		 	rowS=rowS+"<td>&nbsp;<input class='nrCheckBox' type=checkbox id='"+datap[1]+"' name='"+datap[0]+"' onclick='findselected(this);' title='Select Budget Holder'>";
...
function getheadlist(){
	...
	$.post("getUnitPrefList?"+key+"="+value,{nPara:selsus}, function(j) {
		if(j == ""){
			$("#srctable").empty();
			$("#d_reg").show();
    		$("#d_reg2").show();
...
function getAssignData()
{
	...
	var selsus= $("#sus_no").val();
	if (selsus==null || selsus=="") {
		selsus="${roleSusNo}";
	}
	$.post("getfpUnitAssignList?"+key+"="+value,{user_id:user_id1,nPara:selsus}, function(j) {
   		$("#d_reg").show();
   		$("#d_reg2").show();
   		$("#ch_d2").show();
   		$("#tartable").show();
...
$(document).ready(function() {
	...
	$("#btn_save").click(function(){
		var flag = false;
		flag = uncheckedCount > 0 ? (confirm("Associated Code Head with the unchecked budget holder(s) will be deleted\nDo you wish to continue?") ? true : false) : true;
		return flag;
	});
});	
</script>