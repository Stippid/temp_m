...
<c:url value="SearchDelete_Computing_AssetsUrl_1" var="Search_Computing_AssetsUrl_1" />
		...
		,from_date:from_date,to_date:to_date,status:status,s_state:s_state,unit_sus_no:unit_sus_no,unit_name:unit_name},function(j) {
			...
			for (var i = 0; i < j.length; i++) {
         	var data =[j[i].assets_name,j[i].model_number,j[i].machine_number,j[i].mac_address,j[i].ip_address,j[i].processor_type,j[i].ram,
				j[i].hdd,j[i].os,j[i].office,j[i].os_firmware,j[i].dply_env,j[i].chekboxaction
//                  <c:if test="${roleType == 'APP'  && status1 == '0'}">
//                	 ,j[i].chekboxaction
//                  </c:if>
//                  ,j[i].action 
	...
	$.post("getTotalAssetCount1Delete?"+key+"="+value,{Search:Search,assets_type:assets_type,model_number:model_number,machine_number:machine_number,ram_capi:ram_capi,hdd_capi:hdd_capi,operating_system:operating_system
		,from_date:from_date,to_date:to_date,status:status,s_state:s_state,unit_sus_no:unit_sus_no,unit_name:unit_name},function(j) {
		FilteredRecords = j;
	});
}
