...
<form:form  name="Pop_Up_Computing_Asset_WO" id="Pop_Up_Computing_Asset_WO"  method='POST'   >
	<div class="col-md-12" align="center">
	   <div class="card">
	  		<div class="card-header"><h5> SEARCH COMPUTING WORK ORDER</h5></div>
	  ...
	  <div class="col-md-12">
			<table id="WO_Computing_Asset_reporttbl" class="display table no-margin table-striped  table-hover table-bordered ">
				<thead>
			        <tr>					
						<th>Work Order No</th>
	...
	<!--  Modal Start -->
	 ...
	 <span class="popupClose" data-dismiss="modal">&times;</span>
	  <div class="modal-body">
						<table class="table no-margin table-striped  table-hover  table-bordered table-primary" id="PopUp_Tb_Computing_History">
							<thead>
								<tr>
  ...
  <!-- Modal End -->
	...
	$(document).ready(function() {  
		...
		mockjax1('WO_Computing_Asset_reporttbl');
		table = dataTable('WO_Computing_Asset_reporttbl');
	...
	function data(tableName){
		if(tableName == "WO_Computing_Asset_reporttbl"){
			...
			$.post("Pop_UP_Computing_Asset_History_ReportDataList?"+key+"="+value,{
				startPage:startPage,
			...
			$.post("Pop_UP_Computing_Asset_History_List_TotalCount?"+key+"="+value,{Search:Search,assets_type:assets_type,
				wo_no:wo_no,
				wo_dt:wo_dt,
				dt_eqpt_reqd_fwd_wksp:dt_eqpt_reqd_fwd_wksp,
				wksp_unit_name:wksp_unit_name,
	...
	function historyData(id){  
		...
		$.post("pop_up_detail_Computing_Asset_History_RecordDataList?"+key+"="+value,{id:id},function(j) {
			
			 if(j.length>0){
				 $('#computing_PopTbbody').empty();
	 	        	for ( var i = 0; i < j.length; i++) {
	 	        		 $("#computing_PopTbbody").append(' <tr id="tr_id_Computingpopup'+i+'">'
