...
<form:form  name="" id="" action="Model_master_Action?${_csrf.parameterName}=${_csrf.token}" method='POST' commandName="Model_master_CMD" > 
<spring:htmlEscape defaultHtmlEscape="true" /> 
<div class="animated fadeIn">
   		<div class="container" align="center">
 			<div class="card">
	   			<div class="card-header"><h5>MODEL:OTHERS</h5></div>
...
function setApproveStatus(){
	...
	if(a == ""){
		alert("Please Select the Data for Approval"); 
			...
			$.post("Approve_MpodelotherData?"+key+"="+value, {a:a,status:"1"}).done(function(j) {
			alert(j);
			location.reload();
		}); 
	}	
...
function setDeleteStatus(){
	...
	if(a == ""){
		alert("Please Select the Data for Rejection."); 
			...
			$.post("Delete_MpodelotherData?"+key+"="+value, {a:a}).done(function(j) {
			alert(j);
			location.reload();
		}); 
	}	
...
function setRejectStatus(){
	...
	if(a == ""){
		alert("Please Select the Data for Reject"); 
			...
			$.post("Approve_MpodelotherData?"+key+"="+value, {a:a,status:"3"}).done(function(j) {
			alert(j);
			location.reload();
		}); 
	}	
