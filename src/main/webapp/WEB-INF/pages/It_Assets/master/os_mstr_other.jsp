...
<form:form name="Formname" id="Formid" action="os_mstrAction?${_csrf.parameterName}=${_csrf.token}" method="POST" commandName="os_mstrCMD">
<div class="container" align="center">
	<div class="card">
    <div class="card-header"> <h5>OPERATING SYSTEM : OTHERS</h5></div>
       <div class="card-body card-block cue_text">
<div class="col-md-12" id="divLine" style="display: none;"><span class="line"></span></div>
 <div class='col-md-6'>
	  <div class='row form-group'>
	      <div class='col col-md-4'>
	           <strong style="color: red;">*</strong> <label for="text-input" class="form-control-label">OPERATING SYSTEM</label>
          </div>
          <div class='col-12 col-md-6'>
	             <form:input type="text" id="os" path="os"  class="form-control" autocomplete='off' maxlength="100"  oninput="this.value = this.value.toUpperCase()"></form:input>
        </div>
    </div>
</div>
<div class="col-md-6">
...
<table id="os_mstr_reporttbl" class="display table no-margin table-striped  table-hover  table-bordered report_print">
	<thead>
        <tr>
...
function data(tableName){
	...
	$.post("getOs_mstr_OtherReportDataList?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType,os:os,status:status},function(j) {
	...
	$.post("getOs_mstr_OtherTotalCount?"+key+"="+value,{Search:Search,os:os,status:status},function(j) {
		FilteredRecords = j;
	});
}
	...
	function setApproveStatus(){
		...
		if(a == ""){
			alert("Please Select the Data for Approval"); 
				...
				$.post("Approve_OsotherData?"+key+"="+value, {a:a,status:"1"}).done(function(j) {
				alert(j);
				location.reload();
			}); 
		}	
	...
	function setDeleteStatus(){
		...
		if(a == ""){
			alert("Please Select the Data for Rejection."); 
				...
				$.post("Delete_OsotherData?"+key+"="+value, {a:a}).done(function(j) {
				alert(j);
				location.reload();
			}); 
		}	
