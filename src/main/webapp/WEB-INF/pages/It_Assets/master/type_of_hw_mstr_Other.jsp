...
function data(tableName){
	...
	$.post("getType_of_hw_other_mstrReportDataList?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType,peripheral_type:peripheral_type,type_of_hw:type_of_hw,status:status},function(j) {
	...
	$.post("getType_of_hw_Other_mstrTotalCount?"+key+"="+value,{Search:Search,peripheral_type:peripheral_type,type_of_hw:type_of_hw,status:status},function(j) {
		FilteredRecords = j;
	});
}
	...
	function setApproveStatus(){
		...
		if(a == ""){
			alert("Please Select the Data for Approval"); 
				...
				$.post("Approve_type_of_hw_Data?"+key+"="+value, {a:a,status:"1"}).done(function(j) {
				alert(j);
// 				Search();
				location.reload();
			}); 
	...
	function setDeleteStatus(){
		...
		if(a == ""){
			alert("Please Select the Data for Rejection."); 
				...
				$.post("Delete_type_of_hw_Data?"+key+"="+value, {a:a}).done(function(j) {
				alert(j);
				location.reload();
			}); 
		}	
