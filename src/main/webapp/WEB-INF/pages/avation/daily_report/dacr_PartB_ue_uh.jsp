				...
				<table id="getpartBReportMcr"
					...
					<tbody>
						<c:if test="${getdetails_ue_uhdacr.size() != 0}">
							<c:forEach items="${getdetails_ue_uhdacr}" var="list"
								varStatus="num">
								<tr>
...
<c:url value="SearchReportDacr" var="backUrl" />
...
function Reject(ason_date,issue_states){
	 var sus =  $("#sus_noSer").val();
	   document.getElementById('rejectid_model').value=sus;
		var ason_date1= convertToISO8601(ason_date) ;
		
		generate_tail_no(ason_date1,issue_states);

}
function generate_tail_no(ason_date1,issue_states){
	var sus_no = $("#sus_noSer").val();
	$("table#addQuantity tbody").empty();
	$.post("generate_tail_no?"+key+"="+value, {sus_no:sus_no,ason_date1:ason_date1,issue_states:issue_states}).done(function(k) {
		var i =1;
