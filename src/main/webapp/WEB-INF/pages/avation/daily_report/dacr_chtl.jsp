...
.table th {
  ...
  background-color: #9c27b0;  /* #4CAF50;  Green background for header cells */
  color: white; /* White text color */
}
  ...
  <div class="container" align="center">
    <div class="card">
      <div class="card-header">
        <h5>DAILY AVIATION CASUALTY RETURN FOR CTH/CTK/CTL<br></h5>
        <h6><span style="font-size: 12px; color: red">(To be entered by Unit)</span></h6>
      </div>
    <div class="card-body card-block">
      ...
      <div class="form-control card-footer" align="center" id="searchBTN">
        <input type="button" class="btn btn-primary btn-sm" value="Search" onclick="submitSearch();">
      </div>
    </div>
  </div>
...
<table  class="table no-margin table-striped  table-bordered report_print">
    ...
    <!-- First Table: First 9 Columns -->
    <div class="tbl_scroll" id="dacr_header">
      <table id="SearchReportPart1" class="table no-margin table-striped table-hover table-bordered report_print">
        <thead>
          <tr>
            ...
            <th colspan="2"  style=" width:18%">Daily Flg Hrs as on 
            ...
            <th rowspan="2"  style=" width:9%">ENG Ser No<br><span>---------</span><br>ENG Hrs</th>
             <th rowspan="2" style=" width:9%">HRS Left<br><span style="font-size: 10px;">(Next INSP)</span></th>
          </tr>
          <tr>
    ...
    <div class="tbl_scroll" id="dacr_footer">
      <table id="SearchReportPart2" class="table no-margin table-striped table-hover table-bordered report_print">
        <thead>
          <tr>
             <th rowspan="2" style=" width:6%">Sr No</th>
         
           <th rowspan="2" style=" width:10%">DAYS Left<br><span style="font-size: 10px;">(Next INSP)</span></th>
            <th rowspan="2" style=" width:9%">Next INSP</th>
        ...
        <tbody>
          <c:if test="${list.size() == 0}">
            <tr>
              <td colspan="8" align="center" style="color: red;">Data Not Available</td>
            </tr>
          </c:if>
          <c:forEach items="${list}" var="item" varStatus="num">
            <tr>
             <td style="font-size: 15px; width:6%;">${num.index + 1}</td>
            
              <td style=" width:10%">${item[20]}</td>
              <td style=" width:9%">${item[10]}</td>
...
<c:url value="UpdategetDACRReportCHTLList" var="searchUrl" />
...
function submitData(acc_no, j) {
    ...
    var engHrs = jQuery("#eng_hrs_" + j).val();
    var hrsLeft = jQuery("#hrs_left_" + j).val();
    var daysLeft = jQuery("#days_left_" + j).val();
...
function calculateHrsForRecord(recordId) {
    ...
    var isFirstDayOfQuarter = isFirstDayOfMonth && [0, 3, 6, 9].includes(today.getMonth());


    var isFirstDayOfHalfYear = isFirstDayOfMonth && [3, 9].includes(today.getMonth()); 

    var isFirstDayOfApril = isFirstDayOfMonth && today.getMonth() === 3; 

 
    var falfHrsDay = document.getElementById("falf_hrs_day_" + recordId).value;
    var falfHrsNight = document.getElementById("falf_hrs_night_" + recordId).value;
    var gRun = document.getElementById("g_run_" + recordId).value;
    var afhrs = document.getElementById("af_hrs_" + recordId).value;
    var lh = document.getElementById("eng_hrs_" + recordId).value;
    ...
    if (!isValidTimeFormat(falfHrsDay) || !isValidTimeFormat(falfHrsNight) || !isValidTimeFormat(gRun)) {
    ...
    var afHrs = minutesToTime(afhrsMinutes + falfHrsDayMinutes + falfHrsNightMinutes);
    var lhHrs = minutesToTime(lhMinutes + falfHrsDayMinutes + falfHrsNightMinutes + gRunMinutes);
    var hrs = minutesToTime(hleftMinutes - falfHrsDayMinutes - falfHrsNightMinutes - gRunMinutes);
    var mHrs = minutesToTime(mMinutes + falfHrsDayMinutes + falfHrsNightMinutes);
    var qrHrs = minutesToTime(qrMinutes + falfHrsDayMinutes + falfHrsNightMinutes);
    var hyHrs = minutesToTime(hyMinutes + falfHrsDayMinutes + falfHrsNightMinutes);
    var yHrs = minutesToTime(yrMinutes + falfHrsDayMinutes + falfHrsNightMinutes);

    // Update the calculated values in the corresponding fields
    document.getElementById("af_hrs_" + recordId).value = afHrs;
    document.getElementById("eng_hrs_" + recordId).value = lhHrs;
...
function isValidTimeFormat(time) {
    var regex = /^\d{2}:\d{2}$/;
    return regex.test(time) && !isNaN(time.split(":")[0]) && !isNaN(time.split(":")[1]);
}
...
// Ensure the user cannot type anything else, and add leading zeros for single digits
function ensureLeadingZero(event) {
    let inputElement = event.target;
    let value = inputElement.value;

    if (value.length === 1) {
        inputElement.value = "0" + value; // Add leading zero
    }
}

document.getElementById('falf_hrs_day_123').addEventListener('mousedown', function (e) {
    e.preventDefault(); 
});


document.getElementById('falf_hrs_day_123').addEventListener('keydown', function (e) {
...
document.getElementById('falf_hrs_day_123').addEventListener('selectstart', function (e) {
    e.preventDefault(); 
});
</script>
...
var sus_no='${sus_no}';
	...
	else if((issue_summary != "" && issue_summary != null) || (description != "" && description != null))
	{
		...
		$.ajax({
			...
			url : "uploadDocDACRchtl?${_csrf.parameterName}=${_csrf.token}",
			data : data,
			processData : false,
			contentType : false,
			cache : false,
