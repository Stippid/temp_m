 ...
 {	
 	if(val != "" || val != null) {		
	     ...
	     $.post("getSearchsusbywepenoDetailsList?"+key+"="+value, {sus_no : sus}).done(function(j) {
 	    	 if(j!="" && j!=null)	
 			 	document.getElementById("We_pe_no").value=j[0][1];		
 			 else{
 				 if('${unit_sus_no1}' !=""){
 				 	document.getElementById("We_pe_no").value="";
 				 }
...
<div class="container" align="center">
	        	<div class="card">
	        		<div class="card-header"><h5><b>Search LINK SUS No with WE/PE No Transport</b><br>
						...
						<div class="col-md-6">
 							<div class="row form-group">  								
               					...
               					<div class="col-12 col-md-6">
                 					<input  id="unit_sus_no_trans" name="sus_no" style="font-family: 'FontAwesome',Arial;" placeholder="&#xF002; Search" autocomplete="off" class="form-control">
								</div>
 							</div>
 						</div>             					
              			<div class="col-md-6">
  					...
  					<div class="col-md-12">	
  							...
  							<div class="col-md-6">
  									<div class="row form-group">
	                					...
	                					<div class="col-12 col-md-6">
	                  						<input  id="We_pe_no" name="wepe_trans_no" style="font-family: 'FontAwesome',Arial;" placeholder="&#xF002; Search" class="form-control autocomplete" autocomplete="off">
	                					</div>
	                			</div>
	                		</div>
	                	</div>
...
<div style="width: 20%; padding-left: 1%; display: none;" id="divSerachInput">
	<input id="searchInput" type="text" style="font-size:12px; font-family: 'FontAwesome',Arial;margin-bottom: 5px;" placeholder="&#xF002; Search Word"  size="35" class="form-control">
		...
		<div id="divShow" style="display: block;">
			...
			<div  class="watermarked" data-watermark="" id="divwatermark" style="display: block;">
					<span id="ip"></span>
			<table  id="SearchLinkTrans" class="table no-margin table-striped  table-hover  table-bordered report_print" >
				 <thead>
					<tr>
						...
						<th>MISO WE/PE No</th>
						<th>Modification</th>
						<th class='lastCol' id="thAction">Action</th>
      ...
      <c:url value="searchLinkWEPETrans1" var="searchUrl" />
   		<form:form action="${searchUrl}" method="post" id="searchForm" name="searchForm" modelAttribute="unit_sus_no1">
   			...
   			<input type="hidden" name="wepe_trans_no1" id="wepe_trans_no1" value="0"/>
   			<input type="hidden" name="radio_doc01" id="radio_doc01" value="0"/>
   			<input type="hidden" name="status1" id="status1" value="0"/>
   		</form:form>   
            	
    <c:url value="ApprovedWEPELinkTransUrl" var="appUrl" />
	<form:form action="${appUrl}" method="post" id="appForm" name="appForm" modelAttribute="appid">
		...
		<input type="hidden" name="wepe_trans_no2" id="wepe_trans_no2" value="0"/>
		<input type="hidden" name="radio_doc02" id="radio_doc02" value="0"/>
		<input type="hidden" name="status2" id="status2" value="0"/>
	</form:form> 
	
	
	<c:url value="rejectWEPELinkTransUrl" var="rejectUrl" />
	<form:form action="${rejectUrl}" method="post" id="rejectForm" name="rejectForm" modelAttribute="rejectid">
		...
		<input type="hidden" name="wepe_trans_no3" id="wepe_trans_no3" value="0"/>
		<input type="hidden" name="radio_doc03" id="radio_doc03" value="0"/>
		<input type="hidden" name="status3" id="status3" value="0"/>
	</form:form> 
	
	<c:url value="delinkapprvWEPELinkTransUrl" var="delinkapprvUrl" />
	<form:form action="${delinkapprvUrl}" method="post" id="delinkapprvForm" name="delinkapprvForm" modelAttribute="delinkapprvid">
		...
		<input type="hidden" name="wepe_trans_no5" id="wepe_trans_no5" value="0"/>
		<input type="hidden" name="radio_doc05" id="radio_doc05" value="0"/>
		<input type="hidden" name="status5" id="status5" value="0"/>
		
	</form:form> 
	
	<c:url value="delinkrejectWEPELinkTransUrl" var="delinkrejectUrl" />
	<form:form action="${delinkrejectUrl}" method="post" id="delinkrejectForm" name="delinkrejectForm" modelAttribute="delinkrejid">
		...
		<input type="hidden" name="wepe_trans_no6" id="wepe_trans_no6" value="0"/>
...
$(document).ready(function() {
	...
	if("${roleAccess}" === "Line_dte"){
        ...
        $("td[data-column='thAction1']").hide();
		 ...
		 $(".card-header h5 b").text("Search LINK SUS No with WE/PE No Transport");
	...
	if('${status1}' != "")
	{
		...
		if('${list.size()}' == 0 ){
			...
			$("table#SearchLinkTrans").append("<tr><td colspan='10' style='text-align :center;'>Data Not Available</td></tr>");
		 }
	}
...
function ApprovedTrans(id,sus,wepe){
		...
		$("#unit_sus_no2").val($("#unit_sus_no_trans").val());
		$("#unit_name2").val($("#unit_name").val());
		$("#wepe_trans_no2").val($("#We_pe_no").val());
		$("#status2").val($("#status").val());
		document.getElementById('appForm').submit();
 }
 
 function RejectTrans(id,sus,wepe){
		...
		$("#unit_sus_no3").val($("#unit_sus_no_trans").val());
		$("#unit_name3").val($("#unit_name").val());
		$("#wepe_trans_no3").val($("#We_pe_no").val());
 ...
 function DelinkApprve(id,sus,wepe){
	   ...
	   document.getElementById('sus_no_da').value=sus;
		 ...
		 var radio_doc01 = $("input[name='radio_doc']:checked").val();
			...
			$("#unit_sus_no5").val($("#unit_sus_no_trans").val());
			$("#unit_name5").val($("#unit_name").val());
			$("#wepe_trans_no5").val($("#We_pe_no").val());
...
function DelinkReject(id,sus,wepe){
	   ...
	   document.getElementById('sus_no_dr').value=sus;	
		 ...
		 var radio_doc01 = $("input[name='radio_doc']:checked").val();
			...
			$("#unit_sus_no6").val($("#unit_sus_no_trans").val());
			$("#unit_name6").val($("#unit_name").val());
			$("#wepe_trans_no6").val($("#We_pe_no").val());
