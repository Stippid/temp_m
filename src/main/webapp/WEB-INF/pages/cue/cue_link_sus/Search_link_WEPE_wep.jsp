	...
	function getNameByCode(val) {
		if (val != "" || val != null) {
			...
			$.post("getSearchsusbywepenoDetailsList?" + key + "=" + value, {sus_no : sus}).done(function(j) {
				if (j != "" && j != null){
					document.getElementById("We_pe_no").value = j[0][2];
				} else {
					if ('${unit_sus_no1}' != "") {
						document.getElementById("We_pe_no").value = "";
					}
...
<script>
	...
	<div class="container" align="center">
		<div class="card">
	        		<div class="card-header"><h5><b>Search LINK SUS No with WE/PE No Weapon</b><br>
						...
						<div class="col-md-6">
 							<div class="row form-group">  								
               					...
               					<div class="col-12 col-md-6">
                 					<input  id="unit_sus_no_wep" name="sus_no" style="font-family: 'FontAwesome',Arial;" placeholder="&#xF002; Search" class="form-control" autocomplete="off">
								</div>
 							</div>
 						</div>              					
              			<div class="col-md-6">
              			 	<div class="row form-group"> 
               					<div class="col col-md-6">
                 					<label for="text-input" class=" form-control-label">Unit's Name</label>
               					</div>
               					<div class="col-12 col-md-6">
  					...
  					<div class="col-md-12">	
  						<div class="col-md-6">
  							...
  							<div class="col-md-6">
  									<div class="row form-group">
	                					<div class="col col-md-6">
	                  						<label class=" form-control-label">WE/PE No</label>
	                					</div>
	                					<div class="col-12 col-md-6">
	                  						<input  id="We_pe_no" name="wepe_wep_no" style="font-family: 'FontAwesome',Arial;" placeholder="&#xF002; Search" class="form-control autocomplete" autocomplete="off" >
	                					</div>
	                			</div>
	                			</div>
	                			</div>
...
<div style="width: 20%; padding-left: 1%; display: none;" id="divSerachInput">
			...
			<table id="SearchLinkWep" class="table no-margin table-striped  table-hover  table-bordered report_print" >
				<thead>
					<tr>
						...
						<th>WE/PE No</th>
						<th>Modification</th>
						<th id="thActionId"  class='lastCol'>Action</th>
						</tr>
				</thead>
				<tbody>
					<c:forEach var="item" items="${list}" varStatus="num" >
						<tr>
							...
							<td>${item.modifi}</td>
							<td id="thActionId1" class='lastCol' data-column="thActionId1">${item.id}</td>
						</tr>	
					</c:forEach>
				</tbody>
			</table>
	...
	<div id="divShow" style="display: block;"></div>
 	...
 	<c:url value="searchLinkWEPEWeap1" var="searchUrl" />
 		<form:form action="${searchUrl}" method="post" id="searchForm" name="searchForm" modelAttribute="unit_sus_no1">
 			...
 			<input type="hidden" name="wepe_wep_no1" id="wepe_wep_no1" value="0"/>
 			<input type="hidden" name="radio_doc01" id="radio_doc01" value="0"/>
 			<input type="hidden" name="status1" id="status1" value="0"/>
 		</form:form>
 		     
	<c:url value="ApprovedWEPELinkWepUrl" var="appUrl" />
	<form:form action="${appUrl}" method="post" id="appForm" name="appForm" modelAttribute="appid">
		...
		<input type="hidden" name="wepe_wep_no2" id="wepe_wep_no2" value="0"/>
	...
	<c:url value="rejectWEPELinkWepUrl" var="rejectUrl" />
	<form:form action="${rejectUrl}" method="post" id="rejectForm" name="rejectForm" modelAttribute="rejectid">
		...
		<input type="hidden" name="radio_doc06" id="radio_doc06" value="0"/>
		<input type="hidden" name="unit_sus_no6" id="unit_sus_no6" value="0"/>
		<input type="hidden" name="unit_name6" id="unit_name6" value="0"/>
		<input type="hidden" name="wepe_wep_no6" id="wepe_wep_no6" value="0"/>
		<input type="hidden" name="status6" id="status6" value="0"/>
	</form:form> 
	
	<c:url value="delinkapprvWEPELinkWeapUrl" var="delinkapprvUrl" />
	<form:form action="${delinkapprvUrl}" method="post" id="delinkapprvForm" name="delinkapprvForm" modelAttribute="delinkapprvid">
		...
		<input type="hidden" name="sus_no_da" id="sus_no_da" value=""/>
		<input type="hidden" name="wepe_no_da" id="wepe_no_da" value=""/>
	...
	<c:url value="delinkrejectWEPELinkWeapUrl" var="delinkrejectUrl" />
...
$(document).ready(function() {
	...
	if("${roleAccess}" === "Line_dte"){
		...
		$("#thActionId").hide(); 
        $("td[data-column='thActionId1']").hide();
	}else {
		 $(".card-header h5 b").text("Search LINK SUS No with WE/PE No Weapon");
		 $("#status").prop("disabled", false);
		 $("div#StatusDiv").show();
		 
		 $("#thActionId").show(); // Hide the column header
	     $("td[data-column='thActionId1']").show();
	}
	...
	if('${status1}' != "")
	{
		...
		if('${list.size()}' == 0 ){
			...
			$("table#SearchLinkWep").append("<tr><td colspan='6' style='text-align :center;'>Data Not Available</td></tr>");
		 }
...
function ApprovedWep(id,sus,wepe){
		...
		$("#wepe_wep_no2").val($("#We_pe_no").val());
		$("#status2").val($("#status").val());
	
		document.getElementById('appForm').submit();
 }
 
 function RejectWep(id,sus,wepe){
 ...
 function DelinkApprve(id,sus,wepe){
	   ...
	   document.getElementById('sus_no_da').value=sus;
	   document.getElementById('wepe_no_da').value=wepe;
	   var radio_doc01 = $("input[name='radio_doc']:checked").val();
 ...
 var newWin;
 function UpdateWep(id ,wepe_no_e){	
 ...
 function DeleteWep(id,sus,wepe){
