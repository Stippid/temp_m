...
$(document).ready(function () {
    ...
    var sus_no_t = "${sus_no_t}";
    $("input#unit_sus_no_trans").val(sus_no_t);
    var wepe_pers_no_t = "${wepe_pers_no_t}";
    $("input#We_pe_no").val(wepe_pers_no_t);
    var unit_name_t = "${unit_name_t}";
    $("input#unit_name").val(unit_name_t);
    var r_d = "${radio_doc_t}";	
    if(r_d != ""){
    	getOrbatDetails(sus_no_t);
...
<script>
 ...
 if('${roleAccess}' != "Unit")
	{           
	  ...
	  wepetext1.autocomplete({
	      ...
	      change: function(event, ui) {
	    	 if (ui.item) {   	        	  
	        	  ...
	        	  document.getElementById("unit_sus_no_trans").value="";
	        	  wepetext1.val("");	        	  
	        	  wepetext1.focus();
	        	  return false;	             
	          }   	         
	 ...
	 var wepetext=$("#unit_sus_no_trans");
	  ...
	  wepetext.autocomplete({
	      source: function( request, response ) {
	        ...
	        data: {sus_no : document.getElementById('unit_sus_no_trans').value },
	          success: function( data ) {
	            //response( data );
	            if(data.length > 1){
	        	  var susval = [];
	  	...
	  	$("#unit_sus_no_trans").attr("readonly","readonly");
 ...
 {	
 	if(val != "" || val != null) {		
	    ...
	    $.post("getCUEUnitDetailsList?"+key+"="+value, {unitName : unit_name }).done(function(j) {
	    	if(j!="" && j!=null){
 				document.getElementById("unit_sus_no_trans").value=j[0].sus_no;
...
<form:form name="link_WEPE_Trans" id="link_WEPE_Trans"
	action="link_WEPE_TransAct" method='POST'
	commandName="link_WEPE_TransCmd">

	<div class="container" align="center">
		<div class="card">
			<div class="card-header">
				<h5>
					<b>LINK SUS No with WE/PE No</b><br>
			...
			<div class="card-body card-block cue_text">
						...
						<div class="row form-group">
							<div class="col col-md-6">
								<label for="text-input" class=" form-control-label">SUS
									No</label> <strong style="color: red;">*</strong>
							</div>
							<div class="col-12 col-md-6">
								<input id="unit_sus_no_trans" name="sus_no" maxlength="8"
									value="${unit_sus_no1}"
					...
					<div class="col-md-6">
						<div class="row form-group">
							...
							<div class="col-12 col-md-8">
								<input id="unit_name" name="unit_name" value="${unit_name1}"
									style="font-family: 'FontAwesome', Arial;"
									placeholder="&#xF002; Search" class="form-control"
									autocomplete="off"> <input type="hidden"
									id="id_check_trans_hidden" name="id_check_trans_hidden">
								<input type="hidden" id="id_trans_foot_hidden"
									name="id_trans_foot_hidden">
							</div>
						</div>
					</div>
				</div>
				<div class="col-md-12">
					<div class="col-md-6">
						<div class="row form-group">
							<div class="col col-md-6">
								<label class=" form-control-label">Type of Document </label> <strong
									style="color: red;">*</strong>
							</div>
							<div class="col-12 col-md-6">
								<div class="form-check-inline form-check">
									<label for="inline-radio1" class="form-check-label "> <input
										type="radio" id="radio_doc1" name="radio_doc" value="WE"
										class="form-check-input">WE
									</label>&nbsp;&nbsp;&nbsp; <label for="inline-radio2"
										class="form-check-label "> <input type="radio"
										id="radio_doc2" name="radio_doc" value="PE"
										class="form-check-input">PE
									</label>&nbsp;&nbsp;&nbsp; <label for="inline-radio3"
										class="form-check-label "> <input type="radio"
										id="radio_doc3" name="radio_doc" value="FE"
										class="form-check-input">FE
									</label>&nbsp;&nbsp;&nbsp; <label for="inline-radio4"
										class="form-check-label "> <input type="radio"
										id="radio_doc4" name="radio_doc" value="GSL"
					...
					<div class="col-md-6">
						<div class="row form-group">
							...
							<div class="col-12 col-md-8">
								<input id="We_pe_no" name="wepe_trans_no" maxlength="100"
									style="font-family: 'FontAwesome', Arial;"
									placeholder="&#xF002; Search" class="form-control autocomplete"
									autocomplete="off">
							</div>
						</div>
					</div>
				</div>
				<div class="col-sm-12" id="trans_orbat_div" style="display: none;" align="center">
					<h3 class="heading_content_inner">Orbat Details</h3>
					<table id="orbatLinkTrans" class="table no-margin table-striped  table-hover  table-bordered">
						<thead>
							<tr>
				...
				<div class="col-md-12" style="display: block;">
					<c:if test="${fn:length(table_view_trans)>0}">
						...
						<table id="tblData" class="table no-margin table-striped  table-hover  table-bordered">
							...
							<tbody>
								<c:forEach var="item" items="${table_view_trans}"
									varStatus="num">
									<tr>

										<td class="tableheadSer"><c:out value="${num.index+1}" />
										</td>
										<td>${item.modification}</td>
										<td><input type="checkbox"
											onclick="viewDetails_Trans('${item.modification}')"
											name="idCheck_trans" id="idCheck_trans"
											value="${item.modification}"></td>
										<td><input
											onclick="moreModitrans('${item.modification},${num.index+1}');"
											type="button" value="View More Details"></td>

									</tr>
									<tr>
										<td colspan="4">
											<table id="tblDatai${num.index+1}" style="display: none;"
												class="table no-margin table-striped  table-hover  table-bordered">

												<thead>
													<tr>
														<th>MCT No</th>
														<th>Veh Nomclature</th>
				...
				<div class="col-md-12" style="display: block;">
					<c:if test="${fn:length(table_view_trans_footnotes)>0}">
						<h3 class="heading_content_inner">General/In Lieu Notes
							Details</h3>
						<table id="tblData_foot"
							class="table no-margin table-striped  table-hover  table-bordered">
							...
							<thead>
								<tr>
									<th class="tableheadSer">Ser No</th>
									<th>MCT No</th>
									<th>Std. Nomclature</th>
									<th>Amt of Incr/Decr</th>
									<th>In Lieu MCT No</th>
									<th>In Lieu Amount of Inc/Dec</th>
									<th>Condition</th>
									<th>View</th>
							</thead>

							<tbody>
								<c:forEach var="item" items="${table_view_trans_footnotes}"
									varStatus="num">
									<tr>
										...
										<td>${item.inlieu_amt}</td>
										<td>${item.condition}</td>
										<td><input type="checkbox"
											onclick="viewDetailsTrnasfootnotes(${item.id})"
											name="idCheck_trans_foot" id="idCheck_trans_foot"
											value="${item.id}"></td>
									</tr>
								</c:forEach>
			...
			<div class="card-footer" align="center">
				<a href="link_WEPE_sus_Trans" class="btn btn-success btn-sm">
					Clear </a> <input type="submit" class="btn btn-primary btn-sm"
					value="Save" onclick="return isvalidData();"> <i
...
<div class="col s12" style="display: none;" id="divPrint">
	...
	<div class="watermarked" data-watermark="" id="divwatermark"
		style="display: block;">
		<span id="ip"></span>
		<table id="SearchLinkTrans"
			class="table no-margin table-striped  table-hover  table-bordered report_print">
			<thead>
				<tr>
					<th class="tableheadSer">Ser No</th>
					<th class="tableheadSer">SUS No</th>
					<th>Unit Name</th>
					<th>MISO WE/PE No</th>
					<th>Modification</th>
			...
			<tbody>
				<c:forEach var="item" items="${list}" varStatus="num">
					<tr>
						...
						<td class="tableheadSer">${item.sus_no}</td>
						<td>${item.unit_name}</td>
						<td>${item.wepe_trans_no}</td>
						<td>${item.modifi}</td>
						<td class="tableheadSer">
							<c:if test="${item.footnotes_count != 0}">
								<i class='action_icons action_view' onclick="return viewFootnotesDetails('${item.sus_no}','${item.wepe_trans_no}')"></i>
							</c:if>
						</td>
...
<c:url value="linkWEPETrans1" var="searchUrl" />
<form:form action="${searchUrl}" method="post" id="searchForm"
	...
	<input type="hidden" name="wepe_trans_no1" id="wepe_trans_no1"
		value="0" />
	<input type="hidden" name="status1" id="status1" value="0" />
	<input type="hidden" name="id_check_trans_hidden1"
		id="id_check_trans_hidden1" value="0" />
	<input type="hidden" name="id_trans_foot_hidden1"
		id="id_trans_foot_hidden1" value="0" />
</form:form>
<c:url value="updateWEPELinkTransUrl" var="updateUrl" />
...
<c:url value="getMoreModFootCmdTran" var="moreUrl" />
<form:form action="${moreUrl}" method="post" id="moreForm"
	name="moreForm" modelAttribute="wepe_id">
...
function viewFootnotesDetails(sus_no,wepe_no){
	...
	$.post("getFootNotesTransDetails?"+key+"="+value,{sus_no:sus_no,wepe_no:wepe_no},function(j) {
		var markup = "";
		if(j.length > 0){
			for(var i=0;i<j.length;i++){
				markup += "<tr align='center'><td>"+j[i].std_nomclature+"</td><td>"+j[i].we_pe_no+"</td><td>"+j[i].mct_no+"</td><td>"+j[i].amt_inc_dec+"</td><td>"+j[i].condition+"</td></tr>";
			}
		}else{
...
function closeModal(){
	...
	function Search(){
		...
		$("#unit_sus_no1").val($("#unit_sus_no_trans").val());
		$("#unit_name1").val($("#unit_name").val());
		$("#wepe_trans_no1").val($("#We_pe_no").val());
		$("#status1").val($("#status").val());
		$("#id_check_trans_hidden1").val($("#id_check_trans_hidden").val());
		$("#id_trans_foot_hidden1").val($("#id_trans_foot_hidden").val());
...
<script>
$(document).ready(function() {
	if('${unit_sus_no1}' != "" || '${wepe_trans_no1}' != "")
	{ 		
		if('${radio_doc01}' != ""){
		...
		$("input#id_check_trans_hidden").val("${id_check_hidden_txt}");
		$("input#id_trans_foot_hidden").val("${id_check_foot_hidden_txt}");
		$("#unit_sus_no_trans").val('${unit_sus_no1}');
		$("#We_pe_no").val('${wepe_trans_no1}');
		...
		if('${list.size()}' == 0 ){
			...
			$("#SearchLinkTrans").append("<tr><td colspan='5' style='text-align :center;'>Data Not Available</td></tr>");
			$("#divPrint").show();
			$("div#divwatermark").val('').addClass('watermarked'); 
			watermarkreport();
		 }
	...
	$("#searchInput").on("keyup", function() {
			...
			$("#SearchLinkTrans tbody tr").filter(function() { 
	...
	try{
		...
		if(m.includes("Updated+Successfully")){
			window.opener.getRefreshReport('link_trans',m);
...
function getOrbatDetails(sus)
{
	...
	function drawTable1(data) {
		$("div#trans_orbat_div").show();
		if(data.length == 0)
		{
			var row = $("<tr />")
			$("#orbatLinkTrans").append(row);
...
function moreDetailsTrans(wepe_no)
{
	 var sus = $("input#unit_sus_no_trans").val();
...
{
	if($("input#unit_sus_no_trans").val()==""){
		alert("Please Enter Unit Sus No");
		$("input#unit_sus_no_trans").focus();
		return false;
	}
	...
	/* function check_location()
	{
		  ...
		  $.ajaxSetup({
		    ...
		    var unit_sus_no =$("#unit_sus_no_trans").val(); 
			var we_pe = document.getElementById("We_pe_no").value;
			

			$.post("check_locationWeapTrans?" + key + "=" + value, { wepe_trans_no: we_pe,unit_sus_no: unit_sus_no }, function(j) {
			    var length= j.length;
 ...
 function moreModitrans(mod)
{
	...
	function drawTable1(data) {
	 ...
	 else
	 {
		 ...
		 var tab = $("#tblDatai"+mod_i[1]+" > tbody ");
		 	tab.empty(); 
	 }
}
...
function deleteData(id,sus,wepe){
	     ...
	     $.post("deleteWEPELinkTransUrlAdd?"+key+"="+value, {deleteid : id,sus : sus , wepe : wepe}).done(function(j) {
	    	 alert(j);
	 		Search();
	      }).fail(function(xhr, textStatus, errorThrown) { }); 
}
