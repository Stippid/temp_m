...
$(document).ready(function () {
    ...
    var sus_no_w = "${sus_no_w}";
    $("input#unit_sus_no_wep").val(sus_no_w);
    var wepe_pers_no_w = "${wepe_pers_no_w}";
    $("input#We_pe_no").val(wepe_pers_no_w);
    var unit_name_w = "${unit_name_w}";
    $("input#unit_name").val(unit_name_w);
    var r_d = "${radio_doc_w}";	
    if(r_d != ""){
    	getOrbatDetails(sus_no_w);
	 ...
	 var wepetext1=$("#unit_name");
	  ...
	  wepetext.autocomplete({
	      source: function( request, response ) {
	        ...
	        data: {sus_no : document.getElementById('unit_sus_no_wep').value },
	        //data: {sus_no:wepetext.val()},
	          success: function( data ) {
	            //response( data );
	            if(data.length > 1){
	        	  var susval = [];
	  	...
	  	$("#unit_sus_no_wep").attr("readonly","readonly");
...
<form:form name="link_WEPE_Wep" id="link_WEPE_Wep" action="link_WEPE_WepAct" method='POST' commandName="link_WEPE_WepCmd">
	<div class="container" align="center">
					...
					<div class="col-md-6">
 						<div class="row form-group">  								
           					...
           					<div class="col-12 col-md-6">
                 				<input  id="unit_sus_no_wep" name="sus_no" value="${unit_sus_no1}" maxlength="8" style="font-family: 'FontAwesome',Arial;" placeholder="&#xF002; Search" class="form-control" autocomplete="off">
							</div>
 						</div>
 					</div>              					
              		<div class="col-md-6">
              		 	<div class="row form-group"> 
               				...
               				<div class="col-12 col-md-6">
                 				...
                 				<input  id="unit_name" name="unit_name" value="${unit_name1}" style="font-family: 'FontAwesome',Arial;" placeholder="&#xF002; Search" class="form-control"  autocomplete="off">
							</div>
 						</div> 
  					</div>
  				</div>
 				<div class="col-md-12">	
 					<div class="col-md-6">
  					...
  					<div class="col-md-6">
  						<div class="row form-group">
	                		<div class="col col-md-6">
	                			<label class=" form-control-label">WE/PE No </label> <strong style="color: red;">*</strong>
	                		</div>
	                		<div class="col-12 col-md-6">
	                			<input  id="We_pe_no" name="wepe_weapon_no" maxlength="100" style="font-family: 'FontAwesome',Arial;" placeholder="&#xF002; Search" class="form-control autocomplete" autocomplete="off" >
	                		</div>
	                	</div>
	                </div>
  				</div>
            	<div class="col-sm-12" id="weap_orbat_div" style="display: none;" align="center" >
            		<h3 class="heading_content_inner">Orbat Details</h3>
					<table id="orbatLinkWep" class="table no-margin table-striped  table-hover  table-bordered" >
						<thead>
							<tr>
					...
					<c:if test="${fn:length(table_view_wep)>0}"><h3 class="heading_content_inner">Modification Details</h3>
						<table id="tblData" class="table no-margin table-striped  table-hover  table-bordered" >
							<thead style="background-color: #9c27b0; color: #fff;">
								<tr><th>Ser No</th><th>Mod</th><th>View</th><th></th>
							</thead>
							<tbody>
									...
									<tr >
										 <td colspan="4" >
											<table id="tblDatai${num.index+1}" style="display: none;" class="table no-margin table-striped  table-hover  table-bordered" >
												<thead>
													<tr><th>WE/PE  No</th><th>Nomenclature</th><th>PRF Group</th><th>Amt of Incr/Decr</th><th>Scenario</th><th>Loc/ Fmn/ Unit</th></tr>
												</thead>
												<tbody>
												</tbody>
											</table>
				...
				<div  class="col-md-12"  style="display:block;"> 
					<c:if test="${fn:length(table_view_wep_footnotes)>0}"><h3 class="heading_content_inner">General Notes Details</h3>
						<table id="tblData_foot" class="table no-margin table-striped  table-hover  table-bordered"  >
							<thead >
								<tr><th>Sr No</th><th>Nomenclature</th><th>PRF Group</th><th>Scenario</th><th>Loc/ Fmn/ Unit</th><th>Amt of Incr/Decr</th><th>Condition</th><th>View</th>
							</thead>
							<tbody>
							<c:forEach var="item" items="${table_view_wep_footnotes}" varStatus="num" >
		 						<tr>
		 							<td>
		 								<c:out value="${num.index+1}" /> 
		 							</td>
		 							<td>
		...
		<div class="card-footer" align="center">
			<a href="link_WEPE_sus_Wep" class="btn btn-success btn-sm">  Clear </a>
...
<input type="hidden" name="location1" id="location1" value=""/>
			<input type="hidden" name="input_location" id="input_location" value=""/>
</form:form>
...
<div class="col s12" style="display: none;" id="divPrint" align="center">
	<div id="divShow" style="display: block;"></div>
	<div  class="watermarked" data-watermark="" id="divwatermark" style="display: block;">
		<span id="ip"></span>
		<table  id="SearchLinkWep" class="table no-margin table-striped  table-hover  table-bordered report_print" >
			<thead >
				<tr >
			...
			<tbody>
				<c:forEach var="item" items="${list}" varStatus="num" >
					<tr >
						...
						<td>${item.wepe_weapon_no}</td>
						<td>${item.modifi}</td>
						<td class="tableheadSer">
							<c:if test="${item.footnotes_count != 0}">
								<i class='action_icons action_view' onclick="return viewFootnotesDetails('${item.sus_no}','${item.wepe_weapon_no}')"></i>
							</c:if>
						</td>
...
<div class="col s12" style="display: none;" id="divPrint1" align="center"> 
	<div id="divShow" style="display: block;"></div>
	<div  class="watermarked" data-watermark="" id="divwatermark1" style="display: block;">
		<span id="ip"></span>
		<table  id="SearchLinkWep1" class="table no-margin table-striped  table-hover  table-bordered report_print" >
			<thead >
				<tr>
					<th class="tableheadSer">Ser No</th>
					<th>Item Code</th>
			...
			<tbody>
				<c:forEach var="item1" items="${flist}" varStatus="num" >
					<tr>
						<td class="tableheadSer">${num.index+1}</td>
...
<!--reject Modal view General FootNotes  26-10-2023-->
...
<c:url value="linkWEPEWeap1" var="searchUrl" />
<form:form action="${searchUrl}" method="post" id="searchForm" name="searchForm" modelAttribute="unit_sus_no1">
	...
	<input type="hidden" name="wepe_weapon_no1" id="wepe_weapon_no1" value="0"/>
	<input type="hidden" name="status1" id="status1" value="0"/>
	<input type="hidden" name="id_check_wep_hidden1" id="id_check_wep_hidden1" value="0"/>
    <input type="hidden" name="id_wep_foot_hidden1" id="id_wep_foot_hidden1" value="0"/>
</form:form>    
<c:url value="getMoreModFootCmdWep" var="moreUrl" />
...
<c:url value="updateWEPELinkWepUrl" var="updateUrl" />
...
function viewFootnotesDetails(sus_no,wepe_no){
	...
	$.post("getFootNotesWeaponDetails?"+key+"="+value,{sus_no:sus_no,wepe_no:wepe_no},function(j) {
		var markup = "";
		if(j.length > 0){
			for(var i=0;i<j.length;i++){
				markup += "<tr align='center'><td>"+j[i].std_nomclature+"</td><td>"+j[i].we_pe_no+"</td><td>"+j[i].item_code+"</td><td>"+j[i].amt_inc_dec+"</td><td>"+j[i].condition+"</td></tr>";
			}
		}else{
...
function Search(){	
	...
	$("#unit_sus_no1").val($("#unit_sus_no_wep").val());
	$("#unit_name1").val($("#unit_name").val());
	$("#wepe_weapon_no1").val($("#We_pe_no").val());
	$("#status1").val($("#status_weap").val());
	$("#id_check_wep_hidden1").val($("#id_check_wep_hidden").val());
	$("#id_wep_foot_hidden1").val($("#id_wep_foot_hidden").val());
...
function deleteData(id,sus,wepe){
	...
	$.post("deleteWEPELinkWeapUrlAdd?"+key+"="+value, {deleteid : id,sus : sus , wepe : wepe}).done(function(j) {
    	alert(j);
		Search();
	}).fail(function(xhr, textStatus, errorThrown) { }); 
...
<script>
$(document).ready(function() {
	if('${unit_sus_no1}' != "" || '${wepe_weapon_no1}' != "")
	{
		if('${radio_doc01}' != ""){
		...
		$("input#id_check_wep_hidden").val("${id_check_hidden_txt}");
		$("input#id_wep_foot_hidden").val("${id_check_foot_hidden_txt}");
		$("#unit_sus_no_wep").val('${unit_sus_no1}');
		$("#We_pe_no").val('${wepe_weapon_no1}');
		$("#status_weap").val('${status1}');
		getNameByCode('${unit_sus_no1}');
		...
		if('${list.size()}' == 0 ){
			...
			$("#SearchLinkWep").append("<tr><td colspan='5' style='text-align :center;'>Data Not Available</td></tr>");
			$("#divPrint").show();
			$("div#divwatermark").val('').addClass('watermarked'); 
			watermarkreport();
		 }
		
		/// displaying footnotes data is started here  18 09 2020
		if('${flist.size()}' > 0 ){
			$("#divPrint1").show();
			$("div#divwatermark1").val('').addClass('watermarked'); 
			watermarkreport();
			$("div#divSerachInput").show();
			document.getElementById("printId").disabled = false;
		}
		if('${flist.size()}' == 0 ){
			$("div#divSerachInput").hide(); 
			document.getElementById("printId").disabled = true;
			$("#SearchLinkWep1").append("<tr><td colspan='5' style='text-align :center;'>No Footnote data for this Unit</td></tr>");
			$("#divPrint1").show();
			$("div#divwatermark1").val('').addClass('watermarked'); 
			watermarkreport();
		}
		/// displaying footnotes data is ended here  18 09 2020	
	}
	$("#searchInput").on("keyup", function() {
		var value = $(this).val().toLowerCase();
		$("#SearchLinkWep tbody tr").filter(function() { 
	...
	try{
		if(window.location.href.includes("msg="))
		{
			...
			if(m.includes("Updated+Successfully")){
				window.opener.getRefreshReport('link_weap',m);
...
function getOrbatDetails(sus)
{
	...
	function drawTable1(data) {
		$("div#weap_orbat_div").show();
		if(data.length == 0)
		{
			var row = $("<tr />")
			$("#orbatLinkWep").append(row);
...
function moreDetailsWep(wepe_no)
{
	var sus = $("input#unit_sus_no_wep").val();
...
{
	if($("input#unit_sus_no_wep").val()==""){
		alert("Please Enter Unit Sus  No");
		$("input#unit_sus_no_wep").focus();
		return false;
	}
...
/* function checkLocation() {
    ...
    var unit_sus_no_wep =$("#unit_sus_no_wep").val(); 
	if(we_pe!="" && unit_sus_no_wep !="" )
		{
	$.post("check_locationWeap?" + key + "=" + value, { wepe_pers_no: we_pe,unit_sus_no_wep:unit_sus_no_wep }, function(j) {
	    var length=j.length;
