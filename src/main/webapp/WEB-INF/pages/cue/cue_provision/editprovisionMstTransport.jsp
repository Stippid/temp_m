...
<form:form name="provision_policyTransForm" id="provision_policyTransForm" action="editprovisionMstTransAct" method='POST' commandName="EditprovisionMstTransCmd" > 
	    		...
	    		<div class="card">
	          		...
	          		<div class="card-body card-block cue_text">
	          			...
	          			<div class="col-md-12">	
	  						...
	  						<div class="row form-group">	           					
             					...
             					<input id="id" name="id" type="hidden" class="form-control" value="${EditprovisionMstTransCmd.id}"/>
                					<input id="month_cal" name="month_cal" type="number" min="1" max="12" step="1" class="form-control" maxlength="2" value="${EditprovisionMstTransCmd.month_cal}" readonly="readonly"/>
             					</div>
             				</div>
             				</div>	
             				<div class="col-md-6">
	  						<div class="row form-group">	           					
             					...
             					<div class="col-12 col-md-6">
             						<input id="year_cal" name="year_cal" type="number" min="1900" max="2099" step="1" class="form-control" maxlength="4"  value="${EditprovisionMstTransCmd.year_cal}" readonly="readonly"/>
               					</div>
             				</div>
             				</div>	           					
	  					</div>	
	  					...
	  					<div class="col-md-12">	              					
	              			...
	              			<div class="row form-group">              			 	
               					...
               					<div class="col-12 col-md-6">
                 					<input id="letter_no" name="letter_no" class="form-control" value="${EditprovisionMstTransCmd.letter_no}">
								</div>
							</div>	 							
	  						</div>
	  						<div class="col-md-6">	
	              			<div class="row form-group">              			 	
               					...
               					<div class="col-12 col-md-6">
                 					<input id="letter_date" name="letter_date" class="form-control" placeholder="dd-MM-yyyy" value="${EditprovisionMstTransCmd.letter_date}">
								</div>
							</div>	 							
	  						</div>
	  					</div>
	          		...
	          		<div class="col-md-12"><span class="line"></span></div>
	          			...
	          			<div class="col-md-12">	              					
	              			...
	              			<div class="row form-group">              			 	
               					...
               					<div class="col-12 col-md-6">
                 					<input  id="we_pe_no" name="we_pe_no" class="form-control" autocomplete="off"  value="${EditprovisionMstTransCmd.we_pe_no}" readonly="readonly">
                 					<input type="hidden" id="we_type" name="we_type" class="form-control" >
                 					<input type="hidden" id="sponsor_dire" name="sponsor_dire" class="form-control" >
	              			...
	              			<div class="row form-group">              			 	
               					...
               					<div class="col-12 col-md-6">
                 					<input id="unit_type" name="unit_type" class="form-control" maxlength="150" autocomplete="off" value="${EditprovisionMstTransCmd.unit_type}" >
								</div>
							</div>	 							
	  						</div>
	  					</div>	
	            		...
	            		<div class="col-md-12">	              					
             					...
             					<div class="col-12 col-md-6">
               						...
               						<option value="">--Select--</option>
                 						<c:forEach var="item" items="${getForceTypeList}" varStatus="num" >
           									<option value="${item[1]}" <c:if test="${item[1] eq EditprovisionMstTransCmd.force_type}">selected="selected"</c:if>> ${item[1]}</option>
           								</c:forEach>
               						</select>
             					</div>
             				</div>
	  						...
	  						<div class="row form-group">           					
             					...
             					<div class="col-12 col-md-6">
               						<input id="no_of_unitsst" name="no_of_units" maxlength="4" class="form-control" onkeypress="return validateNumber(event, this);" value="${EditprovisionMstTransCmd.no_of_units}">
             					</div>
             				</div>
             				</div>   
					...
					<div class="" id="divModId" style="display: none;" align="center">
						...
						<div class="col-md-12">
			              	<table id="tblData" class="table no-margin table-striped  table-hover  table-bordered" width="100%">
								<thead>
									<tr><th style="width: 6%;text-align: center">Ser No</th><th style="width: 50px;"></th><th>Modification</th><th>No of Units</th></thead>
								<tbody id="tbodyModId">							 
		  						</tbody>
							</table>
		              	</div>
	              	...
	              	<div class=""  id="divFootnoteId" style="display: none;" align="center">
						...
						<div class="col-md-12">
			              	<table id="tblData1" class="table no-margin table-striped  table-hover  table-bordered" width="100%">
								<thead>
									<tr><th style="width: 6%;text-align: center">Ser No</th><th style="display : none;">Id</th><th style="width: 50px;"></th><th>Condition</th><th style="width: 120px;">MCT No</th><th>Nomenclature</th><th>Scenario</th><th>Loc/ Fmn/ Unit</th><th style="display : none;">Loc/Form/Unit Code</th><th>Amt of Incr/Decr</th><th>No of Units</th></thead>
	              	...
	              	<div class=""  id="divStandardId" style="display: none;">
						<div class="col-md-12">
			              	<table id="tblData1" class="" width="100%">
								<thead>
									<tr style="width: 6%;text-align: center"><th>Ser No</th><th></th><th>Nomenclature</th><th>Authorisation Amt</th></thead>
					...
					<div class="card-footer" align="center">
						<input type="submit" class="btn btn-primary btn-sm" value="Update" onclick="return getAllDetails()">
	              		<a href="searchProvisionMstTransUrl" class="btn btn-danger btn-sm" onClick="javascript:window.close('','_parent','');">  Cancel </a>              		
		            </div> 
...
function bindMod(val)
{
	...
	$.post("getModByTransByWePeNo?"+key+"="+value, {we_pe_no : val}).done(function(j) {
		...
		for(var i=0; i<j.length; i++)
		{
			...
			row += '<td><input type="checkbox" id="checkm'+i+'" value="'+j[i][0]+'" name="checkm" class="form-control" ></td>';
			row += '<td><input type="hidden" id="itemcode_tbl'+j[i][0]+'" value="'+j[i][1]+'" class="form-control"><input id="mod'+j[i][0]+'" class="form-control" value="'+j[i][0]+'" readonly="readonly"></td>';
			row += '<td><input type="hidden" id="amtincdec_tbl'+j[i][0]+'" value="'+j[i][2]+'" class="form-control"><input id="no_of_unitsm'+j[i][0]+'" class="form-control" maxlength=4 onkeypress="return validateNumber(event, this);"></td>';
			row += '</tr>';								
		}					
		...
		getMDFTransBindCheckbox();
     }).fail(function(xhr, textStatus, errorThrown){ }); 
}
function getMDFTransBindCheckbox()
{
	var id = $('input#id').val();
	
	 $.post("getmdfProWepCheck?"+key+"="+value, {id : id,tbl:"CUE_TB_MISO_PROVISION_TRANSPORT_MDFS"}).done(function(j) {
 ...
 function bindFootnote(val)
 {
	 ...
	 $.post("getAttributeFromFootnoteMaster23?"+key+"="+value, {we_pe_no : val, status:"1"}).done(function(j) {
		 var k=0;
			for(var i=0; i<j.length; i++)
			{
				if(j[i].condition != "" && j[i].condition != null && j[i].condition != "null" && j[i].condition != "undefined")
				{
					...
					row += '<td><input type="text" id="item'+j[i].id+'" class="form-control" value="'+setItem+'" readonly="readonly"></td>';
					row += '<td><input id="itemName'+j[i].id+'" class="form-control" value="'+j[i].std_nomclature+'" readonly="readonly"></td>';
					...
					row += '<td><input id="no_of_unitsf'+j[i].id+'" maxlength=4 class="form-control" onkeypress="return validateNumber(event, this);"></td>';
					row += '</tr>';
				}
				else
				{
...
function getBindFootTransCheckbox()
{
	var id = $('input#id').val();	
	
	 $.post("getFootProWepCheck?"+key+"="+value, {id : id,tbl:"CUE_TB_MISO_PROVISION_TRANSPORT_FOOTNOTES"}).done(function(j) {
