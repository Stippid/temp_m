...
<div id ="tdheaderView" style="display: none;"  align="center" class="nrTableHeading">WEAPON AND EQPT DB UE UH</div>
						 ...
						 <div class="nrTableDataDiv">
						 		<table border="1" class="table_collapse report_print" width="100%">
									<thead>
										...
										<th  width="8%">PRF Group</th>
										<th  width="12%">Item Name</th>
										<th  width="8%">Type Holding</th>
										<th  width=2%">UE</th>
										<th  width="2%">UH</th>
										<th  width="2%">Surplus</th>
										<th  width="2%">Defi</th>
										</tr>
									</thead>
									<tbody id="nrTable"  >
										...
										<c:if test="${m_12[0][0] != 'NIL'}"> 
										    <c:forEach var="item" items="${m_12}" varStatus="num">
										        <tr style="font-size: 12px;" id="NRRDO" name="NRRDO">
											        ...
											        <td style="width:8%; text-align:center; ">${item[7]}</td>
											        <td style="width8%">${item[8]}</td>
											        <td style="width:8%">${item[10]}</td>
											        <td style="width:12%">${item[12]}</td>
											        <td style="width:8%">${item[14]}</td>
											        <td style="width:2%;text-align: center;">${item[17]}</td>
											        <td style="width:2%;text-align: center;">${item[18]}</td>
											        <td style="width:2%;text-align: center;">${item[19]}</td>
											        <td style="width:2%;text-align: center;">${item[20]}</td>
											        <% ntotln++; %>
										        </tr>
										    </c:forEach>
										</c:if>
