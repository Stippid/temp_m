     ...
     <div class="container" align="center"id="headerView" style="display: block;"> 
        <div class="card">
				<div class="card-header mms-card-header">
		               <b>COMMAND TYPE WISE HOLDING</b>
		        </div> 
		   </div>
      </div>
  </div>
  
<div id ="tdheaderView" style="display: none;"  align="center" class="nrTableHeading">COMMAND TYPE WISE HOLDING</div>
...
<div  class="watermarked" data-watermark="" id="divwatermark">
				 ...
				 <div class="nrTableDataDiv">			
					<table border="1" class="table_collapse report_print"
						...
						<tr>
										<th   width="10%" rowspan='2' title='PRF Group'>PRF Group</th>
			                  			<th   width="5%" rowspan='2' title='Census No'>Census No</th>
			                  			<th   width="9%" rowspan='2' title='Item Nomenclature'>Nomenclature</th>
			                  			<th   width="9%" rowspan='2' title='Type of Holding'>Type Holding</th>
										<th   width="2%" COLSPAN='2' title='Southern Command'>SC</th>
										<th   width="2%" COLSPAN='2' title='Eastern Command'>EC</th>
										<th   width="2%" COLSPAN='2' title='Western Command'>WC</th>
										<th   width="2%" COLSPAN='2' title='Central Command'>CC</th>
										<th   width="2%" COLSPAN='2' title='Northern Command'>NC</th>
										<th   width="2%" COLSPAN='2' title='ARTRAC'>ARTRAC</th>
										<th   width="2%" COLSPAN='2' title='Andaman & Nicobar Command'>ANC</th>
										<th   width="2%" COLSPAN='2' title='South Western Command'>SWC</th>
										<th   width="2%" COLSPAN='2' title='UN Mission'>UN</th>
										<th   width="2%" COLSPAN='2' title='SFC'>SFC</th>
										<th   width="2%" COLSPAN='2' title='Ministry of Defence'>MOD</th>
									   <tr>
			                  			<th   width="1%">UE</th>
			                  			<th   width="1%">UH</th>
			                  			<th   width="1%">UE</th>
										...
										<c:if test="${m_12[0][0] != 'NIL'}"> 
										    <c:forEach var="item" items="${m_12}" varStatus="num">
										        <tr style="font-size: 12px; id="NRRDO" name="NRRDO">
										            <td width="10%" style="text-align: left;">${item[0]}</td>
										            <td style="width: 5%">${item[1]}</td>
										            <td style="width: 9%;text-align: left">${item[2]}</td>
										            <td style="width: 9%">${item[3]}</td>
											        <td style="text-align: center;width: 1%">${item[4]}</td>
											        <td style="width: 1%">${item[5]}</td>
											        <td style="width: 1%">${item[6]}</td>
											        <td style="width: 1%">${item[7]}</td>
											        <td style="width: 1%">${item[8]}</td>
											        <td style="text-align: center;width: 1%">${item[9]}</td>
											        <td style="text-align: center;width: 1%">${item[10]}</td>
										            <td style="width: 1%">${item[11]}</td>
										            <td style="width: 1%">${item[12]}</td>
										            <td style="width: 1%">${item[13]}</td>
											        <td style="text-align: center;width: 1%">${item[14]}</td>
											        <td style="width: 1%">${item[15]}</td>
											        <td style="width: 1%">${item[16]}</td>
											        <td style="width: 1%">${item[17]}</td>
											        <td style="width: 1%">${item[18]}</td>
											        <td style="text-align: center;width: 1%">${item[19]}</td>
											        <td style="text-align: center;width: 1%">${item[20]}</td>
											        <td style="width: 1%">${item[21]}</td>
										            <td style="width: 1%">${item[22]}</td>
										            <td style="width: 1%">${item[23]}</td>
											        <td style="text-align: center;width: 1%">${item[24]}</td>
											        <td style="width: 1%">${item[25]}</td>
											      <% ntotln++; %>
										        </tr>
										    </c:forEach>
										</c:if>
