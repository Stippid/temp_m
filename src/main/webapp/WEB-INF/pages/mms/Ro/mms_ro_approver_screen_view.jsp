...
<body class="mmsbg">
<% int ntotln=0; %>
<div id="nrWaitLoader" style="display:none;"><center>
			  ...
			  <div class="card-body card-block">
			       <div class="col-md-12 row form-group">		
						...
						<div class="col-md-4">
							...
							<input type="hidden" id="ro_type_lb" name="ro_type_lb">
							<input type="hidden" id="ro_for_lb" name="ro_for_lb">
						</div>
...
<div id ="tdheaderView" style="display: none;" align="center" class="nrTableHeading"> <b>RO APPROVER SCREEN</b></div>
<div class="card" id="re_tb" style="background: transparent;">	 
	                      ...
	                      <div class="nrTableDataDiv">
							<table border="1" class="report_print" width="100%">
	                        			<thead style="text-align: center;">
	                          			<tr>
										<th width="17%">RO No with Date</th>
									    <th width="5%">RO Type</th>
										<th width="15%">To Unit</th>
										<th width="11%">Issue Against</th>
										<th width="8%">Stock</th>
									    <th width="8%">Type of Issue</th>
										<th width="17%">PRF Group</th>
										<th width="5%">Qty</th>
									...
									<c:if test="${m_1[0][0] != 'NIL'}"> 
									     <c:forEach var="item" items="${m_1}" varStatus="num">
									          <tr class='nrTableLineData' id="NRRDO" name="NRRDO">
									              <td style="width: 17%;"><input class="nrCheckBox"  type="checkbox" id="NRRDOO${item[0]}" name="${item[8]}:${item[9]}:${item[6]}" onclick="setid('${item[0]}','${item[6]}','${item[8]}','${item[7]}','${item[9]}')">&nbsp;${item[0]}-${item[12]}</td>
									              <td style='text-align:center;'width="5%">${item[1]}</td>
									              <td style='text-align:left;'width="15%">${item[2]}</td>
									              <td style='text-align:center;'width="11%">${item[3]}</td>
									              <td style='text-align:center;'width="8%">${item[11]}</td>
									              <td style='text-align:center;'width="8%">${item[10]}</td>
									              <td style='text-align:left;'width="17%">${item[4]}</td>
									              <td style='text-align:center;'width="5%">${item[5]}</td>
									              <td style='text-align:center;'width="7%">${item[6]}</td>
									              <% ntotln++; %>
									          </tr>
									     </c:forEach>
									</c:if>   
	...
	<div class="card-footer">
      ...
      <input type="button" class="btn btn-primary btn-sm" value="Print Page" id="btn_p" onclick="printDiv();"> 
		...
		<input type="button" class="btn btn-primary btn-sm"id="view_p" value="View / Print RO" onclick="viewPrint();" style="float: right;margin-left: 4px;">
		<input type="button" class="btn btn-danger btn-sm" id="em" value="Sanctioned - Brig EM" onclick="return setAuthApp();" style="background-color: #ff6337;float: right;margin-left: 4px;">
		<input type="button" class="btn btn-success btn-sm"id="a_r" value="Approve RO" onclick="return setUpdateStatus();" style="float: right;">
	</div>
  </div>	
  	</div>
</form:form>

<c:url value="printMMSROUrl" var="printUrl" />
<form:form action="${printUrl}" method="post" id="printForm" name="printForm" modelAttribute="printMMSId" target="result">
	<input type="hidden" name="printMMSId" id="printMMSId"/>
</form:form> 

<c:url value="ROApproverList" var="backUrl" />
...
function setAuthApp(){
	...
	$("#ro_type_lb").val(ro_forVals.join(","));
	var ro_type = $("#ro_type_lb").val();
    ...
    var n1 = b.includes("APPROVED");
    var n2 = b.includes("PENDING-1");
    var n3 = b.includes("CANCELLED");
    var n4 = b.includes("PENDING");
    
    if(ro_no!= null && ro_no !="" && ro_no!="null"){
    	if(n3 == true){
    		alert("Selected Request is already Cancelled. Please Select Only Pending-1 Records.");
			getEqptAppList();
    	}else if(n1 == true){
    		alert("Selected Request is already Approved. Please Select Only Pending-1 Records.");
			getEqptAppList();
    	}else if(n4 == true){
    		alert("Selected Request is require First Level Approval. Please Select Only Pending-1 Records.");
			getEqptAppList();
    	}else{
...
function ExtendRO() {
	...
	if(ro !="" && ro!=null && ro!="null"){
		var arr = ro.split(',');
		if(arr.length > 1){
			alert("You Can Extend Only One RO At a time...");
		}else{
			var d = new Date();
...
function CancelRO(){
	...
	if(ro !="" && ro!=null && ro!="null"){	
		if(statusid != "Cancelled"){	
			...
			if(arr.length > 1){
				alert("You Can Cancel Only One RO At a time...");
			}else{
...
function viewPrint(){
	...
	if(a!= null && a !="" && a!="null"){
		var arr = a.split(',');
		if(arr.length > 1){
			alert("You Can Select Only One RO At a time...")
		}else{			
