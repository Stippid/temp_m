...
<form:form name="Sho_input_feco_edit" id="Sho_input_feco_edit" action="Sho_input_feco_editAction" method='POST' commandName="Sho_input_feco_editCMD">

   <div class="card">
              <div class="card-header">
		          <h5 align="center">Edit EPIDEMIOLOGICAL INVESTIGATION REPORT(FOOD/WATER-BORNE)</h5>
				<h6>
			...
			<div class="card-body card-block">
				<div class="row">
				 <div class="col-md-12">
						...
						<div class="col-md-9">
							<input type="hidden" id="id" name="id" class="form-control-sm form-control" placeholder="Search..." >
						...
						<div class="row form-group">
							...
							<div class="col-md-8">
								<input type="text" id="personal_name" name="personal_name" class="form-control-sm form-control" autocomplete="off"
									oninput="this.value = this.value.toUpperCase()" placeholder="Enter Personnel Name..." maxlength="100" value="${Sho_input_feco_editCMD.personal_name}">
							</div>
						</div>
					</div>
				</div>
		...
		<div class="col-md-12">
            ...
            <thead style="width: 100%;">
							...
							<tr>
								...
								<tbody data-spy="scroll" id="h_food_tbody"
									...
									class="form-control-sm form-control"  autocomplete="off" >
										...
										<td id="food_f1"  class="form-control-label" onclick="foodCb(1);">
										</td>
					...
					placeholder="Source of Water Supply" value="${Sho_input_feco_editCMD.source_of_water}">
						</div>
						</div>
						</div>
</div>		
 			<div class="col-md-12">
                ...
                <div class="col-md-12">
						...
						<div class="col-md-12" style="display: none;" id="hedu">
						<div class="col-md-6">
						<div class="row form-group">
						<div class="col-md-4">
							  <label for="text-input" class=" form-control-label">Health Education </label>
						   </div>
						   <div class="col-md-8">
						     <input type="text" id="health_edu" name="health_edu" autocomplete="off" class="form-control-sm form-control">
						  </div>
					   </div>
					   </div>
					   </div>
			...
			<div class="form-control card-footer" align="center">			  
             <a href="Search_mnh_inputs_fecooral_" class="btn btn-danger btn-sm" id="btn_cancl" onclick="btn_clc();">Cancel</a>
...
<c:url value="getTblDeleteFeco" var="deleteUrl" />
<form:form action="${deleteUrl}" method="post" id="deleteForm" name="deleteForm" modelAttribute="id1">
	<input type="hidden" name="id1" id="id1" value="${Sho_input_feco_editCMD.id}"/> 
</form:form> 
<script>
...
$(document).ready(function() {
	...
	$("#id").val('${Sho_input_feco_editCMD.id}');
	...
	$().getCurDt(datee);
     ...
     var l = '${Sho_input_feco_editCMD.persnl_no}'.length;
    
 	$("#service").val(
 			'${Sho_input_feco_editCMD.service}');
 	
 	$.post("getMNHUnitNameBySUSNo?" + key + "=" + value, {y : '${Sho_input_feco_editCMD.sus_no}'},
 			function(j) {
 	...
 	if ('${Sho_input_feco_editCMD.category}' == "OR"
 			&& '${Sho_input_feco_editCMD.service}' == "ARMY") {
 		$("#persnl_no2").val(
 				'${Sho_input_feco_editCMD.persnl_no}'
 						.substring(0, l - 1));
 		$("#persnl_no3").val(
 				'${Sho_input_feco_editCMD.persnl_no}'
 						.substring(l - 1, l));
 	} else if ('${Sho_input_feco_editCMD.service}' == "AIRFORCE"
 			|| '${Sho_input_feco_editCMD.service}' == "NAVY") {
 		$("#persnl_no2").val(
 				'${Sho_input_feco_editCMD.persnl_no}'
 						.substring(0, l - 1));
 		$("#persnl_no3").val(
 				'${Sho_input_feco_editCMD.persnl_no}'
 						.substring(l - 1, l));
 	} else if ('${Sho_input_feco_editCMD.service}' == "ARMY"
 			&& '${Sho_input_feco_editCMD.category}' != "OR") {
 		$("#persnl_no1").val(
 				'${Sho_input_feco_editCMD.persnl_no}'
 						.substring(0, 2));
 		$("#persnl_no2").val(
 		...
 		$("#persnl_no3").val(
 				'${Sho_input_feco_editCMD.persnl_no}'
 						.substring(l - 1, l));
 	} else if ('${Sho_input_feco_editCMD.service}' == "OTHERS") {
 		$("#persnl_no1").val('-1');
 		$("#persnl_no2").val('');
 		$("#persnl_no3").val('-1');
 	}

 	if ('${Sho_input_feco_editCMD.service}' == "NAVY"
 			|| '${Sho_input_feco_editCMD.service}' == "AIRFORCE") {
 		$("#persnl_no1").attr('readonly', true);
 		$("#category").attr('readonly', false);
 	}
 	if ('${Sho_input_feco_editCMD.service}' == "OTHERS") {
 		$("#persnl_no1").attr('readonly', true);
 	...
 	if ('${Sho_input_feco_editCMD.service}' == "ARMY") {
 		$("#category").attr('readonly', true);
 	}

 	if ('${Sho_input_feco_editCMD.service}' != "OTHERS") {
 		$("#category").val(
 				'${Sho_input_feco_editCMD.category}');
 		getRank();
 	}
	...
	try{
	 ...
	 $("#id").val('${Sho_input_feco_editCMD.id}');
	 	...
	 	$("#sus_no1").val('${Sho_input_feco_editCMD.sus_no}');
	 	$.post("getMNHUnitNameBySUSNo?"+key+"="+value, {y:'${Sho_input_feco_editCMD.sus_no}'},function(j) {
	 		var enc = j[j.length - 1].substring(0, 16);
	 		var a = dec(enc, j[0]);
	 		$("#unit_name1").val(a);
	 	});
	 	$("#disease").attr('readonly',true);
	 	$("#disease").val('${Sho_input_feco_editCMD.disease}');
	 	
	 	$("#case_no").val('${Sho_input_feco_editCMD.case_no}');
	 	$("#service").val('${Sho_input_feco_editCMD.service}');
	 	$("#relationship").val('${Sho_input_feco_editCMD.relationship}');
	 	
	 	
	 	if('${Sho_input_feco_editCMD.service}' != "OTHERS"){
	 		$("#category").val('${Sho_input_feco_editCMD.category}');
	 		getRank();
	 		
	 	}

	 	$("#personnel_unit").val('${Sho_input_feco_editCMD.personnel_unit}');
	 	$("#present_address").val('${Sho_input_feco_editCMD.present_address}');
	 	$("#contact_no").val('${Sho_input_feco_editCMD.contact_no}');
	 	$("#type_of_symptom").val('${Sho_input_feco_editCMD.type_of_symptom}');
	 	$("#history").val('${Sho_input_feco_editCMD.history}');
	 	$("#prob_source_insp").val('${Sho_input_feco_editCMD.prob_source_insp}');
	 	$("#loc1").val('${Sho_input_feco_editCMD.en_loc1}');
	 	$("#loc2").val('${Sho_input_feco_editCMD.en_loc2}');
	 	$("#loc3").val('${Sho_input_feco_editCMD.en_loc3}');
	 	
	 	$("#final_epid_diag").val('${Sho_input_feco_editCMD.final_epid_diag}');
	 	$("#final_epid_diagnosis").val('${Sho_input_feco_editCMD.final_epid_diagnosis}');
	 	$("#action_for_patient").val('${Sho_input_feco_editCMD.action_for_patient}');
	 	$("#action_by_sho").val('${Sho_input_feco_editCMD.action_by_sho}');
	 	$("#recommendation_unit").val('${Sho_input_feco_editCMD.recommendation_unit}');
	 	
	 	
	 	
	      var d1 = new Date('${Sho_input_feco_editCMD.date_onset_symp}');
	 	var d2 = new Date('${Sho_input_feco_editCMD.date_repo_med_centre}');
 ...
 var date_of_birth = '${Sho_input_feco_editCMD.date_of_birth}';
	date_of_birth= date_of_birth.substring(0,10);
	
	$("#date_of_birth1").val(date_of_birth);
	calculate_age_edit('${Sho_input_feco_editCMD.date_of_birth}');
	gettravelDetails();
	getlistDetails();
});
...
function isvalidData(){
 	...
 	if($("#service").val() == "NAVY" && $("#").val() == "AIRFORCE"){
...
function getRank() {
	...
	$.post("getMNHRank?"+key+"="+value, {a1 : category,a2 : service},function(j) {
		...
		$("#rank").html(options);
		var q = '${Sho_input_feco_editCMD.eir_rank.id}';
		if(q != ""){
			$("#rank").val(q);
		}
	});
...
function getlistDetails() {
    ...
    var p_id = '${Sho_input_feco_editCMD.id}';
    var i = 0;
...
function h_travel_add_fn(q){
 ...
 function gettravelDetails(){
 	//alert('${Sho_input_feco_editCMD.id}');
 	var p_id='${Sho_input_feco_editCMD.id}';	
