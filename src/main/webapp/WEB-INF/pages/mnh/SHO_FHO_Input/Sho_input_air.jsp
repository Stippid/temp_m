...
<form:form name="mnh_input_air" id="mnh_input_air" action="mnh_input_airAction" method='POST' commandName="mnh_input_airCMD">
 
          <div class="card">
              <div class="card-header">
		           <h5 align="center">EPIDEMIOLOGICAL INVESTIGATION REPORT(AIR BORNE)</h5>
				<h6>
			...
			<div class="card-body card-block">
				<div class="row">
				 ...
				 <div class="col-md-12">
						...
						<div class="col-md-8">
	             			  <input type="date" id="datee1" name="datee1" class="form-control-sm form-control" placeholder="Search..." 
	...
	<%-- <div class="col-md-12">
 	 ...
 	 -->
           ...
           <div class="form-control card-footer" align="center">
			    <a	href="mnh_input_air" type="reset" class="btn btn-primary btn-sm"  onclick="btn_clc();" > Clear </a> 
...
function Clearvalue(){
	$("#ip_end_date").val("");
	$("#ip_start_date").val("");
	document.getElementById('ip_end_date').disabled=false;
	document.getElementById('ip_start_date').disabled=false;
	$("#divPrint").hide();
	$("#divPrint3").hide();
	$("#divPrint4").hide();
	$("#divPrint5").hide();
	...
	var tb0 = $("#myTable > tbody");
	tb0.empty(); 
	var tableRow3 = $("#myTable3 > tbody");
	tableRow3.empty(); 
	var tableRow4 = $("#myTable4 > tbody");
	tableRow4.empty(); 
	var tableRow5 = $("#myTable5 > tbody");
	tableRow5.empty(); 
...
function tb0(a,b,c){
	...
	$("#ip_end_date").attr('disabled',false);
	$("#ip_start_date").attr('disabled',false);
	
	var diff = new Date(new Date(b) - new Date(a));
	var days = diff/(1000*60*60*24) + 1;
	var dif_val = $("#dif_val_hid").val(days);
	var c = $("#disease").val();

	//Air
	if(c == "6" &&  days <2 ){
		alert("Minimum 2 days Incubation Period required");
		$("#divPrint").hide();
		}
	else if(c == "16" &&  days <3 ){
		alert("Minimum 3 days Incubation Period required");
		$("#divPrint").hide();
		}
	else if(c == "16" &&  days >5 ){
		alert("Maximum 5 days Incubation Period required");
		$("#divPrint").hide();
		}
	else if(c == "36" &&  days <7 ){
		alert("Minimum 7 days Incubation Period required");
		$("#divPrint").hide();
		}
	else if(c == "36" &&  days >8  ){
		alert("Maximum 8 days Incubation Period required");
		$("#divPrint").hide();
		}
	else if(c == "37" &&  days <10 ){
		alert("Minimum 10 days Incubation Period required");
		$("#divPrint").hide();
		}
	else if(c == "34" &&  days <7 ){
		alert("Minimum 7 days Incubation Period required");
		...
		for (var i = 0; i < days; i++){
			...
			row +="<td> <input type='date' id='ip_date_"+i+"' name='ip_date' onchange='ipdate()'> </td>";
			row +="<td> <input type='time' id='time1"+i+"' name='time1' ></td>";
			row +="<td> <input  id='loc1_"+i+"'  name='loc1' autocomplete='off'</td>";
			row +="<td> <input type='time' id='time2"+i+"' name='time2'  ></td>";
			row +="<td> <input  id='loc2_"+i+"'  name='loc2' autocomplete='off'></td>";
			row +="<td> <input type='time' id='time3"+i+"' name='time3' ></td>";
			row +="<td> <input id='loc3_"+i+"'  name='loc3' autocomplete='off'></td>";
			row +="<td> <input type='time'id='time4"+i+"' name='time4'></td>";
			row +="<td> <input id='loc4_"+i+"'  name='loc4' autocomplete='off'></td>";
			row +="<td> <input type='time' id='time5"+i+"' name='time5' ></td>";
			row +="<td> <input id='loc5_"+i+"'  name='loc5' autocomplete='off'></td>";	
			row +="</tr>";
		}
		$("#myTable").append(row);
		$("#divPrint").show();
...
function tableRow3(a,b){
	...
	var c = $("#disease").val();
		...
		for (var i = 0; i < days; i++){
			...
			row +="<td> <input id='close_contact_air1_"+i+"' name='close_contact_air1' class='form-control-sm form-control' autocomplete='off'</td>";
			row +="<td> <input type='date' id='date_exp_air1_"+i+"' name='date_exp_air1' onchange='dateexpair()'  class='form-control-sm form-control' </td>";
			row +="<td> <input id='remarks_air_"+i+"' name='remarks_air' class='form-control-sm form-control' autocomplete='off' </td>";
			row +="</tr>";
			}
		$("#myTable3").append(row);
		$("#divPrint3").show();
}
function tableRow4(a,b){
	...
	var c = $("#disease").val();
		...
		var tableRow3 = $("#myTable4 > tbody");
		tableRow3.empty();
		
		var row ="";
		for (var i = 0; i < days; i++){
			row +="<tr>";
			row +="<td> <input type='date' id='date_air_"+i+"' name='date_air' onchange='dateair()' class='form-control-sm form-control' </td>";
			row +="<td> <input type='time' id='time_air_"+i+"' name='time_air' class='form-control-sm form-control' </td>";
			row +="</tr>";
			}
		$("#myTable4").append(row);
		$("#divPrint4").show();
...
function tableRow5(){
    ...
    for (var i = 0; i < 2; i++){
	      ...
	      row +="<tr>";
	      row +="<td> "+m+"</td>";
	      row +="<td> <input id='close_contact_air2_"+i+"' name='close_contact_air2' class='form-control-sm form-control' autocomplete='off'</td>";
		  row +="<td> <input type='date' id='date_exp_air2_"+i+"' name='date_exp_air2' onchange='dateexpair2()' class='form-control-sm form-control' </td>";
		  row +="<td> <input id='period_segration_"+i+"' name='period_segration' class='form-control-sm form-control' autocomplete='off' </td>";
	      row +="</tr>";
		}
		$("#myTable5").append(row);
		$("#divPrint5").show();
...
function ipdate() {
	...
	for (var p = 0; p < days; p++){
		var ip_date_id = 'ip_date_' +p ;
		var ip_date = document.getElementById(ip_date_id).value; 
	 	if((Date.parse(c_d) < Date.parse(ip_date)))	{
	 	    alert("Can't select the Future  Date");
	 		$(ip_date_id).focus();
	 		document.getElementById(ip_date_id).value="";
	 		return false;
	 	}
	}
}
...
function dateair() {
	...
	var c_d = d.getFullYear()+"-"+("0" + (d.getMonth()+1)).slice(-2)+"-"+("0" + d.getDate()).slice(-2);
	for (var p = 0; p < days; p++){
	var date_air_id = 'date_air_' +p ;
		var date_air = document.getElementById(date_air_id).value;	
	 	if((Date.parse(c_d) < Date.parse(date_air)))	{
	 	    alert("Can't select the Future  Date");	 	  
	 	    $(date_air_id).focus();
	 	    document.getElementById(date_air_id).value="";
	 		return false;
	 	}
	}
}
...
function dateexpair() {
	...
	var c_d = d.getFullYear()+"-"+("0" + (d.getMonth()+1)).slice(-2)+"-"+("0" + d.getDate()).slice(-2);
	for (var p = 0; p < days; p++){
	var date_exp_air1_id = 'date_exp_air1_' +p ;
		var date_exp_air1 = document.getElementById(date_exp_air1_id).value;	
	 	if((Date.parse(c_d) < Date.parse(date_exp_air1)))	{
	 	    alert("Can't select the Future  Date");
	 	    $(date_exp_air1_id).focus();
	 	    document.getElementById(date_exp_air1_id).value="";
	 		return false;
	 	}
	}
}

function dateexpair2() {
	var a = $("#ip_start_date").val();
	var b = $("#ip_end_date").val();
	var diff = new Date(new Date(b) - new Date(a));
	var days = diff/(1000*60*60*24) + 1;
