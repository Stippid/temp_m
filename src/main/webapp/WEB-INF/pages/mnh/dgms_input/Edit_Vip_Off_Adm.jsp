...
<form:form name="edit_vip_off_adm" id="edit_vip_off_adm"
	action="vip_off_adm_editAction" method='POST'
	commandName="vip_off_adm_editCmd">
	...
	<div class="container" align="center">
		<div class="card">
			<div class="card-header">
				<h5>
					<span id="lbladd"></span> UPDATE VIP (MAJ GEN & ABOVE) ADMISSIONS
				</h5>
					...
					<div class="col-md-12">
						...
						<div class="col-md-4">
							<div class="row form-group">
								...
								<div class="col-md-8">
									<input type="hidden" id="persnl_no_Original"
										name="persnl_no_Original" class="form-control-sm form-control">
									<input type="hidden" id="dt_report1Original"
							...
							<div class="row form-group">
								<div class="col-md-4">
									<select id="persnl_no1" name="persnl_no1"
										...
										<option value="-1">--Select--</option>
										<c:forEach var="item" items="${getMedSystemCode_PERSNL_PRE}"
											varStatus="num">
											<option value="${item}" name="${item}">${item}</option>
										</c:forEach>
									</select>
								...
								<div class="col-md-4">
									<select id="persnl_no3" name="persnl_no3"
										class="form-control-sm form-control">
										<option value="-1">--Select--</option>
										<c:forEach var="item" items="${getMedSystemCode_PERSNL_SUF}"
											varStatus="num">
											<option value="${item}" name="${item}">${item}</option>
										</c:forEach>
									</select>
					...
					<div class="col-md-12">
						...
						<div class="col-md-6">
							<div class="row form-group">
								...
								<div class="col-md-8">
									<input type="text" id="adhar_card_no1" name="adhar_card_no"
							...
							<div class="row form-group">
								...
								<div class="col-md-8">
									<select id="type" name="type"
										class="form-control-sm form-control">
										<option value="-1">--Select--</option>
										<c:forEach var="item" items="${getMedSystemCode_TYPE}"
											varStatus="num">
											<option value="${item}" name="${item}">${item}</option>
										</c:forEach>
									</select>
							...
							<div class="row form-group">
								...
								<div class="col-md-8">
									<input type="text" id="contact_no" name="contact_no1"
										value="${vip_off_adm_editCmd.contact_no}" autocomplete="off"
										class="form-control-sm form-control"
			...
			<input type="hidden" id="id" name="id"
				value="${vip_off_adm_editCmd.id}" class="form-control"
				autocomplete="off">

			<div class="card-footer" align="center">

				<a href="mnh_search_vip_officersDA" class="btn btn-danger btn-sm"
					id="btn_cancl" onclick="btn_clc();">Cancel</a> <input type="submit"
					id="btn_update" class="btn btn-success btn-sm" value="Update"
...
<script>
	...
	function isvalidData() {
		...
		if ($("#service").val() == "-1") {
			...
			/* 	if ($("#diagnosis1").val() == "-1" || $("#diagnosis1").val() == "") {
		...
		$
				.post(
						...
						function(j) {
							...
							$("#rank").html(options);
							var q = '${vip_off_adm_editCmd.v_rank.id}';
							if (q != "") {
								$("#rank").val(q);
							}
						});
	...
	$("#diagnosis1").keyup(function() {
		var d1 = this.value;
		$().Autocomplete2('GET', 'getMNHDistinctICDList', diagnosis1, {
			a : d1,
			b : "ALL"
	...
	$(document)
			.ready(
					function() {
						...
						var service = $("#service").val();
						$("#appointment").val(
								'${vip_off_adm_editCmd.appointment}');
						$("#contact_no").val(
								'${vip_off_adm_editCmd.contact_no}');

						$().getCurDt(dt_report1);
						$("#diagnosis_code1d").val(
								'${vip_off_adm_editCmd.diagnosis_code1d}');
						$("#icd_cause_code1d").val(
								'${vip_off_adm_editCmd.icd_cause_code1d}');

						if ('${vip_off_adm_editCmd.persnl_no}' == '-2') {
							$("#persnl_no_Original").val('-2');
						}

						if ('${vip_off_adm_editCmd.diagnosis_code1d}' != "") {
							...
							$
									.post(
											"getMNHICDCodeToCauseVi?" + key
													+ "=" + value,
											{
												a : '${vip_off_adm_editCmd.diagnosis_code1d}',
												b : "2"
											},
											function(j) {
												var enc = j[j.length - 1]
														.substring(0, 16);
												var a = dec(enc, j[0]);
												var data = '${vip_off_adm_editCmd.diagnosis_code1d}'
														+ "-" + a;
												$("#diagnosis_code1d")
														.val(data);

												var b = '${vip_off_adm_editCmd.diagnosis_code1d}'
														.substring(0, 1);
												if (b == "S" || b == "T"
														|| b == "V" || b == "W"
														|| b == "X" || b == "Y") {
						...
						if ('${vip_off_adm_editCmd.icd_cause_code1d}' != "") {
							$
									.post(
											"getMNHICDCodeToCauseVi?" + key
													+ "=" + value,
											{
												a : '${vip_off_adm_editCmd.icd_cause_code1d}',
												b : "2"
											},
											function(j) {
												var enc = j[j.length - 1]
														.substring(0, 16);
												var a = dec(enc, j[0]);
												var data = '${vip_off_adm_editCmd.icd_cause_code1d}'
														+ "-" + a;
												$("#icd_cause_code1d")
														.val(data);
						...
						var l = '${vip_off_adm_editCmd.persnl_no}'.length;
						let b = '${vip_off_adm_editCmd.service}'
						//alert("b"+b);
						$("#service").val('${vip_off_adm_editCmd.service}');
						$("#type").val('${vip_off_adm_editCmd.type}');
						$.post("getMNHUnitNameBySUSNo?" + key + "=" + value, {
							y : '${vip_off_adm_editCmd.sus_no}'
						}, function(j) {
						...
						let a = '${vip_off_adm_editCmd.persnl_no}';
						//alert(a);
						if ('${vip_off_adm_editCmd.persnl_no}' == '-2') {
							$("#persnl_no_Original").val('-2');
						}

						if ('${vip_off_adm_editCmd.category}' == "OR"
								&& '${vip_off_adm_editCmd.service}' == "ARMY") {
							$("#persnl_no2").val(
									'${vip_off_adm_editCmd.persnl_no}'
											.substring(0, l - 1));
							$("#persnl_no3").val(
									'${vip_off_adm_editCmd.persnl_no}'
											.substring(l - 1, l));
						} else if ('${vip_off_adm_editCmd.service}' == "AIRFORCE"
								|| '${vip_off_adm_editCmd.service}' == "NAVY") {
							$("#persnl_no2").val(
									'${vip_off_adm_editCmd.persnl_no}'
											.substring(0, l - 1));
							$("#persnl_no3").val(
									'${vip_off_adm_editCmd.persnl_no}'
											.substring(l - 1, l));
						} else if ('${vip_off_adm_editCmd.service}' == "ARMY"
								&& '${vip_off_adm_editCmd.category}' != "OR") {
							$("#persnl_no1").val(
									'${vip_off_adm_editCmd.persnl_no}'
											.substring(0, 2));
							$("#persnl_no2").val(
							...
							$("#persnl_no3").val(
									'${vip_off_adm_editCmd.persnl_no}'
											.substring(l - 1, l));
						} else if ('${vip_off_adm_editCmd.service}' == "OTHERS") {
							$("#persnl_no1").val('-1');
							$("#persnl_no2").val('');
							$("#persnl_no3").val('-1');
						} else if ('${vip_off_adm_editCmd.service}' == "-1") {
							$("#persnl_no1").val('-1');
							$("#persnl_no2").val('');
							$("#persnl_no3").val('-1');
						...
						if ('${vip_off_adm_editCmd.service}' == "NAVY"
								|| '${vip_off_adm_editCmd.service}' == "AIRFORCE") {
							$("#persnl_no1").attr('readonly', true);
							$("#category").attr('readonly', false);
						}
						if ('${vip_off_adm_editCmd.service}' == "OTHERS") {
							$("#persnl_no1").attr('readonly', true);
							$("#persnl_no2").attr('readonly', true);
							$("#persnl_no3").attr('readonly', true);
							$("#category").attr('readonly', true);
						...
						if ('${vip_off_adm_editCmd.service}' == "ARMY") {
							$("#category").attr('readonly', true);
						}

						if ('${vip_off_adm_editCmd.service}' != "OTHERS") {
							$("#category").val(
									'${vip_off_adm_editCmd.category}');
							getRank();
						}

						var dt_report = '${vip_off_adm_editCmd.dt_report}'
								.substring(0, 10);
						$("#dt_report1Original").val(dt_report);

						$("#dt_report1").val(dt_report);
						$("#sex").val('${vip_off_adm_editCmd.sex}');
						$("#relation").val('${vip_off_adm_editCmd.relation}');
						var date_of_birth = '${vip_off_adm_editCmd.date_of_birth}'
								.substring(0, 10);

						$("#date_of_birth1").val(date_of_birth);

						var d2 = new Date(
								'${vip_off_adm_editCmd.date_time_admission}');
						...
						if ('${vip_off_adm_editCmd.adhar_card_no}' != "") {
							$("#adhar_card_no1").val(
									'${vip_off_adm_editCmd.adhar_card_no}');

							if ('${vip_off_adm_editCmd.adhar_card_no}' == "0") {
								$("#adhar_card_no1").val('');
							}

						}

						$("#remarks").text('${vip_off_adm_editCmd.remarks}');
...
<script>
	function personal_number() {
		...
		var personnel_no = '${vip_off_adm_editCmd.persnl_no}';
