  ...
  <form id = "filed_form" >
		...
		<table id="head_table" class="table-bordered" style="margin-top: 10px; width: 100%;">
								<thead style="color: white; text-align: center;">
									<tr>
										<th>Sr No</th>
										<th><strong style="color: red;"> </strong>Detail</th>
								...
								<tbody data-spy="scroll" id="head_tbody" style="min-height: 46px; max-height: 650px; text-align: center;">
									<tr id="tr_head1">
										...
										<input type="hidden" id="hd_table1" name="hd_table1" class="form-control autocomplete" autocomplete="off"></input>
									</div>
								</td>
							 ...
							 <tr id="tr_allergic1">
								...
								<td>    
									...
									<div id="checkboxes_filter1" class="checkboxes"
										...
										<input type="hidden" id="hd_column_filter1" name="hd_column_filter1" class="form-control autocomplete"  autocomplete="off"></input>
									</div>
						...
						<div align="left">
								...
								<tbody data-spy="scroll" id="sort_tbody" style="min-height: 46px; max-height: 650px; text-align: center;">
									...
									<div id="checkboxes_sort1" class="checkboxes"
										...
										<input type="hidden" id="hd_column_sort1" name="hd_column_sort1" class="form-control autocomplete" autocomplete="off"></input>
									</div>
								</td>
							<td>
...
function add_fn_sort(x){
	...
	$("table#sort_table").append('<tr id="tr_sort'+x+'">'
			...
			+ '</div>'
			+ '<input type="hidden" id="hd_column_sort'+x+'" name="hd_column_sort'+x+'" class="form-control autocomplete" autocomplete="off"></input>'
			+ '</div>'
			+ '</td>'
			+'<td>'
...
function remove_fn_sort(Rv){
	...
	function add_fn_head(xk){
		...
		$("table#head_table").append('<tr id="tr_head'+xk+'">'
				...
				+ '	</c:forEach>'
				+ '</div>'
				+ '<input type="hidden" id="hd_table'+xk+'" name="hd_table'+xk+'" class="form-control autocomplete" autocomplete="off"></input>'
				+ '</div>'
				+ '</td>'
				+'<td style="display: none;">'
	...
	function get_query() {
		...
		var vk = $("input#count_head").val();
			for (var nm = 1; nm <=vk; nm++) {
				var hd = $("#hd_column"+nm).val();
				
				
				
				
				var hd1 = hd.split('.').pop();
 				var ntd= hd.includes("date");
				if(ntd==true){
 					var hd_column =  "ltrim(TO_CHAR("+hd+",'DD-MON-YYYY'),'0') as "+hd1;
				}
				else{
					var hd_column = $("#hd_column"+nm).val();
				}
	...
	if ($("#andor"+aller_srno).val() == "-1") {
		...
		$("table#ad_hoc_table").append('<tr id="tr_allergic'+aller_srno+'">'
				...
				+ '</div>'
				+ '<input type="hidden" id="hd_column_filter'+aller_srno+'" name="hd_column_filter'+aller_srno+'" class="form-control autocomplete" autocomplete="off"></input>'
				+ '</div>'
				+ '</td>'
				+'<td>'
	...
	function getExcel() {
		/*  $("#create_query1").val($("#create_query").val());
		document.getElementById('typeReport1').value = 'excelL';
		document.getElementById('search2').submit();
		
        var count_head = $("#count_head").val();
		
		for (var i = 1; i < count_head; i++) {
			var hd_col = $("#hd_column"+i).val();
			
		}
		 */
		 var formdata=$('#filed_form').serialize();
		 var excelL=$('#typeReport1').val("excelL");
		 alert("excel---" + excelL);
		 
		 formdata+="&excelL="+excelL;
		 $.post('AdhocQueryreport?' + key + "=" + value, formdata , function(j){ 
...
<%-- <c:url value="AdhocQueryreport" var="searchUrl" />
<form:form action="${searchUrl}" method="post" id="search2" name="search2" modelAttribute="comd1">
	<input type="hidden" name="typeReport1" id="typeReport1" value="0" />
	<input type="hidden" name="create_query1" id="create_query1" value="0" />
</form:form> --%>