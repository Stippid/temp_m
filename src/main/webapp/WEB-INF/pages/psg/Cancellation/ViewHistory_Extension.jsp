...
<div class="col-md-12" style="padding-top: 5px; " align="center">
           ...
           <div class="watermarked" data-watermark="" id="divwatermark"><span id="ip"></span>
                 <table id="Extensionpopuptable" class="table no-margin table-striped  table-hover  table-bordered report_print">        
                                        <thead style="text-align: center;">
                                           ...
                                           <tbody id="Extensionpopuptbody" align="center">
...
function getHistoryData(){
	...
	var status='${status}';
	var remp_id='${remp_id}';
    $('#Extensionpopuptbody').empty(); 
     $.post('getExtensionHistoryData?' + key + "=" + value, {status:status,comm_id:comm_id,remp_id:remp_id}, function(j){
                   var v=j.length;
                   if(v!=0){
                	   var i=0;          
           for(i;i<v;i++){
             ...
             $("table#Extensionpopuptable").append('<tr><td>'+am+'</td>'            		
            		 +'<td>'+j[i].authority+'</td>'
				 ...
				 +'</tr>');
                	   ...
                	   $("table#Extensionpopuptable").append('<tr><td colspan="8" align="center">Data Not Available</td>'
                				 +'</tr>');
...
function getApproveHistoryData(){
     ...
     $.post('ApproveExtensionHistoryData?' + key + "=" + value, {remp_id:remp_id,comm_id:comm_id}, function(j){
                  if(j==1){
...
function CancelHisExtensionData(id){
	...
	$.post('CancelHisExtensionData?' + key + "=" + value,{id:id,set_status:set_status}, function(j){
