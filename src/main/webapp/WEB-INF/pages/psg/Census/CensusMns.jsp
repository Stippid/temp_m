						...
						<div class="panel panel-default" id="insp_div1">
							<div class="panel-heading">
								...
								<div class="card-header">
									<h5> FILL CENSUS DETAILS OF MNS OFFR</h5>
									<h6 class="enter_by">
...
function personal_number() {
	...
	// this logic for mns pranay 31.05
	
	if(personnel_no.substring(0, 2) == 'NR' || personnel_no.substring(0, 2) == 'NS'){
		commission_institute('mns','2');
		trainig_institute ('mns','2');
	}else {
		commission_institute('miso','1');
		trainig_institute ('miso','2');
	}
	$.post('GetPersonnelNoData?' + key + "=" + value, {
		...
		checkDataFillIn3008Mns(comm_id);
...
$("input#personnel_no").keyup(function() {
	...
	susNoAuto.autocomplete({
		source: function(request, response) {
			$.ajax({
				type: 'POST',
				url: "getpersonnel_noListMns?" + key + "=" + value,
				data: {
...
if(count_classi >= 3){
	...
	form_data.append("org_co", text1);
	form_data.append("org_so", text2);
	form_data.append("org_do", text3);
	form_data.append("org_to", text4);
...
function commission_institute(accesstype,type){

	   var options = '<option value="0">' + "--Select--" + '</option>';
	   var type = type;
	   var accesstype = accesstype;

	   $.post("commission_institute?" + key + "=" + value, { accesstype :accesstype, type: type })
	     .done(function(j) {
	       for (var i = 0; i < j.length; i++) {
	         options += '<option value="' + j[i][0] + '" name="' + j[i][1] + '">' + j[i][1] + '</option>';
	       }
	       $("#com_institute").html(options);
	     })
	     .fail(function(xhr, textStatus, errorThrown) {
...
function trainig_institute(accesstype,type){
	   ...
	   $.post("commission_institute?" + key + "=" + value, { accesstype :accesstype, type: type })
	     .done(function(j) {
	       ...
	       $("#training_institute").html(options);
	     })
	     .fail(function(xhr, textStatus, errorThrown) {
...
function checkDataFillIn3008Mns(comm_id) {
	
	  $.post('check_3008mns_fill?' + key + "=" + value, {comm_id:comm_id}, function(j) {
		    if(j>0){
		    	 $('#marital_status').val(j);   
		    	 $('#marital_status').prop('disabled', true);         
		    
		         $('#marital_status').after('<input type="hidden" name="marital_status" value="' + j + '">');
		    }		    
