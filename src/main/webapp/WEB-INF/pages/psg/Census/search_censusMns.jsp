...
<script src="js/miso/orbatJs/orbatAll.js" type="text/javascript"></script>
	...
	<div class="animated fadeIn">
	    	<div class="container" align="center">
	    		<div class="card">
	    		<div class="card-header"><h5> Search/Approve Census Data Mns</h5> <h6 class="enter_by"><span style="font-size:12px;color:red;">(To be entered by unit)</span></h6></div>
						...
						<div class="card-footer" align="center" class="form-control">
							<a href="search_censusMnsUrl" class="btn btn-success btn-sm" >Clear</a>  
		              		<input type="button" class="btn btn-info btn-sm"  value="Search" onclick="Search();"> 
...
<!-- 							</div> -->
	...
	<div class="col-md-12" id="getSearch_Letter_a" style="display: block;"> 	 
			<div  class="watermarked" data-watermark="" id="divwatermark"><span id="ip"></span>
		           <table id="getSearch_censusbl" class="table no-margin table-striped  table-hover  table-bordered ">
		                 <thead>
		                       <tr>
			                        <th style="text-align: center; width:5%;">Ser No</th>
			                          <th style="text-align: center;"> Cadet No </th>	
			                          ...
			                          <th style="text-align: center;"> Unit</th>	
			                          <th style="text-align: center;"> Parent Arm </th>
			                    <c:if test="${roleType == 'DEO'  && status1 == '3'}">
...
<!-- pranay 07.06 -->
<c:url value="modificationAfterApproval_CensusDetails" var="modificationAfterApprovalUrl" />
	<form:form action="${modificationAfterApprovalUrl}" method="post" id="modificationAfterApprovalForm" name="modificationAfterApprovalForm" >
		<input type="hidden" name="personnel_no_modification" id="personnel_no_modification" value="0"/>
...
$(document).ready(function() {
	...
	if( r == "Unit"){
	 ...
	 if('${list.size()}' == ""){
	    ...
	    mockjax1('getSearch_censusbl');
		table = dataTable('getSearch_censusbl');
...
function data(tableName){
	...
	var rank =	$("#rank").val() ;	
	var status =	$("#statusA1").val() ;	
	var IsMns = "True"
	var cr_by =	$("#cr_by").val() ;	
	var cr_date =	$("#cr_date").val() ;	

	///////////serving/////
	if (tableName=="getSearch_censusbl") {
		
		$.post("GetSearch_censusCount?"+key+"="+value,{Search:Search,unit_sus_no:unit_sus_no,unit_name:unit_name,
			personnel_no:personnel_no,rank:rank,status:status,cr_by:cr_by,cr_date:cr_date,IsMns:IsMns},function(j) {
			FilteredRecords = j;
		});
		
		
	
		$.post("GetSearch_censusdata?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType
		...
		/* 	for (var i = 0; i < j.length; i++) {
				var sr = i+1;
				 // alert(j[i].lf + "------ ")
				jsondata.push([sr,j[i].cadet_no,j[i].personnel_no,j[i].rank,j[i].name,j[i].date_of_birth,
					j[i].unit_name,j[i].parent_arm,j[i].action  
				]);
			...
			for (var i = 0; i < j.length; i++) {
				var sr = i+1;
	         	var data =[sr,j[i].cadet_no,j[i].personnel_no,j[i].rank,j[i].name,j[i].date_of_birth,
					j[i].unit_name,j[i].parent_arm
					<c:if test="${roleType == 'DEO'  && status1 == '3'}">
               	 ,j[i].reject_remarks
                 </c:if>
                 ,j[i].action ];
			 ...
			 var susNoAuto=$("#personnel_no");
			  susNoAuto.autocomplete({
			      source: function( request, response ) {
			        $.ajax({
			        	type: 'POST',
				    	url: "psg_census_getpersonnel_noMnsList?"+key+"="+value,
...
function setApproveStatus(){
	...
	if(census_id == ""){
		...
		$.post("Approve_Census?"+key+"="+value, {census_id:census_id}).done(function(j) {
			alert(j);
			Search();
		}); 
...
function modificationAfterApprovalData(comm_id){
	$("#personnel_no4").val($("#personnel_no").val()) ;	
	$("#status4").val($("#statusA1").val()) ;		
	$("#rank4").val($("#rank").val()) ;		
	$("#unit_name4").val($("#unit_name").val()) ;	
	$("#unit_sus_no4").val($("#unit_sus_no").val()) ;
	
     if(getPersonnelNo(comm_id,'personnel_no_modification','','')){
    	 document.getElementById('modificationAfterApprovalForm').submit();
	}else{
		return false;
	}
}
