...
<form action="" method="post" enctype="multipart/form-data" class="form-horizontal"> 
	<div class="animated fadeIn">
	    	...
	    	<div class="col-md-12" align="center">
	    		<div class="card">
	    			<div class="card-header"><h5>TOTAL NUMBER OF GOVT SERVANT AND THE NUMBER OF SC/ST/OBC AMONGST THE ARMY</h5></div>
	    			<div class="card-header"><h5>DATA AS ON : ${date}</h5></div>
						 ...
						 <div class="col-md-12">
							  <%--  <div class="col-md-6">
										<div class="row form-group">
											<div class="col-md-4">
												<label class=" form-control-label"> Regular/NonRegular </label>
											</div>
											<div class="col-md-8">
												<select name="civilian_status" id="civilian_status"
													class="form-control">
													<option value="0">--Select--</option>
													<c:forEach var="item" items="${GetCiv_Civilian_TypeList}"
														varStatus="num">
														<option value="${item[0]}" name="${item[1]}">${item[1]}</option>
													</c:forEach>
												</select>
									...
									<div class="col-md-6">
										<div class="row form-group">
											<div class="col-md-4">
												<label class=" form-control-label"> If Regular </label>
											</div>
											<div class="col-md-8">
												<select name="service_status" id="service_status"
													class="form-control">
													<option value="0">--Select--</option>
													<c:forEach var="item" items="${getServiceStatusList}"
														varStatus="num">
														<option value="${item[0]}" name="${item[1]}">${item[1]}</option>
													</c:forEach>
												</select>
						...
						<div class="card-footer" align="center">
							<a href="Total_Servant_SC_ST_OBC_Wise_Url" type="reset" class="btn btn-success btn-sm"> Clear </a> 
		...
		<div class="col-md-12">
			<table id="Per_Temp_Report_Tbl" class="display table no-margin table-striped  table-hover table-bordered ">
			<div class="card-header"><h5>REGULAR EST (PERMANENT/TEMPORARY)</h5></div>	
				<thead>
				
			        <tr>
						<th rowspan="2">Arm/Service</th>
						<th rowspan="2">Total Govt Servant</th>
		...
		<div class="col-md-12">
			<table id="Non_Regular_Report_Tbl" class="display table no-margin table-striped  table-hover table-bordered ">
					<div class="card-header"><h5>NON REGULAR EST</h5></div>
				<thead>
			        <tr>
						<th>Arm/Service </th>
						<th>Total Govt Servant</th>
						<th>SC</th>
						<th>ST</th>
						<th>OBC</th>
			    	</tr>
...
<c:url value="Download_Total_Servant_SC_ST_OBC_Wise" var="dwonloadUrl"/>
<form:form action="${dwonloadUrl}" method="post" id="downloadForm" name="downloadForm" modelAttribute="cont_comd_dn">
	   ...
	   <input type="hidden" name="service_status_tx" id="service_status_tx">
	   <input type="hidden" name="unit_name_dn" id="unit_name_dn">
	   <input type="hidden" name="sus_no_dn" id="sus_no_dn">
	   <input type="hidden" name="service_status_dn" id="service_status_dn" value="0">
</form:form>

<c:url value="Excel_Total_Servant_SC_ST_OBC_Wise" var="excelUrl" />
<form:form action="${excelUrl}" method="post" id="ExcelForm" name="ExcelForm" modelAttribute="cont_comd_ex">
	 <input type="hidden" name="cont_comd_ex" id="cont_comd_ex"  value="0">
	   ...
	   <input type="hidden" name="cont_bde_txt" id="cont_bde_txt">
	    <input type="hidden" name="service_status_txt" id="service_status_txt">
	   <input type="hidden" name="unit_name_ex" id="unit_name_ex">
	   <input type="hidden" name="sus_no_ex" id="sus_no_ex">
	   <input type="hidden" name="service_status_ex" id="service_status_ex" value="0">
...
$(document).ready(function() {  
	...
	mockjax1('Per_Temp_Report_Tbl');
	table = dataTable('Per_Temp_Report_Tbl');
	...
	mockjax1('Non_Regular_Report_Tbl');
	table1 = dataTable('Non_Regular_Report_Tbl');
	$('#btn-reload').on('click', function(){
		table1.ajax.reload();
	});
...
function data(tableName){
	if(tableName == "Per_Temp_Report_Tbl"){
		jsondata = [];
		...
		var cont_comd=$("#cont_comd").val();
	 	...
	 	var s_total = 0; var s_gp_a_sc = 0; var s_gp_a_st =0; var s_gp_a_obc = 0; var s_gp_b_sc = 0; 
	 	var s_gp_b_st = 0; var s_gp_b_obc = 0; var s_gp_c_sc = 0; var s_gp_c_st = 0; var s_gp_c_obc = 0; 
	
		$.post("getPer_Temp_SC_ST_OBC_ReportDataList?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType,
			cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no,service_status:service_status},function(j) {
			
			for (var i = 0; i < j.length; i++) {
				jsondata.push([j[i].arm_desc, j[i].total,j[i].gp_a_sc,j[i].gp_a_st,j[i].gp_a_obc,
					j[i].gp_b_sc,j[i].gp_b_st,j[i].gp_b_obc,j[i].gp_c_sc,j[i].gp_c_st,j[i].gp_c_obc,]);
				
				s_total += j[i].total; s_gp_a_sc += j[i].gp_a_sc; s_gp_a_st += j[i].gp_a_st; s_gp_a_obc += j[i].gp_a_obc; 
				s_gp_b_sc += j[i].gp_b_sc; s_gp_b_st += j[i].gp_b_st; s_gp_b_obc += j[i].gp_b_obc;
				s_gp_c_sc += j[i].gp_c_sc; s_gp_c_st += j[i].gp_c_st; s_gp_c_obc +=  j[i].gp_c_obc;
			}
			jsondata.push(["TOTAL".bold(),s_total,
				s_gp_a_sc,s_gp_a_st,s_gp_a_obc,s_gp_b_sc,s_gp_b_st,
				s_gp_b_obc,s_gp_c_sc,s_gp_c_st,s_gp_c_obc]);
		});
		$.post("getPer_Temp_SC_ST_OBC_TotalCount?"+key+"="+value,{Search:Search,cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,
			cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no,service_status:service_status},function(j) {
			FilteredRecords = j;
		});
	}
	if(tableName == "Non_Regular_Report_Tbl"){
		jsondata = [];
		var table = $('#'+tableName).DataTable();
		var info = table.page.info();
		...
		var cont_comd=$("#cont_comd").val();
	 	...
	 	var s_total = 0; var s_sc = 0; var s_st =0; var s_obc = 0;
// 	 	s_total.bold();
// 	 	s_sc.bold();
// 	 	s_st.bold();
// 	 	s_obc.bold();
		$.post("getNon_Regular_SC_ST_OBC_ReportDataList?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType,
			...
			for (var i = 0; i < j.length; i++) {
				jsondata.push([j[i].arm_desc, j[i].total,j[i].sc, j[i].st,j[i].obc]);
				
				s_total += j[i].total; s_sc += j[i].sc; s_st += j[i].st; s_obc += j[i].obc; 
			}
			jsondata.push(["TOTAL".bold(),s_total,s_sc,s_st,s_obc]);
		});
		$.post("getNon_Regular_SC_ST_OBC_TotalCount?"+key+"="+value,{Search:Search,cont_comd:cont_comd,cont_corps:cont_corps,
...
function downloaddata(){
	var cont_comd_tx = $("#cont_comd option:selected").text();
 	...
 	var service_status_tx=$("#service_status option:selected").text();
	...
	$("#service_status_tx").val(service_status_tx);
	...
	$("#service_status_dn").val(service_status);
...
function getExcel() {	
	var cont_comd_txt = $("#cont_comd option:selected").text();
 	...
 	var service_status_txt=$("#service_status option:selected").text();
	...
	$("#service_status_txt").val(service_status_txt);
	...
	$("#service_status_ex").val(service_status);
