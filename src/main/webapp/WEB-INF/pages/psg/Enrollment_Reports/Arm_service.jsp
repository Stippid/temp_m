...
<div class="animated fadeIn">
	<div class="">
		<div class="col-md-12" align="center">
			<div class="card">
			    <div class="card-header" align="center"><h5>NUMBER OF PERS OF THE UNITS / EST CAME FROM OTHER ARMS / SERVICE THROUGH INTER ARM TRANSFER</h5></div>
				<div class="card-body">
    					<div class="col-md-12">
    						<div class="col-md-6">
...
<!-- 										<option value = "0">--Select--</option> -->
		...
		<a href="Arm_service" class="btn btn-success btn-sm" >Clear</a>  
...
mockjax1('arm');
table = dataTable11('arm');

mockjax1('armsum');
table2 = dataTable11('armsum');
...
function data(tableName) {
	...
	if (tableName == "arm") {
	    $.post("search_arm_tablecount?" + key + "=" + value,{Search:Search,
			cont_comd : cont_comd,
		...
		$.post("search_arm_table?" + key + "=" + value, {
			startPage : startPage,
			...
			for (var i = 0; i < j.length; i++) {
				...
				jsondata.push([ sr, j[i].unit_name, j[i].unit_sus_no, j[i].personnel_no,
					j[i].description, j[i].name ,j[i].arm_desc,j[i].date_of_trf]);
					...
					jsondata.push([ sr, j[i].unit_name, j[i].unit_sus_no, j[i].army_no,
						j[i].rank, j[i].full_name ,j[i].arm_desc,j[i].date_of_trf]);
	...
	if (tableName == "armsum") {
	    $.post("search_arm_summery_tablecount?" + key + "=" + value,{Search:Search,
			cont_comd : cont_comd,
			cont_corps : cont_corps,	
			cont_div : cont_div,
			cont_bde : cont_bde,
		...
		$.post("search_arm_summery_table?" + key + "=" + value, {
			startPage : startPage,
			pageLength : pageLength,
			Search:Search,
			orderColunm : orderColunm,
				...
				if(typeID=="1")
								...
								//	$('#th_or').css('display','block');
									table2.column(5).visible(true);
									 $("#th_or").show();
									offf_ttl = offf_ttl+j[i].total;
									jsondata.push([ sr, j[i].unit_name, j[i].unit_sus_no,j[i].jco,j[i].ors,j[i].total]); 
