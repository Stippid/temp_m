...
<div class="animated fadeIn">
	<div class="">
		<div class="col-md-12" align="center">
			<div class="card">
			    <div class="card-header" align="center"><h5>DETAILS OF NON EFFECTIVE</h5></div>
				<div class="card-body">
    					<div class="col-md-12">
    						<div class="col-md-6">
...
<!-- 										<option value = "0">--Select--</option> -->
		...
		<a href="Details_of_non_effective" class="btn btn-success btn-sm" >Clear</a>  
								...
								<table id="non_effective1"
									...
									<thead style="font-size: 15px; text-align: center;">
										<tr>
										    ...
										    <th rowSpan="2" >Cause of Non effective</th>
...
$(document).ready(function() {
	...
	mockjax1('non_effective');
	table = dataTable11('non_effective');
	mockjax1('non_effective1');
	table2 = dataTable11('non_effective1');
...
function data(tableName) {
	...
	var typeID = $("#typeID").val();
	var cause_of_non_effective = $("#cause_of_non_effective").val();
	var from_date = $("#from_date").val();
	var to_date = $("#to_date").val();
	var s_offrs = 0;	var s_jco = 0;	var s_ors = 0; var s_total = 0;
	var s_retired = 0;	var s_premature = 0;	var s_medical = 0; var s_deth = 0; var s_suicide = 0;
	
	
	if (tableName == "non_effective") {
	    $.post("search_non_effective_tablecount?" + key + "=" + value,{Search:Search,
			cont_comd : cont_comd,
			...
			typeID : typeID,
			cause_of_non_effective : cause_of_non_effective,
			from_date : from_date,
			to_date : to_date
		...
		$.post("search_non_effective_table?" + key + "=" + value, {
			startPage : startPage,
			...
			for (var i = 0; i < j.length; i++) {
				...
				jsondata.push([ sr, j[i].cmd_unit, j[i].corp_unit, j[i].div_unit,
					j[i].bde_unit, j[i].unit_name ,j[i].unit_sus_no,j[i].personnel_no, j[i].description, j[i].name,
					j[i].causes_name, j[i].date_of_non_effective ]);
					...
					jsondata.push([ sr, j[i].cmd_unit, j[i].corp_unit, j[i].div_unit,
						j[i].bde_unit, j[i].unit_name, j[i].unit_sus_no, j[i].army_no,
						j[i].rank, j[i].full_name ,j[i].causes_name,j[i].date_of_non_effective]);
	...
	if (tableName == "non_effective1") {
	    $.post("search_non_effective_summery_tablecount?" + key + "=" + value,{Search:Search,
			cont_comd : cont_comd,
			cont_corps : cont_corps,	
			cont_div : cont_div,
			cont_bde : cont_bde,
		...
		$.post("search_non_effective_summery_table?" + key + "=" + value, {
			startPage : startPage,
			pageLength : pageLength,
			Search:Search,
			orderColunm : orderColunm,
			...
			for (var i = 0; i < j.length; i++) {
				...
				if(typeID=="1")
								...
								jsondata.push([ sr,j[i].cmd_unit, j[i].corp_unit, j[i].div_unit,
									j[i].bde_unit, j[i].unit_name ,j[i].unit_sus_no,j[i].causes_name,j[i].officer,j[i].total,""]); 
								s_offrs +=  j[i].officer; s_total +=  j[i].total; 
									...
									offf_ttl = offf_ttl+j[i].total;
									jsondata.push([ sr,j[i].cmd_unit, j[i].corp_unit, j[i].div_unit,
										j[i].bde_unit, j[i].unit_name ,j[i].unit_sus_no,j[i].causes_name,j[i].jco,j[i].ors,j[i].total]); 
