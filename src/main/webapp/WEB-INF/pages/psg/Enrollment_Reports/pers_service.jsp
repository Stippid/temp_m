...
<div class="animated fadeIn">
	<div class="">
		<div class="col-md-12" align="center">
			<div class="card">
			    <div class="card-header" align="center"><h5>LIST OF PERS SERVICE</h5></div>
				<div class="card-body">
    					<div class="col-md-12">
    						<div class="col-md-6">
...
<!-- 										<option value = "0">--Select--</option> -->
					...
					<div class="col-md-12">
						...
						<div class="col-md-6">
							...
							<div class="row form-group">
								...
								<div class="col-12 col-md-8">
									<select id="serving" name="serving" class="form-control">
										<option value = "0">--Select--</option> 
										<option value = "below10">Below 10 Yrs</option>
										<option value = "11to20years">11 to 20 Yrs</option>
										<option value = "above20">Above 20 Yrs</option>
									</select>
								</div>
							</div>
						</div>
		...
		<a href="Pers_service" class="btn btn-success btn-sm" >Clear</a>  
	...
	$('#btn-reload').on('click', function(){
    	...
    	table2.ajax.reload(); 
    	 ...
    	 if(typeID == "2"){
             ...
             document.getElementById('th_officer').innerHTML='Officer';
...
function data(tableName) {
	...
	if (tableName == "service") {
	    $.post("search_service_tablecount?" + key + "=" + value,{Search:Search,
			cont_comd : cont_comd,
		...
		$.post("search_service_table?" + key + "=" + value, {
			startPage : startPage,
	...
	if (tableName == "servicesum") {
	    $.post("search_service_summery_tablecount?" + key + "=" + value,{Search:Search,
			cont_comd : cont_comd,
			cont_corps : cont_corps,	
			cont_div : cont_div,
			cont_bde : cont_bde,
		...
		$.post("search_service_summery_table?" + key + "=" + value, {
			startPage : startPage,
			pageLength : pageLength,
			Search:Search,
			orderColunm : orderColunm,
			...
			for (var i = 0; i < j.length; i++) {
				...
				if(typeID=="1")
								...
								offf_ttl = offf_ttl+j[i].total;
								jsondata.push([ sr, j[i].unit_name, j[i].unit_sus_no,j[i].serving,j[i].officer,j[i].total,""]); 
								s_offrs +=  j[i].officer;
								s_total +=  j[i].total;
							...
							//		$('#th_or').css('display','block');
									...
									offf_ttl = offf_ttl+j[i].total;
									jsondata.push([ sr, j[i].unit_name, j[i].unit_sus_no,j[i].serving,j[i].jco,j[i].ors,j[i].total]); 
									s_jco +=  j[i].jco;
									s_or +=  j[i].ors;
									s_total +=  j[i].total;
								}
			...
			jsondata.push(["","","","TOTAL",s_jco, s_or, s_total, ""]);
			}
		});
