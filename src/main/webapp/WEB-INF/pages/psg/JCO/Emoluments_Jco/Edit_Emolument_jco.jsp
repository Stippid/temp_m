...
<form:form name="Edit_Emolument" id="Edit_Emolument" action="Edit_Emolument_Jco_Action"
	method="post" class="form-horizontal" commandName="Edit_Emolument_Jco_CMD">
	 
	<div class="animated fadeIn">
		<div class="col-md-12" align="center">
			<div class="card">
				<div class="card-header">
					<h5>EDIT EMOLUMENTss</h5>
...
<%-- 							</c:if>			  --%>
				...
				<!-- 	<a href="Emolument" class="btn btn-success btn-sm">Cancel</a> -->
					<input type="submit" class="btn btn-primary btn-sm" value="Update"  id="update" onclick="return validate();">
					<a href="Search_Emoluments_jco"><input type="button" class="btn btn-danger btn-sm" value="Cancel"></a>
...
function m_emoDetails() {
	...
	$.post('getm_emu_JCOdetailsDataListEdit?' + key + "=" + value,{comm_id:comm_id},function(j) {
...
function personal_number()
{
	 ...
	 var personnel_no = $("input#army_no").val();
	 var data_result=true; 
		
     $.post("getPersonnelNoAlreadyExistemJCO?"+key+"="+value, {personnel_no : personnel_no}).done(function(j) {
		 	if(j == false){
		 		
				alert("Army No already Exist please either edit/Update Data .")
				$("input#army_no").val(" ");
				$("#name").text(" ");
		    	$("#rank").text(" ");
		    	$("#unit").text(" ");
	 ...
	 $.post('GetPersonnelNoDataJcoEmoul?' + key + "=" + value,{personnel_no:personnel_no },function(h) {
		 ...
		 var susNoAuto=$("#army_no");
		  susNoAuto.autocomplete({
		      source: function( request, response ) {
		        $.ajax({
		        	type: 'POST',
			    	url: "getpersonnel_no_JcoList_BA_PY_CA?"+key+"="+value,
		        data: {personel_no:personel_no},
		          success: function( data ) {
		        	 var susval = [];
		        	  var length = data.length-1;
...
function Rejectdata(){
    var comm_id = $('#jco_id').val()
     $.post('Reject_eno_Jco?' + key + "=" + value, {comm_id:comm_id}, function(j){
              alert(j);
...
var comm_id = $('#jco_id').val()
     $.post('approve_eno_jco?' + key + "=" + value, {comm_id:comm_id}, function(j){
              alert(j);
              window.location.href="Search_Emoluments_jco";
      });
}
