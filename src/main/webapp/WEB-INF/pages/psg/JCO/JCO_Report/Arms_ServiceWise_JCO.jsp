...
<form action="" method="post" enctype="multipart/form-data" class="form-horizontal"> 
	<div class="animated fadeIn">
	    	...
	    	<div class="col-md-12" align="center">
	    		<div class="card">
	    			<div class="card-header"><h5>HELD STR OF JCO/OR ARM/ SERVICE WISE</h5></div>
	    			<div class="card-header"><h5>Report No. MISO/JCO/OR/55       Report Generated On:${date}  </h5></div>
	          			
		    			
		<div class="col-md-12">
			<table id="ArmServices_Wise_JCO_reporttbl" class="display table no-margin table-striped  table-hover table-bordered ">
				<thead>
			        <tr>
...
<c:url value="Download_ArmServices_Wise_JCO_query" var="dwonloadUrl"/>
	<form:form action="${dwonloadUrl}" method="post" id="downloadForm" name="downloadForm" >	
</form:form>

<c:url value="Excel_ArmServices_Wise_query_JCO" var="excelUrl" />
...
$(document).ready(function() {  
	...
	mockjax1('ArmServices_Wise_JCO_reporttbl');
	table = dataTable('ArmServices_Wise_JCO_reporttbl');
...
function data(tableName){
	if(tableName == "ArmServices_Wise_JCO_reporttbl"){
		jsondata = [];
		...
		$.post("getArmServices_Wise_Report_JCODataList?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType,
			},function(j) {
				
			for (var i = 0; i < j.length; i++) {
				jsondata.push([j[i].arm_service,j[i].arm_desc,j[i].jan,j[i].feb,j[i].mar,j[i].apr,
		...
		$.post("getArmServices_Wise_JCO_TotalCount?"+key+"="+value,{Search:Search},function(j) {
			FilteredRecords = j;
		});
	}
}	
