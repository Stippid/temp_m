...
<div class="animated fadeIn">
	    	<div class="container-fluid" align="center">
	    		<div class="card">
	    		<div class="card-header"><h5>Approve Cancelled History of Field Service</h5> <h6 class="enter_by"><span style="font-size:12px;color:red;">(To be entered by UNIT)</span></h6></div>
...
<div id="Field_Service" class="tabcontent"  style="display:block;">
  	<div class="card" style="margin-top:20px;">
			   ...
			   <tbody data-spy="scroll" id="field_service_tbody" style="min-height:46px; max-height:650px; text-align: center;">
					...
					<c:if test="${item.cancel_status==0}">
					...
					<input type="hidden" class="datefrom"  value="${item.from_date}"/>
						...
						<td>
							<a class="btn btn-danger btn-sm"  title = "REJECT"  onclick="RejectCancelHistoryChData(${item.id});"><i class="fa fa-times"></i></a>
						</td>
						</tr>
						</c:if>
						<c:if test="${item.cancel_status!=0}">
...
<div id="submit_data" class="card-footer" align="center" class="form-control" >
	<a href=Search_field_service_Jco_url id="serviceUrl"	class="btn btn-success btn-sm">Back</a>
	<input type="button" class="btn btn-info btn-sm" onclick="if (confirm('Are You Sure You Want to Approve This Data?') ){ApprCancelHistoryData();}else{ return false;}" value="Approve">
	<input type="button" class="btn btn-info btn-sm" onclick="if (confirm('Are You Sure You Want to Reject This Data?') ){RejCancelHistoryData();}else{ return false;}" value="Reject">  
</div>

<c:url value="Appr_Cancel_field_service_dat_Jco" var="cancelHisytoryUrl" />
...
<c:url value="Reject_Cancel_field_service_dat_Jco" var="RejcancelHisytoryUrl" />
<form:form action="${RejcancelHisytoryUrl}" method="post" id="RejCancelHistoryForm" name="RejCancelHistoryForm" >
<input type="hidden" name="${_csrf.parameterName}" id="rej${_csrf.parameterName}" value="${_csrf.token}"/>
	<input type="hidden" name="id_rj" id="id_rj" value="0"/> 
<!-- 	<input type="hidden" name="cat_rj" id="cat_rj" value="0"/> -->
</form:form>

<c:url value="RejectCh_Cancel_field_service_dat_Jco" var="RejChcancelHisytoryUrl" />
<form:form action="${RejChcancelHisytoryUrl}" method="post" id="RejChCancelHistoryForm" name="RejChCancelHistoryForm" >
<input type="hidden" name="${_csrf.parameterName}" id="rejch${_csrf.parameterName}" value="${_csrf.token}"/>
<input type="hidden" name="id_rjP" id="id_rjP" value="0"/> 
	<input type="hidden" name="id_rjCh" id="id_rjCh" value="0"/> 
<!-- 	<input type="hidden" name="cat_rjCh" id="cat_rjCh" value="0"/> -->
</form:form>
...
function ApprCancelHistoryData(){
...
function RejCancelHistoryData(){
	$("#id_rj").prop("disabled",false);
// 	$("#cat_rj").prop("disabled",false);
	$("#rej"+'${_csrf.parameterName}').prop("disabled",false);
	$("#id_rj").val($("#p_id").val());
// 	$("#cat_rj").val($("#service_category").val()) ;	
	document.getElementById('RejCancelHistoryForm').submit();
	
}

function RejectCancelHistoryChData(ch_id){
	$("#id_rjP").prop("disabled",false);
	$("#id_rjCh").prop("disabled",false);
// 	$("#cat_rjCh").prop("disabled",false);
	$("#rejch"+'${_csrf.parameterName}').prop("disabled",false);
	$("#id_rjP").val($("#p_id").val());
	$("#id_rjCh").val(ch_id);
// 	$("#cat_rjCh").val($("#service_category").val()) ;	
	document.getElementById('RejChCancelHistoryForm').submit();
