...
<form action="Edit_Posting_Out_JCOAction?${_csrf.parameterName}=${_csrf.token}" method="post" class="form-horizontal" >
	...
	function personal_number() {
		...
		if (personnel_no != "") {
			$.post('GetArmyNoData?' + key + "=" + value, {army_no:personnel_no}, function(j) {
				...
				setMinDate('dt_of_sos', enroll_dt);
				setMinDate('out_auth_dt', enroll_dt);
				$("#jco_id").val(jco_id);
...
var tsus=$("input#to_sus_no").val();
	...
	if(enroll_dt!=''){
		if(getformatedDate(out_auth_dt) < getformatedDate(enroll_dt)) {
			alert("Date of Auth Should Be Greater Then Date of Enroll");
			$("input#out_auth_dt").focus();
			return false;
		}
	}
	...
	if(enroll_dt!=''){
		if(getformatedDate($("input#dt_of_sos").val()) < getformatedDate(enroll_dt)) {
			alert("SOS Should Be Greater Then Date of Enroll");
			$("input#dt_of_sos").focus();
			return false;
		}
	}		
