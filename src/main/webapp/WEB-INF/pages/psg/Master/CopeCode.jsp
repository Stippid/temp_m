...
<form:form name="copecode" id="copecode" action="copecodeAction" method="post" class="form-horizontal" commandName="copecodeCMD"> 
	<div class="animated fadeIn">
	    	<div class="container" align="center">
	    		<div class="card">
	    		<div class="card-header"><h5>ADD/UPDATE COPE CODE</h5> <h6 class="enter_by"><span style="font-size:12px;color:red;">(To be entered by MISO)</span></h6></div>
	              				...
	              				<div class="col-md-6">
	              					<div class="row form-group">
						               <div class="col-md-4">
						                ...
						                <div class="col-md-8">
						                   <input type="text" id="value" name="value" oninput="this.value = this.value.toUpperCase()" maxlength="100"  class="form-control autocomplete" autocomplete="off" > 
						                </div>
						            </div>
	              				</div>
								</div>	
						...
						<div class="card-footer" align="center" class="form-control">
							<a href="CopecodeUrl" class="btn btn-success btn-sm" >Clear</a> 
		           ...
		           <table id="getbankSearch" class="table no-margin table-striped  table-hover  table-bordered report_print">
		                 <thead style="text-align: center;">
		                       <tr>
									...
									<th style="font-size: 15px;">Cope Code</th>
									<th style="font-size: 15px;">Value & Description</th>
			                 ...
			                 <c:if test="${list.size()==0}">
		                       ...
		                       <c:forEach var="item" items="${list}" varStatus="num" >
									<tr>
										...
										<td style="font-size: 15px;">${item[1]} - ${item[2]}</td>
										<td style="font-size: 15px;text-align: center ;">${item[3]} ${item[4]}</td> 
									</tr>
								</c:forEach>
		                     </tbody>
		                 </table>
...
<c:url value="getcopecodeSearch" var="searchUrl" />
<form:form action="${searchUrl}" method="post" id="searchForm" name="searchForm" modelAttribute="copeCode1">
		<input type="hidden" name="copeCode1" id="copeCode1" />
...
<c:url value="Edit_copeCode" var="editUrl"/>
<form:form action="${editUrl}" method="post" id="updateForm" name="updateForm" modelAttribute="id2">
	  <input type="hidden" name="id2" id="id2">
</form:form>

<c:url value="deleteCopeCodeURL" var="deleteUrl" />
<form:form action="${deleteUrl}" method="post" id="deleteForm" name="deleteForm" modelAttribute="id1">
	<input type="hidden" name="id1" id="id1" value="0"/> 
</form:form>
<c:url value="CopeCodereport" var="searchUrl" />
...
function Search(){
	...
	if($('#cope').val() == "0" && $('#status').val() == "0" && $('#value').val() == "" && $('#description').val() == ""){
		alert("Please Enter Something to Search");
		$('#cope').focus();
		return  false;
	}
	
	    $("#copeCode1").val($('#cope').val());
	    $("#value1").val($('#value').val());
	    $("#decription1").val($('#decription').val());
