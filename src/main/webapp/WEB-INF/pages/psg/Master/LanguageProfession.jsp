...
<form:form name="LanguageProf" id="LanguageProf" action="LanguageProfAction" method="post" class="form-horizontal" commandName="LanguageProfCmd"> 
	<div class="animated fadeIn">
	    	<div class="container" align="center">
	    		<div class="card">
	    		<div class="card-header"><h5>ADD/UPDATE LANGUAGE PROFICIENCY</h5> 
	    		<h6 class="enter_by"><span style="font-size:12px;color:red;">(To be entered by MISO)</span></h6></div>
	            			...
	            			<div class="col-md-12">	              					
	              				<div class="col-md-6">
	              					<div class="row form-group">
						               <div class="col-md-4">
						                    <label class=" form-control-label"><strong style="color: red;">* </strong>LANGUAGE PROFICIENCY </label>
						                </div>
						                <div class="col-md-8">
						                   <input type="text" id="language" name="language" oninput="this.value = this.value.toUpperCase()" maxlength="50" class="form-control autocomplete" autocomplete="off" onkeypress="return onlyAlphabets(event);"> 
						                </div>
						            </div>
	              				</div>
	              				<div class="col-md-6">
						...
						<div class="card-footer" align="center" class="form-control">
							<a href="LanguageProfession" class="btn btn-success btn-sm">Clear</a> 
		                      ...
		                      <tbody>
		                           <c:forEach var="item" items="${list}" varStatus="num" >
							          <tr>
                                        ...
                                        <td >${item[0]}</td>
                                        <td id="thAction1" style="text-align:center;width: 20%">${item[2]} ${item[3]}</td>
							          </tr>
							       </c:forEach>
		                     </tbody>
...
<c:url value="search_LanguageProf" var="searchUrl" />
	<form:form action="${searchUrl}" method="post" id="searchForm" name="searchForm" modelAttribute="language1">
	 ...
	 <c:url value="edit_LanguageProf" var="updateUrl" />
<form:form action="${updateUrl}" method="post" id="updateForm" name="updateForm" modelAttribute="id" >
	<input type="hidden" name="updateid" id="updateid" value="0" />
</form:form> 
<c:url value="languageProf_delete" var="deleteUrl" />
	<form:form action="${deleteUrl}" method="post" id="deleteForm" name="deleteForm" modelAttribute="id1">
		<input type="hidden" name="id1" id="id1" value="0"/> 
	</form:form>
 <c:url value="LangProfreport" var="searchUrl" />
...
function Validate(){
	if ($("input#language").val().trim() == "") {
		alert("Please Enter Language Proficiency");
...
function Search(){
	...
	if ($("input#language").val() == "" && $("#status").val() == "0") {
