...
<form:form name="Personnel_type" id="Personnel_type" action="Personnel_typeAction" method="post" class="form-horizontal" commandName="Personnel_typeCMD"> 
	<div class="animated fadeIn">
	    	<div class="container" align="center">
	    		<div class="card">
	    		<div class="card-header"><h5>ADD/UPDATE PERSONNEL TYPE</h5> <h6 class="enter_by"><span style="font-size:12px;color:red;">(To be entered by MISO)</span></h6></div>
...
<!-- 												<option value="0">--Select--</option> -->
						...
						<div class="card-footer" align="center" class="form-control">
							<a href="Personnel_type" class="btn btn-success btn-sm" >Clear</a>  
...
<div class="container" id="getpers_tySearch" style="display: block;"> 	 
			<div  class="watermarked" data-watermark="" id="divwatermark"><span id="ip"></span>
		           <table id="getpers_tySearch" class="table no-margin table-striped  table-hover  table-bordered report_print">
		                 <thead style="text-align: center;">
		                       <tr>
			                        <th style="font-size: 15px ;width: 10%;">Ser No</th>
									<th style="font-size: 15px">Personnel Name</th>
...
<c:url value="getSearchPersonnel_typeMaster" var="searchUrl" />
<form:form action="${searchUrl}" method="post" id="searchForm" name="searchForm" modelAttribute="rank_type1">
		<input type="hidden" name="personnel_name1" id="personnel_name1" />
		<input type="hidden" name="status1" id="status1" value="0" />
</form:form> 

<c:url value="Edit_Personnel_type" var="editUrl"/>
<form:form action="${editUrl}" method="post" id="updateForm" name="updateForm" modelAttribute="id2">
	  <input type="hidden" name="id2" id="id2">
</form:form>

<c:url value="deletepersonnel_typeMasterURL" var="deleteUrl" />
<form:form action="${deleteUrl}" method="post" id="deleteForm" name="deleteForm" modelAttribute="id1">
	<input type="hidden" name="id1" id="id1" value="0"/> 
</form:form>
<c:url value="PersonnelTypereport" var="searchUrl" />
...
$(document).ready(function() {
	 ...
	 if('${list.size()}' == ""){
		   $("div#getpers_tySearch").hide();
		 }
	
	 if('${personnel_name1}'== null || '${personnel_name1}'=='')
		{
			$("#personnel_name").val('') ;
		}else
		{
			$("#personnel_name").val('${personnel_name1}') ;
		} 
	 if('${status1}' != ""){
...
function Search(){
		...
		$("#personnel_name1").val($('#personnel_name').val());
