...
<form:form name="VisitPurpose" id="VisitPurpose" action="VisitPurposeAction" method="post" class="form-horizontal" commandName="VisitPurposeCMD"> 
	<div class="animated fadeIn">
	    	<div class="container" align="center">
	    		<div class="card">
	    		<div class="card-header"><h5>ADD/UPDATE PURPOSE OF VISIT</h5> <h6 class="enter_by"><span style="font-size:12px;color:red;">(To be entered by MISO)</span></h6></div>
	          			<div class="card-body card-block">
	            			<div class="col-md-12">	              					
	              				<div class="col-md-6">
	              					<div class="row form-group">
						               <div class="col-md-4">
						                    <label class=" form-control-label"><strong style="color: red;">* </strong> VISIT PURPOSE</label>
						                </div>
						                <div class="col-md-8">
						                   <input type="text" id="visitpurpose" name="visitpurpose" oninput="this.value = this.value.toUpperCase()" maxlength="50" class="form-control autocomplete" autocomplete="off" > 
						                </div>
						            </div>
	              				</div>
						...
						<div class="card-footer" align="center" class="form-control">
							<a href="visitPurposeUrl" class="btn btn-success btn-sm" >Clear</a> 
		           ...
		           <table id="getbankSearch" class="table no-margin table-striped  table-hover  table-bordered report_print">
		                 <thead style="text-align: center;">
		                       <tr>
									...
									<th style="font-size: 15px">Visit Purpose</th>
...
<c:url value="getVisitPurposeSearch" var="searchUrl" />
<form:form action="${searchUrl}" method="post" id="searchForm" name="searchForm" modelAttribute="visitpurpose1">
		<input type="hidden" name="visitpurpose1" id="visitpurpose1" />
		<input type="hidden" name="status1" id="status1" />
</form:form> 

<c:url value="Edit_VisitPurpose" var="editUrl"/>
<form:form action="${editUrl}" method="post" id="updateForm" name="updateForm" modelAttribute="id2">
	  <input type="hidden" name="id2" id="id2">
</form:form>

<c:url value="deleteVisitURL" var="deleteUrl" />
<form:form action="${deleteUrl}" method="post" id="deleteForm" name="deleteForm" modelAttribute="id1">
	<input type="hidden" name="id1" id="id1" value="0"/> 
</form:form>
<c:url value="VisitPurposereport" var="searchUrl" />
...
$(document).ready(function() {
	$("#visitpurpose").val('${visitpurpose1}');
	if('${status1}' != '')
	$("#status").val('${status1}');
});
function getExcel() {
...
function validate(){
	if ($("input#visitpurpose").val().trim() == "") {
		alert("Please Enter Visit Purpose");
		$("input#visitpurpose").focus();
		return false;
	}
...
function Search(){
	...
	if($('#visitpurpose').val() == "" && $('#status').val() == "0"){
		alert("Please Enter Something to Search");
		$('#visitpurpose').focus();
		return  false;
	}
	
	    $("#visitpurpose1").val($('#visitpurpose').val());
