...
<div class="" id="Canteen_History" style="display: block;"> 	 
			<div  class="watermarked" data-watermark="" id="divwatermark"><span id="ip"></span>
		           <table id="Canteen_History" class="table no-margin table-striped  table-hover  table-bordered">
		                 <thead style="text-align: center;">
		                       <tr>
			                        <th style="font-size: 15px ;">Ser No</th>
			                        <th style="font-size: 15px">Relation</th>
			                        <th style="font-size: 15px">Name</th>
			                        <th style="font-size: 15px">Date of Birth</th>
			                        <th style="font-size: 15px">Type of Card</th>
			                        <th style="font-size: 15px">Card No</th>
			                 ...
			                 <c:if test="${list.size()==0}">
		                       ...
		                       <c:forEach var="item" items="${list}" varStatus="num" >
									<tr>
										...
										<td style="font-size: 15px;">${item.label}</td>
										<td style="font-size: 15px;">${item.name}</td>
										<td style="font-size: 15px;">${item.date_of_birth}</td>
										<td style="font-size: 15px;">${item.type_of_card}</td>
										<td style="font-size: 15px;">${item.card_no}</td>
									    ...
									    <td style="font-size: 15px;">${item.modified_date}</td>
									</tr>
								</c:forEach>
		                     </tbody>
		                 </table>
