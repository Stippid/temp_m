...
<form action="" method="post" enctype="multipart/form-data" class="form-horizontal"> 
	<div class="animated fadeIn">		
	    	<div class="col-md-12" align="center">
	    		<div class="card">
	    			<div class="card-header"><h5>NUMBERS OF PERS AGE/RELIGION/STATE WISE</h5></div>
	          			<div class="card-body">
		          			<div class="col-md-12">
								<div class="col-md-6">
						...
						<div class="card-footer" align="center">
							<a href="Age_Religion_StateWise_Url" type="reset" class="btn btn-success btn-sm"> Clear </a> 
	...
	<div class="card-header"> 
		...
		<div class="col-md-12">
			<table id="NumbersOF_Pers_Total_List_reporttbl" class="display table no-margin table-striped  table-hover table-bordered ">
				<thead>
			        <tr>					
						<th>Unit</th>
						<!-- <th>SUS No</th> -->
		...
		<div class="col-md-12" id="getSearch_Letter" style="display: block;">
	<div class="card-header"> 
		<h5> RELIGION WISE DETAILS </h5>				
	</div>
	</div>		
		<div class="col-md-12">
			<table id="Religion_Wise_Total_List_reporttbl" class="display table no-margin table-striped  table-hover table-bordered ">
				<thead>
			        <tr>
						<th>Religion</th>
						<th>Offrs</th>
		...
		<div class="col-md-12" id="getSearch_Letter" style="display: block;">
<div class="card-header"> 
		<h5> AGE WISE DETAILS </h5>				
	</div>	
	</div>
		<div class="col-md-12">
			<table id="Age_Wise_Total_List_reporttbl_pers" class="display table no-margin table-striped  table-hover table-bordered ">
				<thead>
			        <tr>
						<th>Age (Yrs)</th>
						<th>Offrs</th>
		...
		<div class="col-md-12" id="getSearch_Letter" style="display: block;">
	<div class="card-header"> 
		<h5> STATE WISE DETAILS </h5>				
	</div>
	</div>					
		<div class="col-md-12">
			<table id="State_Wise_Total_List_reporttbl_pers" class="display table no-margin table-striped  table-hover table-bordered ">
				<thead>
			        <tr>
						<th>State</th>
						<th>Offrs</th>
...
<c:url value="Download_Age_Religion_StateWise_query" var="dwonloadUrl"/>
...
<c:url value="Excel_Age_Religion_StateWise_query" var="excelUrl" />
...
$(document).ready(function() {  
	...
	var select = '<option value="' + "0" + '">'+ "--Select--" + '</option>';
   	$('select#cont_comd').change(function() {
	   	...
	   	mockjax1('NumbersOF_Pers_Total_List_reporttbl');
		table = dataTable('NumbersOF_Pers_Total_List_reporttbl');
		...
		mockjax1('Religion_Wise_Total_List_reporttbl');
		table1 = dataTable('Religion_Wise_Total_List_reporttbl');
		$('#btn-reload').on('click', function(){
			table1.ajax.reload();
		});
		$.ajaxSetup({
		...
		mockjax1('Age_Wise_Total_List_reporttbl_pers');
		table2 = dataTable('Age_Wise_Total_List_reporttbl_pers');
		$('#btn-reload').on('click', function(){
			table2.ajax.reload();
		});
		$.ajaxSetup({
		...
		mockjax1('State_Wise_Total_List_reporttbl_pers');
		table3 = dataTable('State_Wise_Total_List_reporttbl_pers');
		$('#btn-reload').on('click', function(){
			table3.ajax.reload();
		});
		$.ajaxSetup({
...
function data(tableName){
	if(tableName == "NumbersOF_Pers_Total_List_reporttbl"){
		jsondata = [];
		...
		$.post("getNumbersOF_Pers_Total_List_ReportDataList?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType,
			...
			for (var i = 0; i < j.length; i++) {
				jsondata.push([j[i].unit_name, 
					/* j[i].unit_sus_no, */
					j[i].army_no,j[i].description,j[i].name,j[i].religion,j[i].age,j[i].state]);
			}
		});
		$.post("getNumbersOF_Pers_Total_List_TotalCount?"+key+"="+value,{Search:Search,cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,
			cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no},function(j) {
			FilteredRecords = j;
		});
	}	
	if(tableName == "Religion_Wise_Total_List_reporttbl"){
		jsondata = [];
		var table = $('#'+tableName).DataTable();
		var info = table.page.info();
		...
		$.post("getReligion_Wise_Total_List_ReportDataList?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType,
			cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no},function(j) {
			
				for (var i = 0; i < j.length; i++) {
					jsondata.push([j[i].religion,j[i].offr,j[i].jco,j[i].or1,j[i].total]);
				}
		});
		$.post("getReligion_Wise_Total_List_TotalCount?"+key+"="+value,{Search:Search,cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,
			cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no},function(j) {
			FilteredRecords = j;
		});
	}
	if(tableName == "Age_Wise_Total_List_reporttbl_pers"){
		jsondata = [];
		var table = $('#'+tableName).DataTable();
		var info = table.page.info();
		...
		$.post("getAge_Wise_Total_List_ReportDataList_pers?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType,
			cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no},function(j) {
					
			for (var i = 0; i < j.length; i++) {
				jsondata.push([j[i].age, j[i].offr,j[i].jco, j[i].or1,j[i].total]);
			}
		});
		$.post("getAge_Wise_Total_List_TotalCount_pers?"+key+"="+value,{Search:Search,cont_comd:cont_comd,cont_corps:cont_corps,
			cont_div:cont_div,cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no},function(j) {
			FilteredRecords = j;
		});
	}
	if(tableName == "State_Wise_Total_List_reporttbl_pers"){
		jsondata = [];
		var table = $('#'+tableName).DataTable();
		var info = table.page.info();
		...
		$.post("getState_Wise_Total_List_ReportDataList_pers?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType,
			cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no},function(j) {
			
				for (var i = 0; i < j.length; i++) {
					jsondata.push([j[i].state, j[i].offr,j[i].jco, j[i].or1,j[i].total]);
				}
		});
		$.post("getState_Wise_Total_List_TotalCount_pers?"+key+"="+value,{Search:Search,cont_comd:cont_comd,cont_corps:cont_corps,
			cont_div:cont_div,cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no},function(j) {
			FilteredRecords = j;
		});
	}
