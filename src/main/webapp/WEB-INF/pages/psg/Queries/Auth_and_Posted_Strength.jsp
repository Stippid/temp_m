...
<form action="" method="post" enctype="multipart/form-data" class="form-horizontal"> 
	<div class="animated fadeIn">
	    	...
	    	<div class="col-md-12" align="center">
	    		<div class="card">
	    			<div class="card-header"><h5>Auth and Posted Strength Search Report</h5></div>
						...
						<div class="card-footer" align="center">
							<a href="Auth_and_Posted_Strength" type="reset" class="btn btn-success btn-sm"> Clear </a> 
		...
		<div class="col-md-12">
			<table id="Auth_and_Posted_Strength" class="display table no-margin table-striped  table-hover  table-bordered ">
				<thead>
					<tr>
							<th colspan="6" style="text-align: center;"></th>
							<th colspan="2" style="text-align: center;">Offrs</th>
							<th colspan="2" style="text-align: center;">JCOs </th>
							<th colspan="2" style="text-align: center;">ORs </th>
							<th colspan="2" style="text-align: center;">CIV </th>
...
<c:url value="Download_Auth_Posted_query" var="dwonloadUrl"/>
...
$(document).ready(function() {  
	...
	var select = '<option value="' + "0" + '">'+ "--Select--" + '</option>';
   	$('select#cont_comd').change(function() {
	   	...
	   	mockjax1('Auth_and_Posted_Strength');
		table = dataTable('Auth_and_Posted_Strength');
...
function data(tableName){
	 ...
	 var s_total=0; var j_total=0;
	
		$.post("getAuth_and_Posted_StrengthReportDataList?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType,
			cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no,blood_group:blood_group},function(j) {
			
			for (var i = 0; i < j.length; i++) {
				var sr = i+1;
				jsondata.push([sr,j[i].cmd_unit, j[i].corp_unit,j[i].div_unit,j[i].bde_unit,j[i].unit_name,/* j[i].unit_sus_no, */
					j[i].off_total_auth,j[i].offr,j[i].jco_total_auth,j[i].jco,j[i].or_total_auth,j[i].or1,j[i].civ_total_auth,j[i].civ,j[i].auth_total,j[i].held_total]);
				s_total+=j[i].auth_total;
				j_total+=j[i].held_total;
			}
			jsondata.push(["","" ,"","","","","","","","","","","","Total",s_total,j_total]);
		});
		$.post("getAuth_and_Posted_StrengthTotalCount?"+key+"="+value,{Search:Search,cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no,blood_group:blood_group},function(j) {
			FilteredRecords = j;
		});
...
function downloaddata(){debugger;
