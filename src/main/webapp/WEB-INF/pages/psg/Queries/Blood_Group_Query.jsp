...
<form action="" method="post" enctype="multipart/form-data"
		...
		<div class="col-md-12" align="center">
			<div class="card">
				<div class="card-header">
					<h5>DETAIL OF BLOOD GPS OF PERS</h5>
				</div>
				...
				<div class="card-footer" align="center">
					<a href="Blood_Group_Query" type="reset"
	...
	<div class="col-md-12">
		<table id="Blood_group_reporttbl"
	...
	<div class="col-md-12" id="getSearch_Letter" style="display: block;">
		<div class="card-header">
			<h5>Unit Wise Nominal Roll with Blood Gp</h5>
		</div>
	</div>

	<div class="col-md-12">
		<table id="Blood_group_reporttbl_pers"
			class="display table no-margin table-striped  table-hover table-bordered ">
			<thead>
				<tr>
					<th>Unit</th>
...
<c:url value="Download_Blood_Group_query" var="dwonloadUrl" />
<form:form action="${dwonloadUrl}" method="post" id="downloadForm"
	name="downloadForm" modelAttribute="cont_comd_dn">
	...
	<input type="hidden" name="blood_group_tx" id="blood_group_tx">
	<input type="hidden" name="unit_name_dn" id="unit_name_dn">
	<input type="hidden" name="sus_no_dn" id="sus_no_dn">
	<input type="hidden" name="blood_group_dn" id="blood_group_dn"
		value="0">
</form:form>

<c:url value="Excel_Blood_Group_query" var="excelUrl" />
<form:form action="${excelUrl}" method="post" id="ExcelForm"
	...
	<input type="hidden" name="blood_group_txt" id="blood_group_txt">
	<input type="hidden" name="unit_name_ex" id="unit_name_ex">
	<input type="hidden" name="sus_no_ex" id="sus_no_ex">
	<input type="hidden" name="blood_group_ex" id="blood_group_ex"
		value="0">
...
<script>
	$(document).ready(
			function() {
				...
				mockjax1('Blood_group_reporttbl');
				table = dataTable('Blood_group_reporttbl');
				...
				mockjax1('Blood_group_reporttbl_pers');
				table1 = dataTable('Blood_group_reporttbl_pers');
				$('#btn-reload').on('click', function() {
					table1.ajax.reload();
				});
				$.ajaxSetup({
	...
	function data(tableName) {
		if (tableName == "Blood_group_reporttbl") {
			jsondata = [];
			...
			$.post("getBloodGroupReportDataList?" + key + "=" + value, {
				startPage : startPage,
				...
				for (var i = 0; i < j.length; i++) {
					jsondata.push([ j[i].cmd_unit, j[i].corp_unit,
							j[i].div_unit, j[i].bde_unit, j[i].unit_name,
							j[i].blood_desc, j[i].offr,
							j[i].jco, j[i].or1, j[i].total ]);
				}
			});
			$.post("getBloodGroupTotalCount?" + key + "=" + value, {
				Search : Search,
				cont_comd : cont_comd,
				cont_corps : cont_corps,
				cont_div : cont_div,
		...
		if (tableName == "Blood_group_reporttbl_pers") {
			jsondata = [];
			var table = $('#' + tableName).DataTable();
			var info = table.page.info();
			...
			$.post("getBloodGroupReportDataList_pers?" + key + "=" + value, {
				startPage : startPage,
				pageLength : pageLength,
				Search : Search,
				orderColunm : orderColunm,
				...
				for (var i = 0; i < j.length; i++) {
					jsondata.push([ j[i].unit_name,
							j[i].army_no, j[i].description, j[i].name,
							j[i].blood_desc ]);
				}
			});
			$.post("getBloodGroupTotalCount_pers?" + key + "=" + value, {
				Search : Search,
				cont_comd : cont_comd,
				cont_corps : cont_corps,
				cont_div : cont_div,
...
<script>
	...
	function downloaddata() {
		...
		var blood_group_tx = $("#blood_group option:selected").text();
		...
		$("#blood_group_tx").val(blood_group_tx);
		...
		$("#blood_group_dn").val(blood_group);
	...
	function getExcel() {
		...
		var blood_group_txt = $("#blood_group option:selected").text();
		...
		$("#blood_group_txt").val(blood_group_txt);
		...
		$("#blood_group_ex").val(blood_group);
