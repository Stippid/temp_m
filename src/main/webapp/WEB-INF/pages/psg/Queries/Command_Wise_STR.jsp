...
<form action="" method="post" enctype="multipart/form-data" class="form-horizontal"> 
	<div class="animated fadeIn">		
	    	<div class="col-md-12" align="center">
	    		<div class="card">
	    			<div class="card-header"><h5>COMMAND WISE STR AS ON: ${date}</h5></div>	          											      
 				</div> 
	        </div>
	</div>						
		<div class="col-md-12">
			<table id="Command_Wise_STR_reporttbl" class="display table no-margin table-striped  table-hover table-bordered ">
			<thead>
				<tr>
					<th>Command</th>
					<th>Auth</th>
...
<c:url value="Download_Command_Wise_STR_query" var="dwonloadUrl"/>
	<form:form action="${dwonloadUrl}" method="post" id="downloadForm" name="downloadForm" >
</form:form>

<c:url value="Excel_Command_Wise_STR_query" var="excelUrl" />
...
$(document).ready(function() {  
	...
	mockjax1('Command_Wise_STR_reporttbl');
	table = dataTable('Command_Wise_STR_reporttbl');
...
function data(tableName){
	if(tableName == "Command_Wise_STR_reporttbl"){
		jsondata = [];
		...
		$.post("getCommand_Wise_STR_ReportDataList?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType
			},function(j) {
		...
		$.post("Command_Wise_STR_TotalCount?"+key+"="+value,{Search:Search},function(j) {
			FilteredRecords = j;
		});
	}		
}	
