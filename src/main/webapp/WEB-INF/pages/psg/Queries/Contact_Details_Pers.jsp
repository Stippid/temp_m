...
<form action="" method="post" enctype="multipart/form-data" class="form-horizontal"> 
	<div class="animated fadeIn">
	    	...
	    	<div class="col-md-12" align="center">
	    		<div class="card">
	    			<div class="card-header"><h5>CONTACT DETAILS OF PERSON</h5></div>
						...
						<div class="card-footer" align="center">
							<a href="Contact_Details_PersUrl" type="reset" class="btn btn-success btn-sm"> Clear </a> 
		...
		<div class="col-md-12">
			<table id="Contact_detailsof_pers_reporttbl" class="display table no-margin table-striped  table-hover table-bordered ">
				<thead>
			        <tr>
						<th>Sr No. </th>
						<th>Comd </th>
...
<!-- 						<th>SUS No </th> -->
		...
		<div class="col-md-12">
			<table id="SummaryOf_Total_Number_reporttbl_pers" class="display table no-margin table-striped  table-hover table-bordered ">
				<thead>
			        <tr>
						<th>Sr No. </th>
						<th>Comd </th>
...
<c:url value="Download_Contact_Detailsof_Pers_query" var="dwonloadUrl"/>
...
<c:url value="Excel_Contact_Detailsof_Pers_query" var="excelUrl" />
...
$(document).ready(function() {  
	...
	var select = '<option value="' + "0" + '">'+ "--Select--" + '</option>';
   	$('select#cont_comd').change(function() {
	   	...
	   	mockjax1('Contact_detailsof_pers_reporttbl');
		table = dataTable('Contact_detailsof_pers_reporttbl');
		...
		mockjax1('SummaryOf_Total_Number_reporttbl_pers');
		table1 = dataTable('SummaryOf_Total_Number_reporttbl_pers');
		$('#btn-reload').on('click', function(){
			table1.ajax.reload();
		});
});
...
function data(tableName){
		...
		var totalORs = 0;
	 	...
	 	if(tableName == "Contact_detailsof_pers_reporttbl"){	
		$.post("getContact_DetailsofPers_ReportDataList?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType,
			...
			jsondata.push(["", "", "", "", "", "<b>Total</b>", "<b>" + totalOffrs + "</b>", "<b>" + totalJCOs + "</b>", "<b>" + totalORs + "</b>"]);

		});
		$.post("getContact_DetailsofPers_TotalCount?"+key+"="+value,{Search:Search,cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,
			cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no},function(j) {
			FilteredRecords = j;
		});
	}
	 	else if(tableName == "SummaryOf_Total_Number_reporttbl_pers"){		
		$.post("getSummaryOf_Total_Number_ReportDataList_pers?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType,
			cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no},function(j) {
			
			for (var i = 0; i < j.length; i++) {
				var sr = i+1;
				jsondata.push([sr,j[i].cmd_unit, j[i].corp_unit,j[i].div_unit,j[i].bde_unit,j[i].unit_name,/*  j[i].unit_sus_no, */
					j[i].army_no, j[i].description,j[i].name,j[i].mobile_no,j[i].gmail,j[i].nic_mail]);
			}
		});
		$.post("getSummaryOf_Total_Number_TotalCount_pers?"+key+"="+value,{Search:Search,cont_comd:cont_comd,cont_corps:cont_corps,
