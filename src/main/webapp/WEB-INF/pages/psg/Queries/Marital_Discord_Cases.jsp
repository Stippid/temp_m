...
<form action="" method="post" enctype="multipart/form-data" class="form-horizontal"> 
	<div class="animated fadeIn">
	    	...
	    	<div class="col-md-12" align="center">
	    		<div class="card">
	    			<div class="card-header"><h5>MARITAL DISCORD CASES</h5></div>
		          			...
		          			<div class="col-md-12">
								...
								<div class="col-md-6">
									<div class="row form-group">
										...
										<div class="col-md-8">
											<select name="state_birth" id="state_birth" class="form-control">
												<option value="0">--Select--</option>
						...
						<div class="card-footer" align="center">
							<a href="Marital_Discord_Cases" type="reset" class="btn btn-success btn-sm"> Clear </a> 
		...
		<div class="col-md-12">
			<table id="Marital_Discord_cases_reporttbl" class="display table no-margin table-striped  table-hover table-bordered ">
				<thead>
			        <tr>
						<th>Sr No. </th>
						<th>Comd </th>
...
<!-- 						<th>SUS No </th> -->
		...
		<div class="col-md-12">
			<table id="Marital_Discord_cases_reporttbl_pers" class="display table no-margin table-striped  table-hover table-bordered ">
				<thead>
			        <tr>
						<th>Sr No. </th>
						<th>Comd </th>
...
<!-- 						<th>SUS No </th> -->
						...
						<th>State</th>
						<th>District</th>
						<th>Name of the Complaint</th>
						<th>Complaint/Allegations </th>
						<th>Date of Complaint/Allegation</th>
						<th>Date of the Status(Approved Date)</th>
			    </tr>
				</thead>
			</table>
		</div>
...
<c:url value="Download_Marital_Discord_Cases_query" var="dwonloadUrl"/>
<form:form action="${dwonloadUrl}" method="post" id="downloadForm" name="downloadForm" modelAttribute="cont_comd_dn">
	   ...
	   <input type="hidden" name="country_birth_tx" id="country_birth_tx">
	   <input type="hidden" name="state_birth_tx" id="state_birth_tx">
	   <input type="hidden" name="unit_name_dn" id="unit_name_dn">
	   <input type="hidden" name="sus_no_dn" id="sus_no_dn">
	   <input type="hidden" name="country_birth_dn" id="country_birth_dn" value="0">
	   <input type="hidden" name="state_birth_dn" id="state_birth_dn">
	   <input type="hidden" name="dt_from_dn" id="dt_from_dn">
	   <input type="hidden" name="dt_to_dn" id="dt_to_dn">
</form:form>

<c:url value="Excel_Marital_Discord_Cases_query" var="excelUrl" />
<form:form action="${excelUrl}" method="post" id="ExcelForm" name="ExcelForm" modelAttribute="cont_comd_ex">
	 <input type="hidden" name="cont_comd_ex" id="cont_comd_ex"  value="0">
	   ...
	   <input type="hidden" name="country_birth_txt" id="country_birth_txt">
	   <input type="hidden" name="state_birth_txt" id="state_birth_txt">
	   <input type="hidden" name="unit_name_ex" id="unit_name_ex">
	   <input type="hidden" name="sus_no_ex" id="sus_no_ex">
	   <input type="hidden" name="country_birth_ex" id="country_birth_ex" value="0">
	   <input type="hidden" name="state_birth_ex" id="state_birth_ex">
	   <input type="hidden" name="dt_from_ex" id="dt_from_ex">
	   <input type="hidden" name="dt_to_ex" id="dt_to_ex">
...
$(document).ready(function() {  
	...
	var select = '<option value="' + "0" + '">'+ "--Select--" + '</option>';
   	$('select#cont_comd').change(function() {
	   	...
	   	mockjax1('Marital_Discord_cases_reporttbl');
		table = dataTable('Marital_Discord_cases_reporttbl');
		...
		mockjax1('Marital_Discord_cases_reporttbl_pers');
		table1 = dataTable('Marital_Discord_cases_reporttbl_pers');
		$('#btn-reload').on('click', function(){
			table1.ajax.reload();
		});
		$.ajaxSetup({
...
function data(tableName){
		...
		var cont_comd=$("#cont_comd").val();
	 	...
	 	if(tableName == "Marital_Discord_cases_reporttbl"){
		$.post("getMarital_Discord_Cases_ReportDataList?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType,
			cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no,country_birth:country_birth,state_birth:state_birth,
			dt_from:dt_from,dt_to:dt_to},function(j) {
			
			for (var i = 0; i < j.length; i++) {
				var sr = i+1;
				jsondata.push([sr,j[i].cmd_unit, j[i].corp_unit,j[i].div_unit,j[i].bde_unit,j[i].unit_name,/* j[i].unit_sus_no, */j[i].state_name,j[i].district_name,j[i].offr
				,j[i].jco,j[i].or1,j[i].total]);
				s_total+=j[i].total;
			}
			jsondata.push(["","", "","","","",/* j[i].unit_sus_no, */"","",""
				,"","Total",s_total]);
		});
		$.post("getMarital_Discord_Cases_TotalCount?"+key+"="+value,{Search:Search,cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,
			cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no,country_birth:country_birth,state_birth:state_birth,dt_from:dt_from,dt_to:dt_to},function(j) {
			FilteredRecords = j;
		});
	}
	 	else if(tableName == "Marital_Discord_cases_reporttbl_pers"){		

		$.post("getMarital_Discord_Cases_ReportDataList_pers?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType,
			cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no,
			country_birth:country_birth,state_birth:state_birth,dt_from:dt_from,dt_to:dt_to},function(j) {
			
			for (var i = 0; i < j.length; i++) {
				var sr = i+1;
				jsondata.push([sr,j[i].cmd_unit, j[i].corp_unit,j[i].div_unit,j[i].bde_unit,j[i].unit_name,/* j[i].unit_sus_no, */j[i].army_no, j[i].description,j[i].name,j[i].state_name,j[i].district_name
					,j[i].name_lady,j[i].complaint,j[i].dt_of_comp,j[i].date_of_status]);
			}
		});
		$.post("getMarital_Discord_Cases_TotalCount_pers?"+key+"="+value,{Search:Search,cont_comd:cont_comd,cont_corps:cont_corps,
			cont_div:cont_div,cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no,country_birth:country_birth,state_birth:state_birth,dt_from:dt_from,dt_to:dt_to},function(j) {
			FilteredRecords = j;
		});
	}
}	
...
function downloaddata(){
	var cont_comd_tx = $("#cont_comd option:selected").text();
 	...
 	var country_birth_tx=$("#country_birth option:selected").text();
 	var state_birth_tx=$("#state_birth option:selected").text();
	...
	if(country_birth_tx == "--Select--") {
		country_birth_tx = "";
	}
	if(state_birth_tx == "--Select--") {
		state_birth_tx = "";
	}
	...
	$("#country_birth_tx").val(country_birth_tx);
	$("#state_birth_tx").val(state_birth_tx);
	...
	$("#country_birth_dn").val(country_birth);
	$("#state_birth_dn").val(state_birth);
	$("#dt_from_dn").val(dt_from);
	$("#dt_to_dn").val(dt_to);
...
function getExcel() {	
	var cont_comd_txt = $("#cont_comd option:selected").text();
 	...
 	var country_birth_txt=$("#country_birth option:selected").text();
 	var state_birth_txt=$("#state_birth option:selected").text();
	...
	if(country_birth_txt == "--Select--") {
		country_birth_txt = "";
	}
	if(state_birth_txt == "--Select--") {
		state_birth_txt = "";
	}
	...
	$("#country_birth_txt").val(country_birth_txt);
	$("#state_birth_txt").val(state_birth_txt);
	...
	$("#country_birth_ex").val(country_birth);
	$("#state_birth_ex").val(state_birth);
	$("#dt_from_ex").val(dt_from);
	$("#dt_to_ex").val(dt_to);
