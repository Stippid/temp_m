...
<form action="" method="post" enctype="multipart/form-data" class="form-horizontal"> 
	<div class="animated fadeIn">
	    	...
	    	<div class="col-md-12" align="center">
	    		<div class="card">
	    			<div class="card-header"><h5>MARITAL STATUS OF PERSONNELS OF UNITS/EST</h5></div>
						...
						<div class="card-footer" align="center">
							<a href="Marital_Status_Personnels" type="reset" class="btn btn-success btn-sm"> Clear </a> 
		...
		<div class="col-md-12">
			<table id="Marital_Status_Personnels_reporttbl" class="display table no-margin table-striped  table-hover table-bordered ">
				<thead>
			        <tr>
						<th>Comd </th>
						<th>Corps/Area </th>
		...
		<div class="col-md-12">
			<table id="Marital_Status_Personnels_reporttbl_pers" class="display table no-margin table-striped  table-hover table-bordered ">
				<thead>
			        <tr>
						<th>Unit </th>
						<th>SUS No </th>
...
<c:url value="Download_Marital_Status_Personnels_query" var="dwonloadUrl"/>
<form:form action="${dwonloadUrl}" method="post" id="downloadForm" name="downloadForm" modelAttribute="cont_comd_dn">
	   ...
	   <input type="hidden" name="marital_status_tx" id="marital_status_tx">
	   <input type="hidden" name="unit_name_dn" id="unit_name_dn">
	   <input type="hidden" name="sus_no_dn" id="sus_no_dn">
	   <input type="hidden" name="marital_status_dn" id="marital_status_dn" value="0">
	   <input type="text" name="dt_from_dn" id="dt_from_dn">
...
$(document).ready(function() {  
	...
	mockjax1('Marital_Status_Personnels_reporttbl');
	table = dataTable('Marital_Status_Personnels_reporttbl');
	...
	mockjax1('Marital_Status_Personnels_reporttbl_pers');
	table1 = dataTable('Marital_Status_Personnels_reporttbl_pers');
	$('#btn-reload').on('click', function(){
		table1.ajax.reload();
	});
...
function data(tableName){
	if(tableName == "Marital_Status_Personnels_reporttbl"){
		jsondata = [];
		...
		$.post("getMarital_Status_PersonnelsReportDataList?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType,
			cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no,marital_status:marital_status,
			dt_from:dt_from,dt_to:dt_to},function(j) {
			
			for (var i = 0; i < j.length; i++) {
				jsondata.push([j[i].cmd_unit, j[i].corp_unit,j[i].div_unit,j[i].bde_unit,j[i].unit_name,j[i].unit_sus_no,j[i].offr
				,j[i].jco,j[i].or1,j[i].total]);
			}
		});
		$.post("getMarital_Status_PersonnelsTotalCount?"+key+"="+value,{Search:Search,cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,
			cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no,marital_status:marital_status,dt_from:dt_from,dt_to:dt_to},function(j) {
			FilteredRecords = j;
		});
	}
	if(tableName == "Marital_Status_Personnels_reporttbl_pers"){
		jsondata = [];
		var table = $('#'+tableName).DataTable();
		var info = table.page.info();
		...
		$.post("getMarital_Status_PersonnelsReportDataList_pers?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType,
			cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no,
			marital_status:marital_status,dt_from:dt_from,dt_to:dt_to},function(j) {
			
			for (var i = 0; i < j.length; i++) {
				jsondata.push([j[i].unit_name, j[i].unit_sus_no,j[i].army_no, j[i].description,j[i].name,j[i].marital_name,j[i].date_of_birth]);
			}
		});
		$.post("getMarital_Status_PersonnelsTotalCount_pers?"+key+"="+value,{Search:Search,cont_comd:cont_comd,cont_corps:cont_corps,
			cont_div:cont_div,cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no,marital_status:marital_status,dt_from:dt_from,dt_to:dt_to},function(j) {
			FilteredRecords = j;
		});
	}
}	
...
function downloaddata(){
	...
	$("#marital_status_dn").val(marital_status);
