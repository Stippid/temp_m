...
<form action="" method="post" enctype="multipart/form-data" class="form-horizontal"> 
	<div class="animated fadeIn">		
	    	<div class="col-md-12" align="center">
	    		<div class="card">
	    			<div class="card-header"><h5>Service Record </h5></div>
	          			<div class="card-body">
		          			<div class="col-md-12">
								<div class="col-md-6">
		...
		<div class="col-md-12">
			<table id="Record_Service_reporttbl" class="display table no-margin table-striped  table-hover  table-bordered ">
				<thead>
			        <tr>
						<th>Comd </th>
						<th>Corps/Area </th>
...
$(document).ready(function() {  
	...
	mockjax1('Record_Service_reporttbl');
	table = dataTable('Record_Service_reporttbl');
...
function data(tableName){
	if(tableName == "Record_Service_reporttbl"){
		jsondata = [];
		...
		$.post("getRecord_ServiceReportDataList?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType,
			cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no,personnel_no:personnel_no},function(j) {
			
			for (var i = 0; i < j.length; i++) {
				jsondata.push([j[i].cmd_unit, j[i].corp_unit,j[i].div_unit, j[i].bde_unit,j[i].unit_name,
					/* j[i].unit_sus_no, */
					j[i].cadet_no,j[i].personnel_no,j[i].rank,j[i].name,j[i].action]);
			}
		});
		$.post("getRecord_ServiceTotalCount?"+key+"="+value,{Search:Search,cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no,personnel_no:personnel_no},function(j) {
			FilteredRecords = j;
		});
	}
}	
...
<c:url value="view_UpadteOfficerDataUrl" var="ViewUrl"/>
	<form:form action="${ViewUrl}" method="post" id="View1Form" name="View1Form" modelAttribute="idV">
	  ...
	  <input type="hidden" name="whrToOpen" id="whrToOpen" value="query"> 
</form:form> 

<c:url value="Excel_Record_Service_query" var="excelUrl" />
