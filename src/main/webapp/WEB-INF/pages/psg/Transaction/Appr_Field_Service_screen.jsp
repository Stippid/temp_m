				...
				<div class="col-md-12" id="unit_sus_nodiv" style="display: none">
					<div class="col-md-6">
						<div class="row form-group">
							...
							<div class="col-md-8">
								<label id="unit_sus_nolb"
									style="background-color: honeydew; padding: 5px;">${listp[0].sus_id}</label>
					...
					<div class="col-md-6">
						<div class="row form-group">
							...
							<div class="col-md-8">
								<label id="unit_name_lb"
									style="background-color: honeydew; padding: 5px;"></label>
							</div>
						</div>
					</div>
				</div>
				<div class="col-md-12">
						...
						<div class="row form-group">
							...
							<div class="col-md-8">
								<select name="service_category" id="service_category"
									onchange="onCategory()" class="form-control-sm form-control">
					...
					<div class="col-md-6">
						<div class="row form-group">
							<div class="col-md-4">
								<label class=" form-control-label"><strong
									style="color: red;"> </strong> Area </label>
							</div>
							<div class="col-md-8">
								<input type="text" id="fd_serviceslb"
									value="${listp[0].fd_services}" class="form-control autocomplete" /> <select name="fd_service1"
									id="fd_service1" class="form-control-sm form-control"
									style="display: none">
				...
				<div class="col-md-12">
						...
						<div class="row form-group">
							...
							<div class="col-md-8">
								<input type="hidden" id="p_id" name="p_id"
									class="form-control autocomplete" value="${listp[0].id}"
									autocomplete="off"> <input type="text" id="authority1" onkeyup="return specialcharecter(this)"
									name="authority1" value="${listp[0].authority}"
					...
					<div class="col-md-6">
						<div class="row form-group">
							...
							<div class="col-md-8">
								<input type="hidden" id="authority_datelb"
									value="${listp[0].authority_date}" /> <input type="Date"
									id="authority_date1" name="authority_date1"
									class="form-control autocomplete" autocomplete="off">
							</div>
						</div>
					</div>
				...
				<div class="col-md-12">
					<div class="col-md-6">
						<div class="row form-group">
							...
							<div class="col-md-8">
								<input type="text" id="unit_location1" name="unit_location1"
									value="${listp[0].unit_location}"
									class="form-control autocomplete" autocomplete="off">
							</div>
						</div>
					</div>
					<div class="col-md-6">
						<div class="row form-group">
							...
							<div class="col-md-8">
								<input type="text" id="operationlb"
									value="${listp[0].operation}" class="form-control autocomplete" /> <select name="operation1"
									id="operation1" class="form-control-sm form-control"
									style="display: none">
									<option value="0">--Select--</option>
									<c:forEach var="item" items="${getOprationList}"
										varStatus="num">
		...
		<div id="Field_Service" class="tabcontent" style="display: block;">
			<div class="card" style="margin-top: 20px;">
				<label class=" form-control-label" style="color: red;"><strong
					style="color: red;">Note:</strong>Previously Approved Data Is
					Highlighted With Green Color</label>
				<div class="card_body">
					<table id="fieldSerivice_table" class="table-bordered watermarked"
						...
						<tbody data-spy="scroll" id="field_service_tbody"
							...
							<c:forEach var="item" items="${listch}" varStatus="num">
								<tr class="status${item.status}">
									...
									<td><input type="text" value="${item.personnel_no}"
										class="form-control autocomplete" autocomplete="off"
										readonly="readonly"></td>
									<td><input type="hidden" class="datefrom"
										value="${item.from_date}" /> <input type="Date"
										id="fromdate${num.index+1}"
										class="form-control autocomplete datefromto"
										autocomplete="off"></td>
									<td><input type="hidden" class="dateto"
										value="${item.to_date}" /> <input type="Date"
										id="todate${num.index+1}"
										class="form-control autocomplete datefromto"
										autocomplete="off"></td>
									<td><input type="text" value="${item.duration}"
										readonly="readonly" class="form-control autocomplete"
										autocomplete="off"></td>
									<td><input type="text" value="${item.place}"
										class="form-control autocomplete" autocomplete="off">
									</td>
								</tr>
							</c:forEach>
...
<div id="submit_data" class="card-footer" align="center"
	...
	<input type="button" class="btn btn-primary btn-sm" value="Approve"
		onclick="if (confirm('Are You Sure You Want to Approve Officer Data?')){return Approved();}else{return false;}">

	<!-- 	<input type="button" class="btn btn-primary btn-sm" value="Reject" -->
	<!-- 		onclick="if (confirm('Are You Sure You Want to Reject Officer Data?')){return Reject();}else{return false;}"> -->
	...
	<a href="Search_Field_Service_url" id="btn"
		class="btn btn-info btn-sm">Back</a>
</div>


<form:form action="Approve_service" method="post" id="apvForm"
	name="apvForm">
	<input type="hidden" name="${_csrf.parameterName}"
		id="${_csrf.parameterName}" value="${_csrf.token}" />
	<input type="hidden" name="id3" id="id3" value="0" />
	<input type="hidden" name="fd_servicesapp" id="fd_servicesapp"
		value="0" />
</form:form>
<form:form action="Reject_FD_service" method="post" id="RejectForm" name="RejectForm">
	...
	<input type="hidden" name="fd_servicesapp4" id="fd_servicesapp4" value="0" />
...
function field_service_remove_fn(R){
	var rc = confirm("Are You Sure! You Want To Delete");
	 ...
	 var comm_ch_id=$('#comm_ch_id'+R).val();
	 
	  $.post('fieldService_delete_action?' + key + "=" + value, {field_service_ch_id:field_service_ch_id,census_ch_id:census_ch_id,comm_ch_id:comm_ch_id  }, function(data){ 
	...
	if ($("#to_date"+qs).val() == "") {
		...
		var census_id=$("#census_id"+qs).val();
		  var comm_id=$("#comm_id"+qs).val();    
				   ...
				   $.post('field_serviceAction?' + key + "=" + value, {personnel_no:personnel_no,fd_service:fd_service,from_date:from_date
					   ,to_date:to_date,duration:duration,place:place,unit_location:unit_location,operation:operation,authority:authority,authority_date:authority_date,ch_id:ch_id,census_id:census_id,comm_id:comm_id}, function(data){	   
...
function getFieldService(){
	 ...
	 var i=0;
	  $.post('getFieldService?' + key + "=" + value, {q_id:q_id }, function(j){
...
function Reject(){
	...
	var id= $("#p_id").val();
	$("#id4").val(id);
	$("#fd_servicesapp4").val($("#service_category").val());
	$("#rej_remarks4").val($("#reject_remarks").val());
	$("#id4").prop("disabled",false);
	$("#fd_servicesapp4").prop("disabled",false);
	$("#rej"+'${_csrf.parameterName}').prop("disabled",false);
	$("#rej_remarks4").prop("disabled",false);
