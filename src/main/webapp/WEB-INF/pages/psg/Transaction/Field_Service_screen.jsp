	    		...
	    		<div class="card-header"><h5>Field Service</h5> <h6 class="enter_by"><span style="font-size:12px;color:red;">(To be entered by unit)</span></h6></div>
	              		...
	              		<div class="col-md-12">	         
	              			<div class="col-md-6">
	              					<div class="row form-group">
						               <div class="col-md-4">
						                    <label class=" form-control-label"><strong style="color: red;">* </strong>Op Name</label>
						                </div>
						                <div class="col-md-8">
...
function field_service_save_fn(qs){
	...
	if ($("#from_date"+qs).val() == ""  && $("#from_date"+qs).val() == "DD/MM/YYYY") {
		...
		var census_id=$("#census_id"+qs).val();
		  var comm_id=$("#comm_id"+qs).val();    
				   ...
				   $.post('field_serviceAction?' + key + "=" + value, {service_category:service_category,personnel_no:personnel_no,fd_service:fd_service,from_date:from_date,unit_sus_no:unit_sus_no
					   ,to_date:to_date,duration:duration,place:place,unit_location:unit_location,operation:operation,authority:authority,authority_date:authority_date,ch_id:ch_id,census_id:census_id,comm_id:comm_id,p_id:p_id}, function(data){	   
...
function getFieldService(){
	$.ajaxSetup({
	 ...
	 if(cat==0){
		 ...
		 p_url="getFieldService?";
		 ch_url="getFieldServicech?";
	 }
	 else if(cat==2){
	 ...
	 var i=0;
	  $.post(ch_url + key + "=" + value, {p_id:p_id }, function(j){
		...
		$("table#fieldSerivice_table").append('<tr id="tr_field_service'+qu+'">'
			...
			+'<td><input type="text" id="place'+qu+'" name="place'+qu+'" value="' + j[i].place +'" class="form-control autocomplete" maxlength="50" onkeyup="return specialcharecter(this)" autocomplete="off" ></td>'
