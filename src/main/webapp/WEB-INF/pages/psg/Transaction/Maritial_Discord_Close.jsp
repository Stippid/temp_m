...
<form:form name="Maritial_Discord_close" id="Maritial_Discord_close" action="Maritial_Discord_action_close?${_csrf.parameterName}=${_csrf.token}" method="post" class="form-horizontal" commandName="Maritial_Discord_close_CMD">
	...
	<input type="hidden" id="id" name="id" class="form-control autocomplete" autocomplete="off" >
			...
			<input type="hidden" id="Child_Hid" name="Child_Hid" value = "0" class="form-control autocomplete" autocomplete="off" >				
				<div class="card-footer" align="center" class="form-control">
					<input type="button" class="btn btn-danger btn-sm" value="Close" id="sub_p"  onclick=" return Close_Data();" >
				</div>	
...
<c:url value="close_Maritial_case" var="closeUrl" />
<form:form action="${closeUrl}" method="post" id="viewcloseForm"
	...
	<input type="hidden" name="status_of_case6" id="status_of_case6" value="0" />
	<input type="hidden" name="dt_of_status6" id="dt_of_status6" value="0" />
...
<script>
	...
	$(document).ready(function() {
		...
		datepicketDate('dt_of_app');
		  ...
		  if('${Maritial_Discord_close_CMD.id}' != ""){
		    	
		    	  $("#id").val('${Maritial_Discord_close_CMD.id}');
				 	//$("#service_category").val('${Maritial_Discord_close_CMD.service_category}');
				 	fn_service_category_change();
				 	$("#personnel_no").val('${Maritial_Discord_close_CMD.personnel_no}');
				 	personal_number();
				 	$("#name_lady").val('${Maritial_Discord_close_CMD.name_lady}');
				 	 $("#name_lady").attr('readonly', true);
				 	$("#complaint").val('${Maritial_Discord_close_CMD.complaint}');  
				 	 $("#complaint").attr('readonly', true);
				 	$("#dt_of_comp").val(ParseDateColumncommission('${Maritial_Discord_close_CMD.dt_of_comp}'));
...
function Close_Data(){	
	$("#id6").val($('#id').val());
	$("#status_of_case6").val($('#status_of_case').val());
	$("#dt_of_status6").val(ParseDateColumncommission($('#dt_of_status').val()));
