				...
				<div class="col-md-12" style="display:none">
					...
					<div class="col-md-12">
						...
						<div class="card-footer" align="center" class="form-control">
							<a href="Posting_OutUrl" class="btn btn-success btn-sm">Clear</a> 
		...
		<form id="post_in_form">
				 ...
				 <div class="col-md-12" id="xlist" style="display: none" >
					<div class="col-md-6">
						...
						<div class="card-footer" align="center" class="form-control">
							<a href="Posting_OutUrl" class="btn btn-success btn-sm">Clear</a> <input
 ...
 //post in save
	function save_postin() {
		...
		if($('#service_category').val()=='1'){
			saveUrl='posting_in_action_withcensus?'
		}
...
<script>
	...
	if(dt_tos!=null && dt_tos!=""){
	 ...
	 var saveUrl;
		if($('#service_category').val()=='1'){
			saveUrl='posting_out_action_withcensus?'
		}
		else if($('#service_category').val()=='2'){
		...
		$.post(saveUrl + key + "=" + value, {unit_description:unit_description,from_sus_no:from_sus_no,personnel_no : personnel_no,rank:rank, to_sus_no : to_sus_no,out_auth : out_auth,out_auth_dt : out_auth_dt,dt_of_sos : dt_of_sos,comm_id : comm_id,census_id : census_id,h_id : h_id	},
				function(data) {
	...
	function fn_getOfficerData(){
		...
		if (personnel_no != "") {
			...
			$.post(perurl + key + "=" + value, {
				...
				var comm_id = j[0][0];
				var	 cpurl='GetCensusDataApprove_postin_out?';
	...
	function fn_getOfficerCensusIn(){
		...
		if (personnel_no != "") {
			...
			$.post('CheckPersonnelNoData?' + key + "=" + value, {
				...
				$.post('checkrole_hdr_status?' + key + "=" + value,{ comm_id : comm_id },function(rstatus) {
				if(rstatus.pass=="1"){
				$.post('GetCensusDataApprove_postin_out?' + key + "=" + value,{ comm_id : comm_id },function(k) {
		    		 if(k.length > 0)
		    		 {
		    			 if(k[0][18] != "1")	
							{
				 ...
				 var susNoAuto=$("#"+obj.id);
				  susNoAuto.autocomplete({
				      source: function( request, response ) {
				        $.ajax({
				        	type: 'POST',
					    	url: "getTargetUnitsNameActiveList_postinfrom?"+key+"="+value,
				        data: {unit_name:unit_name},
				          success: function( data ) {
				        	 var susval = [];
				        	  var length = data.length-1;
