						...
						<div class="col-md-6">
						  ...
						  <div class="card-footer" align="center" class="form-control">
							    <a href="cda_acc_no_url" class="btn btn-success btn-sm" id ="clear" >Clear</a> 
							        <input type="button" class="btn btn-primary btn-sm" value="Reject" onclick="if (confirm('Are You Sure You Want to Reject Officer Data?')){addRemarkModel('Reject_cda_acc_no',0)}else{return false;}">	              
...
<form:form name="Approve_officer_Data" id="Approve_officer_Data" action="Approve_officer_DataAction_xml?${_csrf.parameterName}=${_csrf.token}" method="post" class="form-horizontal" commandName="Approve_officer_DataCMD">
	<div class="animated fadeIn">
		<div class="card">
...
// 	var census_id=$("#census_id").val();	
	var comm_id=$("#comm_id").val();	
	$.post('address_details_GetData_xml?' + key + "=" + value,{comm_id:comm_id }, function(j){
		var v=j.length;
		if(v!=0){
...
function get_post_in_data_reject(){
// 	var ch_id='${census_id}'; 
	 var comm_id =$("#comm_id").val();
	 var ch_r_id = $("#update_id").val();
	 var reject_remarks = $("#reject_remarks").val();
		
	 
	 $.post('Reject_PostIN_xml?' + key + "=" + value, {comm_id:comm_id,ch_r_id:ch_r_id,reject_remarks:reject_remarks}, function(j){
...
function Reject_cda_acc_no ()
{
	 ...
	 $.post('Reject_CDA_xml?' + key + "=" + value, {comm_id:comm_id,ch_r_id:ch_r_id,reject_remarks:reject_remarks}, function(j){
		 
		 if(j=="Reject Successfully.")
			 {
			 $("#form_cda_acc").hide();
			 }
			alert(j);
