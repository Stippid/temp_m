...
<form:form name="type_of_stock_c" id="type_of_stock_c" action="type_of_stock_c_Action" method="post" class="form-horizontal" commandName="type_of_stock_c_CMD">
	<div class="animated fadeIn">
		<div class="">
			<div class="container" align="center">
				<div class="card">
					<div class="card-header" align="center"><strong><h5>TYPE OF STOCK (C VEHICLE)</h5></strong></div>
					...
					<div class="form-control card-footer" align="center" id="btnhide" style="display: block;">
						<a href="type_of_stock_c" class="btn btn-success btn-sm" >Clear</a>
...
<c:url value="getTypeofstockC" var="searchUrl" />
	<form:form action="${searchUrl}" method="post" id="searchForm" name="searchForm" modelAttribute="code_value1">
		...
		<input type="hidden" name="type_of_stock_c1" id="type_of_stock_c1" value="0"/>
...
<form:form action="${updateUrl}" method="post" id="updateForm"
	...
	<input type="hidden" name="type_of_stock_c" id="type_of_stock_c" value="0" />
...
function Search(){
	...
	$("#mct1").val($("#mct").val()) ;
	$("#type_of_stock_c1").val($("#stock").val()) ;
      	...
      	var unit_name=$("#depot_name").val();
    		 	...
    		 	$.post("getTargetSUSFromUNITNAME?"+key+"="+value, {target_unit_name:unit_name}).done(function(j) {				
    		        ...
    		        $("#sus_no").val(dec(enc,j[0]));
    		    		 	...
    		    		 	$.post("getMCT_NomenList_C?"+key+"="+value, {sus_no:dec(enc,j[0]),stock:$("select#stock").val()}).done(function(j) {				
	 	...
	 	var susNoAuto=$("#sus_no");
	  	susNoAuto.autocomplete({
	      	source: function( request, response ) {
	        	$.ajax({
	        		type: 'POST',
	        		url: "getDepotSUSNoList_C?"+key+"="+value,
	        		data: {sus_no:sus_no},
	      	...
	      	select: function( event, ui ) {
		    	var sus_no = ui.item.value;
		    		 	...
		    		 	$.post("getActiveUnitNameFromSusNo?"+key+"="+value, {sus_no:sus_no}).done(function(j) {				
		   			 	...
		   			 	$.post("getMCT_NomenList_C?"+key+"="+value, {sus_no : sus_no,stock:$("select#stock").val()}).done(function(j) {				
							var options = '<option value="0" >--select--</option>';
				    			for ( var i = 0; i < j.length; i++) {
					    			if( j[i] == '${std_nomclature}')
					    			{
			...
			var unit_name = this.value;
		 	var susNoAuto=$("#depot_name");
		      ...
		      source: function( request, response ) {
		        $.ajax({
		        type: 'POST',
		        url: "getDepotUnitNameList_C?"+key+"="+value,
...
<script>
	$('#std_nomclature').on('change', function() {
		var nomen = this.value;
			 	...
			 	$.post("getNomclature_C?"+key+"="+value, {nomen:nomen}).done(function(j) {				
			 		var length = j.length-1;
			    	var enc = j[length].substring(0,16);
			    	$("#mct").val(dec(enc,j[0]));	
				}).fail(function(xhr, textStatus, errorThrown) {
...
function clear(){   
	document.getElementById("type_of_stock_c").reset();
}
</script>