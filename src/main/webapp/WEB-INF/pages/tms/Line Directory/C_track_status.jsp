								...
								<table id="getVehicleStatusTbl"
								 ...
								 Designated Base Wksp
                                     ...
                                     Designated Repair Agency
                                             ...
                                             <th>CIN</th>
<%-- 											<c:if test="${roleAccess == 'Unit' || roleAccess == 'MISO'  ||  roleAccess == 'Line_dte'}"> --%>
												...
												<c:if test="${item[14] == null}">
												       ...
												       <option value="0">--Select--</option>
														...
														<input type="button" class="btn btn-primary btn-sm"
															...
															<c:if test="${ roleAccess == 'Line_dte' || roleAccess == 'MISO'}">
															<c:if test="${item[14] == null}">
														<select name="base_wp${num.index+1}" id="base_wp${num.index+1}" class="form-control">
												       <option value="0">--Select--</option>
													...
													<td>${item[16]}</td>
													<td>${item[17]}</td>
													<td rawsoan="2">${item[18]}</td>
													<td>${item[19]}</td>
													<c:if test="${roleAccess != 'MISO'}">
													<td id="action${num.index+1}">${item[20]}</td>												
													</c:if>
											</tr>
										</c:forEach>
									</tbody>
...
<c:url value="get_C_trackstatus_report" var="searchUrl" />
<form:form action="${searchUrl}" method="post" id="searchForm"
	name="searchForm">
...
function getyear() 
{
	...
	function update_base_wshop(ps,cin_no){
		...
		if ($('#base_wp'+ps).val() == "0") {
			alert("Please Enter Base workshop");
			$('#base_wp'+ps).focus();
			return false;
		} 
