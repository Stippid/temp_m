		...
		<div class="" align="center">
			<div class="card">
					...
					<div class="col-md-12" id="tpt">
						<div class="col-md-4">
							<div class="row form-group">
								...
								<div class="col-md-8">
									<select id="type_veh" class="form-control-sm form-control" style="width: 100%" onchange="changehrs();getMCTMainList();">
						...
						<div id="chk_box" class="chk_box">
							<c:forEach var="item" items="${getCommandList}" varStatus="num">
								<label for="one" class="quali_subjectlist_cmd"> <input
									type="checkbox" name="multisub_cmd"    class="cmdCheckBox" value="${item[0]}"
				...
				<div class="form-control card-footer" align="center">
					<a href="oh_qfd_url" type="reset" class="btn btn-success btn-sm">Clear </a>
								...
								<table id="getVehicleStatusTbl"
									...
									<tbody>
										<c:if test="${list.size() == 0}">
											<tr>
												<td colspan="5" align="center" style="color: red;">Data Not Available</td>
											</tr>
										</c:if>
...
<!-- 							</div> -->
				...
				<input type="button" id ="save_p" class="btn btn-primary btn-sm" value="Submit" onclick="submit_b_veh();"  style="display: none" > 	
				</div>
			</div>
		</div>
	</div>
</div>
<c:url value="getSearch_discard_condition_report" var="searchUrl" />
<form:form action="${searchUrl}" method="post" id="searchForm"
	...
	<input type="hidden" name="ddlYears1" id="ddlYears1" />
...
<form:form action="${approveUrl}" method="post" id="saveForm" name="saveForm" modelAttribute="id3">
...
var q3 = '${CheckVal1}';
	...
	var 	line_dte_sus=$('#line_dte_sus').val();
		...
		$.post('save_base_action?' + key + "=" + value, {
			...
			vintage:vintage,
			ba_no_t:ba_no_t,
			km_t:km_t,line_dte_sus:line_dte_sus,mode:mode
		}, function(data) {
			if(data.error == null) {
...
//	function getMCTMainList(){
	...
	// 		var option='';
	// 		if (type_veh!="") {
	// 			$.post("getMctNoDigitDetailListfordiscard4?" + key + "=" + value, {type_veh:type_veh}).done(function(j) {
	...
	function getMCTMainList() {
		...
		if (line_dte_sus != "" && type_veh != "") {
			$
					.post(
							"getMctNoDigitDetailListfordiscard4?" + key + "="
									+ value, {
					...
					.done(
							function(j) {
								...
								for (var i = 0; i < j.length; i++) {
									...
									option += '<label for="one" class="quali_subjectlist" ><input  class="nrCheckBox" type="checkbox" onchange="findselected()"  name="multisub_cmd" id=" '+j[i].mct_main_id+ ' "value="'+j[i].mct_main_id+'"/>&nbsp&nbsp&nbsp'
										+ j[i].mct_nomen + '</label>';
