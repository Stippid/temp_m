	...
	$('#btn_search').on('click', function(){
		if($("#type_veh").val() == "") {
			...
			if(table == ""){
				...
				table = dataTable('vehicleStatusTbl',[0,7,8,9,10,11,12,13,14],[]);
			}else{
				table.ajax.reload();
			}
		}
...
function data(tableName){
	...
	$.post("vehicle_status_line_dte_coasList?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType,
		type_veh:type_veh,prf_code:prf_code,mct_main:mct_main,cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,cont_bde:cont_bde,sus_no_list:sus_no_list,line_dte_sus:line_dte_sus},function(j) {
		for (var i = 0; i < j.length; i++) {
			jsondata.push([(i+1),j[i].comd,j[i].corps,j[i].div,j[i].bde,j[i].unit_name,j[i].mct_nomen,j[i].ue,j[i].against_ue,j[i].over_ue,j[i].loan,j[i].sec_store,j[i].acsfp,j[i].total_uh,j[i].gift]);
		}
	});
	$.post("vehicle_status_line_dte_coasCount?"+key+"="+value,{Search:Search,type_veh:type_veh,prf_code:prf_code,mct_main:mct_main,cont_comd:cont_comd,
		cont_corps:cont_corps,cont_div:cont_div,cont_bde:cont_bde,sus_no_list:sus_no_list,line_dte_sus:line_dte_sus},function(j) {
		FilteredRecords = j;
	});
...
function exportFn(){
	if($("#type_veh").val() == "") {
		...
		$("#sus_no_list1").val($("#sus_no_list").val());
		$("#line_dte_sus1").val($("#line_dte_sus").val());
		document.getElementById('exportUserListForm').submit();
	}
}
</script> 
<c:url value="vehicle_status_line_dte_coasExport" var="exportUrl" />
<form:form action="${exportUrl}" method="post" id="exportUserListForm" name="exportUserListForm" modelAttribute="type_veh">
	...
	<input type="hidden" name="sus_no_list1" id="sus_no_list1" value="0"/>
									...
									<table id="vehicleStatusTbl" style="width: 100%;display: none;" class="table no-margin table-striped"> <!-- class="table no-margin table-striped  table-hover  table-bordered report_print" style="margin-bottom: 0rem;" -->
										<thead>
							        		<tr>
												...
												<th  colspan="5"  style="text-align: center;width:25%;" >UH</th>	
												<th rowspan="2" style="text-align: center;width:5%;">Total UH</th>
												<th rowspan="2" style="text-align: center;width:5%;">Gift</th>
											</tr>
											<tr>
												<th style="text-align: center;width:5%;">Against UE</th>
												<th style="text-align: center;width:5%;">Over UE</th>
												<th style="text-align: center;width:5%;">Loan</th>
												<th style="text-align: center;width:5%;">Sector Store</th>
												<th style="text-align: center;width:5%;">ACSFP</th>
											</tr> 
										</thead>
										<tbody>
