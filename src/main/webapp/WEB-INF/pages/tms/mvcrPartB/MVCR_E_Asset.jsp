 ...
 <div class="animated fadeIn" id="printableArea">
	   				...
	   				<tbody style="overflow: hidden;">
	   					<tr>
	   						...
	   						<td align="center">
	   							...
	   							<h5>MONTHLY VEHICLE CASUALTY RETURN - E Asset</h5>
	   						</td>
	   						<td align="right">
    			...
    			<div class="card-body">
					...
					<tbody style="overflow: hidden;">
						...
						<tr style="font-size: 14px;">
							...
							<td style="width: 25%;" align="left"> <label id="approve_date">${app}</label></td>
						...
						<tr style="font-size: 14px;">
							...
							<td style="width: 25%;" align="left"><label id="modify_date">${modi}</label></td>
	...
	<div class="row">
		<div  class="col-md-12">
			...
			<span id="ip"></span>
   				<table id="getEAssetReportMvcr" class="table no-margin table-striped  table-hover  table-bordered report_print" style="margin-bottom: 0rem;">
					<thead>
						<tr style="font-size: 13px;">
							<th style="width: 5%;">Ser No</th>
							<th style="width: 5%;">MCT</th>
							<th style="width: 60%;">Nomenclature</th>
							<th style="width: 10%;">Present Cost</th>	
							<th style="width: 10%;">UH</th>	
							<th style="width: 10%;">Value</th>					
						</tr>
					</thead>
					<tbody>
						<c:forEach items="${partBMvcrE_Asset_UHList}" var="list" varStatus="num" >
							<tr style="font-size: 13px;">
								<td style="width: 5%;" align="center">${num.index+1}</td>
								<td style="width: 5%;">${list[0]}</td>
								<td style="width: 60%;">${list[1]}</td>
								<td align="center" style="width: 10%;">${list[2]}</td>
								<td align="center" style="width: 10%;"> <a hreF='#' data-target='#rejectModal' data-toggle='modal' onclick='Reject(${list[0]},"-1")' >${list[3]}</a></td>
								<td align="center" style="width: 10%;">${list[4]}</td>
							</tr>
						</c:forEach>
						<tr style="font-size: 13px;">
							<td colspan="3" style="width: 70%;text-align: center;"><B>Total</B></td>
							<td align="center" style="width: 10%;" ><B>${PRECOST}</B></td>
							<td align='center' style="width: 10%;"><B>${totalUH}</B></td>
							<td align="center" style="width: 10%;" ><B>${TOTAL}</B></td>
						</tr>
					</tbody>
				</table>
   			</div>
...
<div class="animated fadeIn">
	...
	<c:url value="Detalis_UE_UH1" var="viewUrl" />
		<form:form action="${viewUrl}" method="post" id="viewForm" name="viewForm" modelAttribute="sus_no4">
		<input type="hidden" name="sus_no4" id="sus_no4" value="${sus_no}"/>
		<input type="hidden" name="unit_name4" id="unit_name4" value="${unit_name}"/>
	</form:form>
	    ...
	    <div class="modal-dialog">
	      <div class="modal-content">
	        <div class="modal-header">
	          <h4 class="modal-title">Present Cost of individual BA Number</h4>
...
function Reject(mct){
   ...
   var mct1 = mct;
   generate_baNo_presentCost(mct1);
}
function generate_baNo_presentCost(mct1){
	var sus_no =document.getElementById('rejectid_model').value; 
	$("table#addQuantity tbody").empty();
	
	
	   	$.post("generate_baNo_presentCost?"+key+"="+value, {sus_no:sus_no,mct:mct1}).done(function(k) {
	   		
		for ( var x = 0; x < k.length; x++) {
			if(k[x][1]!=null)
			{
				k[x][1] = parseFloat(k[x][1]).toFixed(2);
 			}
 			else{
 				k[x][1] = 0;
 			}
        	$("table#addQuantity").append('<tr><td>'+(x+1)+'</td>'+'<td>'+k[x][0]+'</td>'+ '<td>'+k[x][1]+'</td>'+ '</tr>');
    	}
