 ...
 */
(function(f) {
	var d = f.ajax, e = [], l = [], u = [], n = /=\?(&|$)/, p = (new Date())
			.getTime();
	function m(w) {
	...
	function s(w, y) {
		...
		if (f.isFunction(w.url.test)) {
			...
			if (w.url !== y.url
					&& x === -1
					|| !new RegExp(w.url.replace(/[-[\]{}()+?.,\\^$|#\s]/g,
							"\\$&").replace(/\*/g, ".+")).test(y.url)) {
				return null
			}
		}
		if (w.data) {
	...
	function v(y) {
		if (f.isArray(y)) {
			var x = y[0];
			var w = y[1];
			return (typeof x === "number" && typeof w === "number") ? Math
					.floor(Math.random() * (w - x))
					+ x : null
		} else {
					...
					var B = function() {
						...
						if (z.dataType == "json"
							...
							if (z.dataType == "xml") {
								...
								if (typeof x.responseText === "object"
										&& x.responseText !== null) {
									x.contentType = "application/json";
									this.responseText = JSON
											.stringify(x.responseText)
								} else {
	...
	function k(C, A, w) {
		...
		if (C.data && n.test(C.data) || n.test(C.url)) {
			q(C, A, w);
			var y = /^(\w+:)?\/\/([^\/?#]+)/, B = y.exec(C.url), z = B
					&& (B[1] && B[1] !== location.protocol || B[2] !== location.host);
			C.dataType = "script";
	...
	function j(y, w, x) {
		...
		if (y.global) {
			(y.context ? f(y.context) : f.event).trigger("ajaxSuccess",
					[ {}, y ])
		}
	}
	function r(x, w) {
		...
		if (x.global) {
			(x.context ? f(x.context) : f.event).trigger("ajaxComplete", [ {},
					x ])
		}
	...
	f.mockjaxSettings = {
		log : function(y, A) {
			if (y.logging === false
					|| (typeof y.logging === "undefined" && f.mockjaxSettings.logging === false)) {
				return
			}
	...
	f.mockjaxClear = function(w) {
		window.console
				&& window.console.warn
				&& window.console
						.warn("DEPRECATED: The $.mockjaxClear() method has been deprecated in 1.6.0. Please use $.mockjax.clear() as the older function will be removed soon!");
		f.mockjax.clear()
	};
