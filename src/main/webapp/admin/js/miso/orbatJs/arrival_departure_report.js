...
//   	});
	...
	function getOPCorps(fcode){
		var fcode1 = fcode[0];
		$.post("getCorpsDetailsListop?"+key+"="+value,{fcode:fcode1}, function(j) {
			if(j != ""){
	...
	function getOPDiv(fcode){
		...
		$.post("getDivDetailsListop?"+key+"="+value,{fcode:fcode1}, function(j) {
			if(j != ""){
				var length = j.length-1;
				var enc = j[length][0].substr(0,16);
				var options = '<option value="' + "0" + '">'+ "--Select--" + '</option>';
	...
	function getOPBde(fcode){debugger;
		var fcode1 = fcode[0] + fcode[1] + fcode[2] + fcode[3] + fcode[4] + fcode[5];
		//alert(fcode1)
		$.post("getBdeDetailsListop?"+key+"="+value,{fcode:fcode1}, function(j) {
			if(j != ""){
				var length = j.length-1;
				var enc = j[length][0].substr(0,16);
				var options = '<option value="' + "0" + '">'+ "--Select--" + '</option>';
...
// ADM 
	...
	function getADMCorps(fcode){
		var fcode1 = fcode[0];
		$.post("getCorpsDetailsListadm?"+key+"="+value,{fcode:fcode1}, function(j) {
			if(j != ""){
				var length = j.length-1;
				var enc = j[length][0].substring(0,16);
				var options = '<option value="' + "0" + '">'+ "--Select--" + '</option>';
	...
	function getADMDiv(fcode){
		var fcode1 = fcode[0] + fcode[1] + fcode[2];
		$.post("getDivDetailsListadm?"+key+"="+value,{fcode:fcode1}, function(j) {
			if(j != ""){
				var length = j.length-1;
				var enc = j[length][0].substring(0,16);
				var options = '<option value="' + "0" + '">'+ "--Select--" + '</option>';
	...
	function getADMBde(fcode){
		var fcode1 = fcode[0] + fcode[1] + fcode[2] + fcode[3] + fcode[4] + fcode[5];
		$.post("getBdeDetailsListadm?"+key+"="+value,{fcode:fcode1}, function(j) {
			if(j != ""){
				var length = j.length-1;
				var enc = j[length][0].substring(0,16);
				var options = '<option value="' + "0" + '">'+ "--Select--" + '</option>';
...
//    	}
 ...
 //260194
    ...
    function addRemarkModel2parameter(method, id, child_id) {     
        if ($('#btnTrigger').length) {}
        else {
            var btn = document.createElement("button"); 
            btn.id = "btnTrigger";
        ...
        div.innerHTML = ''
            ...
            + '   <div class="modal-footer" align="center">'
            + '      <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>'
            + '      <input type="button" id="remarkSubmit" data-dismiss="modal" value="Submit" onclick="' + method + '(' + id + ',' + child_id + ')" class="btn btn-success" >'
            + '   </div>'
            + ' </div>'    
            + '  </div>';
    ...
    var scorbating = 0;
    function scnario_onchange(){
//$("#type_of_letter").change(function(){	
		document.getElementById('sus_no').value = "";
		document.getElementById('unit_name').value = "";
		document.getElementById('op_comd').value = "";
		document.getElementById('op_corps').value = "0";
		document.getElementById('op_div').value = "0";
		document.getElementById('op_bde').value = "0";
		...
		document.getElementById('adm_comd').value = "";
		document.getElementById('adm_corps').value = "0";
		document.getElementById('adm_div').value = "0";
		document.getElementById('adm_bde').value = "0";
		document.getElementById('loc').value = "";
		document.getElementById('nrs_code').value = "";
		document.getElementById('nrs_name').value = "";
		document.getElementById('type_of_location').value = "";
		document.getElementById('modification').value = "";
		jQuery("#cont_aname").val("");
		jQuery("#cont_dname").val("");
		jQuery("#cont_bname").val("");
		jQuery("#cont_cname").val("");
		

	
	scorbating = $("#type_of_letter").val();
	jQuery('select').off('change');		
				     	
		
		if (scorbating == 12) { 
		 document.getElementById('su_no').innerHTML = "<strong style='color: red;'>*</strong>Fmn SUS No";
		 document.getElementById('unit').innerHTML = "<strong style='color: red;'>*</strong>Fmn Name";
//Oprational
		 jQuery('select#op_comd').change(function() {
//			 alert(formation_type);
				 if (formation_type=="Division") {
					 var fcode = this.value;
				   		getOPCorps(fcode);
					 
				 }
				 else  if (formation_type=="Brigade") {
					 var fcode = this.value;
				   		getOPCorps(fcode);
...
//				alert("jjjj");
   	...
   	//Admin
		 ...
		 if(scorbating == 0 || scorbating == 6 || scorbating == 8){
//			alert("===else====");
			
			 document.getElementById('su_no').innerHTML = "<strong style='color: red;'>*</strong>SUS No";
			 document.getElementById('unit').innerHTML = "<strong style='color: red;'>*</strong>Unit Name";
		//Oprational
			 jQuery('select#op_comd').change(function() {debugger;
			 var fcode = this.value;
		   		getOPCorps(fcode);
			 ...
			 jQuery('select#op_corps').change(function() { debugger;
//			 alert(scorbating);
       			var fcode = this.value;
       			getOPDiv(fcode);
       			fcode += "000";
...
//Control
    ...
    document.getElementById("op_bde").removeAttribute("readonly");
    ...
    document.getElementById("cont_bde").removeAttribute("readonly");
    ...
    document.getElementById("adm_bde").removeAttribute("readonly");
