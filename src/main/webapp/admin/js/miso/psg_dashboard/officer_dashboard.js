...
function initializeAmChart(containerId) {
  ...
  if(containerId=="medicalDiv")
  {

     chart_medicalDiv();
    hideLoader('loader_medicalDiv');
  ...
  if(containerId=="regtDiv")
  {
	  chart_regiment();
	  hideLoader('loader_regtDiv');	  
  ...
  if(containerId=="parentlDiv")
  {
	  chart_parentlDiv();	
	  hideLoader('loader_parentlDiv');
  ...
  if(containerId=="bgDiv")
  {
	  chart_bgDiv();		 
	  hideLoader('loader_bgDiv');	 
  ...
  if(containerId=="comdDiv")
  {
	  chart_comdDiv();
	  hideLoader('loader_comdDiv');
  ...
  if(containerId=="genDiv")
  {
     chart_gender();
     hideLoader('loader_genDiv');
  ...
  if(containerId=="dosDiv")
  {
     chart_dosDiv();
     hideLoader('loader_dosDiv');     
  ...
  if(containerId=="motDiv")
  {
     chart_motDiv();    
     hideLoader('loader_motDiv');
  ...
  if(containerId=="maritalDiv")
  {
	   chart_maritalDiv();
	   hideLoader('loader_maritalDiv');	  
  ...
  if(containerId=="reliDiv")
  {
	   chart_reliDiv();	  
	   hideLoader('loader_reliDiv');
  ...
  if(containerId=="borderDiv")
  {
	   chart_boderDiv();	  
	   hideLoader('loader_borderDiv');
	   legend_border.events.on("click", function (event) {
		    var series = event.target;
		    if (chartborder) {		   
		        update_chart_med(chartborder, series.root,"databorder");
		    } else {
		        console.error("Chart not found.");
		    }
		});
...
function getpsg_chartdata( url_var,callback) {
	...
	/*function chart_medicalDiv() {
		
		am5.ready(function() {
        var root = am5.Root.new("medicalDiv");
        root._logo.dispose();
        root.setThemes([
        ...
        getpsg_chartdata("Getrk_medcatlist_off", function(data) {
...
//rk vs regiment
function chart_regiment() {

 am5.ready(function() {	
var comproot = am5.Root.new("regtDiv");
...
getpsg_chartdata("Getrk_regimentlist_off", function(data) {
...
//rk vs parent arm
function chart_parentlDiv() {
    am5.ready(function() {
var parentroot = am5.Root.new("parentlDiv");
...
getpsg_chartdata("Getrk_parent_armtlist_off", function(data) {
...
//rk vs blood group 
function chart_bgDiv() {
    am5.ready(function() {
var bgroot = am5.Root.new("bgDiv");
...
getpsg_chartdata("Getrk_blood_grouplist_off", function(data) {
...
makeSeriesbg("lt", "databg1",false);
		makeSeriesbg("capt", "databg2",false);
		makeSeriesbg("maj", "databg3",false);
		makeSeriesbg("ltcol", "databg4",false); 
		makeSeriesbg("colts", "databg5",false);
		makeSeriesbg("col", "databg6",false);
		makeSeriesbg("brig", "databg7",false);
		makeSeriesbg("majgen", "databg8",false);	
		makeSeriesbg("ltgen", "databg9",false);
		makeSeriesbg("gen", "databg10",true);
...
//rk vs comd
function chart_comdDiv() {
    am5.ready(function() {
var comdroot = am5.Root.new("comdDiv");
...
getpsg_chartdata("Getrk_commandlist_off", function(data) {
 ...
 "datacomd9": 0,
  ...
  if (total) {
	    ...
	    series.bullets.push(function () { 
	      ...
	      var totalLabel1 = am5.Label.new(comdroot, {
	       // text: "{valueY}",
	        fill: comdroot.interfaceColors.get("text"),
	        centerY: am5.p100,
	        centerX: am5.p50,
	      ...
	      totalLabel1.adapters.add("text", function(text, target) {
		
	        var dataContext = target.dataItem.dataContext;
	        var val = Math.abs(dataContext.datacomd1 + dataContext.datacomd2 + dataContext.datacomd3 
												+ dataContext.datacomd4  
	        		+dataContext.datacomd5 + dataContext.datacomd6 + dataContext.datacomd7+dataContext.datacomd8 + dataContext.datacomd9+dataContext.datacomd10 );
	        return " "+val;
	      });
...
//makeSeriescomd("rects", "datacomd8",true);
		...
		makeSeriescomd("lt", "datacomd1",false);
		makeSeriescomd("capt", "datacomd2",false);
		makeSeriescomd("maj", "datacomd3",false);
		makeSeriescomd("ltcol", "datacomd4",false);
		makeSeriescomd("col", "datacomd5",false);
		makeSeriescomd("brig", "datacomd6",false);
		makeSeriescomd("majgen", "datacomd7",false);	
		makeSeriescomd("ltgen", "datacomd8",false);
		makeSeriescomd("gen", "datacomd9",false);	
		makeSeriescomd("colts", "datacomd10",true);
...
function chart_gender() {
    am5.ready(function() {
        var genroot = am5.Root.new("genDiv");
	genroot._logo.dispose();
        ...
        getpsg_chartdata("Getrk_genderlist_off", function(data) {
            var parsedData = JSON.parse(data);
            var data = parsedData;
	if (data.length === 0) {
  // Set default values when data is empty
...
//rk vs dos
function chart_dosDiv() {
    am5.ready(function() {    
        var dosroot = am5.Root.new("dosDiv");
  ...
  chartdos.set("scrollbarY", am5.Scrollbar.new(dosroot, {
        ...
        getpsg_chartdata("Getrk_doslist_off", function(data) {
                    ...
                    series.bullets.push(function () {
                        ...
                        totalLabel.adapters.add("text", function(text, target) {
                            ...
                            var val = Math.abs(dataContext.datados1 + dataContext.datados2 + dataContext.datados3 + dataContext.datados4 
                                + dataContext.datados5 + dataContext.datados6 + dataContext.datados7 + dataContext.datados8+ dataContext.datados9+ dataContext.datados10);
                            return " " + val;
                        });
						...
						makeSeriesdos("lt", "datados1",false);
						makeSeriesdos("capt", "datados2",false);
						makeSeriesdos("maj", "datados3",false);
						makeSeriesdos("ltcol", "datados4",false);
						makeSeriesdos("colts", "datados5",false);
						makeSeriesdos("col", "datados6",false);
						makeSeriesdos("brig", "datados7",false);
						makeSeriesdos("majgen", "datados8",false);	
...
//rk vs mother tongue
function chart_motDiv() {
    am5.ready(function() { 
     	var motroot = am5.Root.new("motDiv");
		...
		//var data =${Getrk_mothertonguelist_jco_or};
	     getpsg_chartdata("Getrk_mothertonguelist_off", function(data) {
...
function chart_maritalDiv() {
    // 
    am5.ready(function() { 
        var maritalroot = am5.Root.new("maritalDiv");
        ...
        getpsg_chartdata("Getrk_marital_statuslist_off", function(data) {
            var parsedData = JSON.parse(data);
            var data = parsedData;
...
var cursor = chartmarital.set("cursor", am5xy.XYCursor.new(maritalroot, {
  behavior: "zoom"
      ...
      //rk vs religion
	function chart_reliDiv() {
    // 
    am5.ready(function() { 	
		var reliroot = am5.Root.new("reliDiv");
	    ...
	    getpsg_chartdata("Getrk_religionlist_off", function(data) {
            var parsedData = JSON.parse(data);
            var data = parsedData;
     ...
     series.data.push(defaultData);
      ...
      series.data.setAll(data);
		  ...
		  if (total) {
			      ...
			      totalLabel.adapters.add("text", function(text, target) {
			        var dataContext = target.dataItem.dataContext;
			         var val = Math.abs(dataContext.datareli1 + dataContext.datareli2 + dataContext.datareli3 + dataContext.datareli4 
				        		...
				        		dataContext.datareli9 +dataContext.datareli10);
				        return " "+val;
			      });
				...
				makeSeriesreli("lt", "datareli1",false);
			makeSeriesreli("capt", "datareli2",false);
			makeSeriesreli("maj", "datareli3",false);
			makeSeriesreli("ltcol", "datareli4",false); 
			makeSeriesreli("colts", "datareli5",false); 
			makeSeriesreli("col", "datareli6",false);
			makeSeriesreli("brig", "datareli7",false);
			makeSeriesreli("majgen", "datareli8",false);	
			makeSeriesreli("ltgen", "datareli9",false);
			makeSeriesreli("gen", "datareli10",true);
...
function 	chart_boderDiv(){
    am5.ready(function() {
      var borderroot = am5.Root.new("borderDiv");
						borderroot._logo.dispose();
        borderroot.setThemes([
            am5themes_Animated.new(borderroot)
        ]);
	 chartborder = borderroot.container.children.push(am5xy.XYChart.new(borderroot, {
						  panX: false,
						  panY: false,
						  wheelX: "panX",
						  wheelY: "zoomX",
						  layout: borderroot.verticalLayout
						}));
						
						chartborder.set("scrollbarX", am5.Scrollbar.new(borderroot, {
						  orientation: "horizontal"
						}));
							chartborder.set("scrollbarY", am5.Scrollbar.new(borderroot, {
						  orientation: "vertical"
						}));
      

        getpsg_chartdata("Getrk_borderlist_off", function(data) {
            var parsedData = JSON.parse(data);
            var data = parsedData;
if (data.length === 0) { 

  // Set default values when data is empty
  var defaultData = {"databorder": "", "databorder1": 0, "databorder2": 0};
  data.push(defaultData);
     var noDataLabel = am5.Label.new(borderroot, {
                    text: "No Data Available",
                  centerX: am5.p5000,
                   centerY: am5.p5000,
						dy :  am5.p5000,
	  				 dx :  am5.p5000,
                  fontSize: 30,
                  fill: borderroot.interfaceColors.get("text")
              });
             chartborder.seriesContainer.children.push(noDataLabel);
               
}
            var xRenderer = am5xy.AxisRendererX.new(borderroot, {});
            var xAxis = chartborder.xAxes.push(am5xy.CategoryAxis.new(borderroot, {
                categoryField: "databorder",
                renderer: xRenderer,
                tooltip: am5.Tooltip.new(borderroot, {})
            }));

            xAxis.data.setAll(data);

            var yAxis = chartborder.yAxes.push(am5xy.ValueAxis.new(borderroot, {
                min: 0,
                renderer: am5xy.AxisRendererY.new(borderroot, {})
            }));

            // Add legend
            legend_border = chartborder.children.push(am5.Legend.new(borderroot, {
                centerX: am5.p50,
                x: am5.p50
            }));
            ...
            // Add series
            function makeSeriesborder(name, fieldName, total1) {
                var name = name.toUpperCase();
                var series = chartborder.series.push(am5xy.ColumnSeries.new(borderroot, {
                    name: name,
                    stacked: true,
                    xAxis: xAxis,
                    yAxis: yAxis,
                    valueYField: fieldName,
                    categoryXField: "databorder"
                }));
 ...
 if (data.length === 0) {
    ...
    var defaultData = {"databorder": "No Data ", "databorder1": 0, "databorder2": 0};
    series.data.push(defaultData);
  } else {
    series.data.setAll(data);
  }
                ...
                series.bullets.push(function() {
                    return am5.Bullet.new(borderroot, {
                        sprite: am5.Label.new(borderroot, {
                            fill: borderroot.interfaceColors.get("alternativeText"),
                            centerY: am5.p50,
                            centerX: am5.p50
                        })
                    });
                ...
                legend_border.data.push(series);

                if (total1) {
                    series.bullets.push(function() {
                        var total1Label = am5.Label.new(borderroot, {
                            text: "{valueY}",
                            fill: borderroot.interfaceColors.get("text"),
                            centerY: am5.p100,
                            centerX: am5.p50,
                        ...
                        total1Label.adapters.add("text", function(text, target) {
                            var dataContext = target.dataItem.dataContext;
                            var val = Math.abs(dataContext.databorder1 + dataContext.databorder2 );
                            return "" + val;
                        });
          	...
          	makeSeriesborder("yes", "databorder1",false);
			makeSeriesborder("no", "databorder2",true);
var cursor = chartborder.set("cursor", am5xy.XYCursor.new(borderroot, {
  behavior: "zoomY"
}));
cursor.lineY.set("forceHidden", true);
cursor.lineX.set("forceHidden", true);
