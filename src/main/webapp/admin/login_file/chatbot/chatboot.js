...
/*const addMessageToConversation = function() {
	...
	if ( messageText.trim().length == 0 ) {
		...
		document.querySelector('.conversation__body').insertAdjacentHTML('beforeend', `<div class="conversation__bubble conversation__bubble--right">
		<p class="conversation__text">${messageText}</p></div>`);
...
// 
//	document.querySelector('.container-fluidC').classList.add('list--active');
//	
//});
...
document.querySelector('#chat1').addEventListener('click', function() {
	...
	document.querySelector('.container-fluidC').classList.remove('list--active');
